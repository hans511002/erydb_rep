...
int updateptrs(char* ptr, FdCacheType_t::iterator fdit, const ERYDBCompressInterface& decompressor)
{
	ssize_t i;
	uint32_t progress;
	...
	ERYDBDataFile* fp = fdit->second->fp;
...
void* thr_popper(ioManager *arg) {
	...
	bool locked = false;
	SPFdEntry_t fe;
	ERYDBCompressInterface decompressor;
	...
	if (iom->IOTrace())
	{
		...
		iomLogFileName << "C:/erydb/log/trace/iom." << threadId;
#else
		threadId=pthread_self();
		iomLogFileName << "/var/log/erydb/trace/iom." << threadId;
#endif
	...
	FdCacheType_t::iterator fdit;
	ERYDBDataFile* fp = 0;
	uint32_t maxCompSz = ERYDBCompressInterface::maxCompressedSize(iom->blocksPerRead * BLOCK_SIZE);
...
#ifdef ERYDB_COMP_USE_CMP_SUFFIX
			if (compType != 0)
			{
	...
	fFileOp(false)
{
	...
	if (temp > 0) {
		fFDCacheTrace=true;
#ifdef _MSC_VER
		FDTraceFile().open("C:/erydb/log/trace/fdcache", ios_base::ate | ios_base::app);
#else
		FDTraceFile().open("/var/log/erydb/trace/fdcache", ios_base::ate | ios_base::app);
#endif
	}
