...
#ifdef _MSC_VER
		brpLogFileName << "C:/erydb/log/trace/brp." << tid;
#else
		brpLogFileName << "/var/log/erydb/trace/brp." << tid;
#endif
...
int BlockRequestProcessor::check(const BRM::InlineLBIDRange& range, const BRM::QueryContext &ver, const BRM::VER_t txn, const int compType, uint32_t& lbidCount) {
	...
	if (rqstBlk.RequestStatus() == fileRequest::BRM_LOOKUP_ERROR)
		throw logging::ERYDBExcept(logging::ERYDBErrorInfo::instance()->errorMsg(logging::ERR_BRM_LOOKUP), logging::ERR_BRM_LOOKUP);
	else if (rqstBlk.RequestStatus() == fileRequest::FS_EINVAL)
		throw logging::ERYDBExcept(logging::ERYDBErrorInfo::instance()->errorMsg(logging::ERR_O_DIRECT),
								 logging::ERR_O_DIRECT);
	else if (rqstBlk.RequestStatus() == fileRequest::FS_ENOENT)
		throw logging::ERYDBExcept(
			logging::ERYDBErrorInfo::instance()->errorMsg(logging::ERR_ENOENT),
			logging::ERR_ENOENT);
