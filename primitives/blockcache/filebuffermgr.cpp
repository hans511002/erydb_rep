...
#ifdef _MSC_VER
	fLog.open("C:/erydb/log/trace/bc", ios_base::app | ios_base::ate);
#else
	fLog.open("/var/log/erydb/trace/bc", ios_base::app | ios_base::ate);
#endif
}
...
void FileBufferMgr::flushPartition(const vector<OID_t> &oids, const set<BRM::LogicalPartition> &partitions)
{
	...
	for (i = 0; i < count; i++) {
		...
		for (currentExtent = 0; currentExtent < extents.size(); currentExtent++) {
			...
			LogicalPartition logicalPartNum(range.dbRoots, range.partitionNum, range.segmentNum);
...
int FileBufferMgr::insert(const BRM::LBID_t lbid, const BRM::VER_t ver, const uint8_t* data)
{
	...
	if (fCacheSize > maxCacheSize())
	{
		...
		erydbassert(iter != fbSet.end());
		pi = iter->poolIdx;
		erydbassert(pi < maxCacheSize());
		erydbassert(pi < fFBPool.size());
	...
	erydbassert(fCacheSize <= maxCacheSize());
// 	erydbassert(fCacheSize == fbSet.size());
// 	erydbassert(fCacheSize == fbList.size());
	return ret;
}
