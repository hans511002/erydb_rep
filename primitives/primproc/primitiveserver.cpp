...
void buildOidFileName(const BRM::OID_t oid,const uint16_t dbr,const uint16_t partNum,const uint32_t segNum,char* file_name)
{
	...
	if (fileOp.getFileName(oid, file_name, dbr, partNum, segNum) != WriteEngine::NO_ERROR)
	{
		file_name[0]=0;
					...
					uint32_t* rCount)
{
	...
	try {
		...
		erydbassert(range.size<=blocksReadAhead);
...
#endif
	} catch (logging::ERYDBExcept& iex) {
		cerr << "DictScanJob caught an ERYDBException: " << iex.what() << endl;
	...
	*/
	...
	void operator()()
	{
		...
		if (bRotateDest) {
			...
			// but disable destination rotation since not in erydb.xml.
		...
		for (;;) {
			...
			try {
				if (bs->length() != 0) {
					erydbassert(bs->length() >= sizeof(ISMPacketHeader));
					...
					case DICT_TOKEN_BY_SCAN_COMPARE: {
						erydbassert(bs->length() >= sizeof(TokenByScanRequestHeader));
