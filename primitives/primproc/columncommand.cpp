...
// 		erydbassert(outMsg->NVALS == bpp->ridCount);
...
#endif
	bs >> tmp8;
	colType.colDataType = (execplan::erydbSystemCatalog::ColDataType) tmp8;
	bs >> tmp8;
...
void ColumnCommand::projectResult()
{
	...
	erydbassert(primMsg->NVALS == outMsg->NVALS);
	erydbassert(outMsg->NVALS == bpp->ridCount);
...
***********************************************************/
const uint64_t ColumnCommand::getEmptyRowValue( const execplan::erydbSystemCatalog::ColDataType dataType, const int width ) const
{
	uint64_t emptyVal = 0;
	int offset;

	offset = ( dataType == execplan::erydbSystemCatalog::VARCHAR )? -1 : 0;
	switch ( dataType )
	{
