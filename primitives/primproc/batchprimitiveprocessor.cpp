...
// 				cout << "deserialized " << smallSideRGs.size() << " small-side rowgroups\n";
				erydbassert(smallSideRGs.size() == joinerCount);
...
#pragma pack(pop)
	...
	if (ot == ROW_GROUP) {
		bs >> joinerNum;
		erydbassert(joinerNum < joinerCount);
...
// 			cout << "copying full row data for joiner " << joinerNum << endl;
			...
			for nullFlag. */
// 			erydbassert(ssrdPos[joinerNum] + (count * smallSideRowLengths[joinerNum]) <=
...
// 			" touchedBlocks=" << touchedBlocks << endl;
	...
	{
		writeErrorMsg(ERYDBErrorInfo::instance()->errorMsg(ERR_MORE_THAN_1_ROW), ERR_MORE_THAN_1_ROW, false);
	}
...
void BatchPrimitiveProcessor::asyncLoadProjectColumns()
{
	...
	//    where # is a multiple of ColScanReadAheadBlocks in erydb.xml
