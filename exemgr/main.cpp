 ...
 * result sets. It sits in a loop waiting for erydbExecutionPlan
 * from the FEP. It then passes the erydbExecutionPlan to the
 ...
 * into the erydbResultFactory, which outputs a erydbResultSet.
 * The ExecutionPlanManager passes the erydbResultSet into the
 ...
 * sends the VendorResultSet back to the erydb Database Connector
...
const uint32_t flagsWantOutput = (0xffffffff &
	~erydbSelectExecutionPlan::TRACE_TUPLE_AUTOSWITCH &
...
//   the same erydbSystemCatalog object for that session.  Here, we
//   define a map to track how many threads are sharing each session, so
//   that we know when we can safely delete a erydbSystemCatalog object
...
private:
	...
	//...erydbSystemCatalog objects are deleted.
	...
	static void decThreadCntPerSession(uint32_t sessionId)
	{
		...
		if (mapIter != threadCntPerSessionMap.end())
		{
			if (--mapIter->second == 0)
			{
				...
				erydbSystemCatalog::removeerydbSystemCatalog((sessionId ^ 0x80000000));
			}
		}
	}
	...
	void setRMParms ( const erydbSelectExecutionPlan::RMParmVec& parms )
	{
		for (erydbSelectExecutionPlan::RMParmVec::const_iterator it = parms.begin();
				it != parms.end(); ++it)
		{
			switch (it->id)
			{
	...
	void buildSysCache(const erydbSelectExecutionPlan& csep, boost::shared_ptr<erydbSystemCatalog> csc)
	{
		const erydbSelectExecutionPlan::ColumnMap& colMap = csep.columnMap();
		...
		erydbSelectExecutionPlan::SelectList::const_iterator subIt;
		for (subIt = csep.derivedTableList().begin(); subIt != csep.derivedTableList().end(); ++ subIt)
		{
			buildSysCache(*(dynamic_cast<erydbSelectExecutionPlan*>(subIt->get())), csc);
		}
	}
	...
	void operator()()
	{
		...
		try
		{
			for (;;)
			{
				...
				else if (bs.length() < 4) //Not a erydbSelectExecutionPlan
				{
					if (gDebug)
...
new_plan:
				...
				if (!csep.isInternal())
				{
					boost::shared_ptr<erydbSystemCatalog> csc =
						erydbSystemCatalog::makeerydbSystemCatalog(csep.sessionID());
				...
				erydbSystemCatalog::OID tableOID;
				...
				for (;;)
				{
					...
					try // @bug2244: try/catch around fIos.write() calls responding to qb command
					{
						...
						else if (qb == 3) //special option-UM wants job stats string
						{
							...
							statsString = formatQueryStats(
								...
								"Query Stats",
								false,
								!(csep.traceFlags() & erydbSelectExecutionPlan::TRACE_TUPLE_OFF),
								(csep.traceFlags() & erydbSelectExecutionPlan::TRACE_LOG),
								totalRowCount
								);

							bs.restart();
							bs << statsString;
							if ((csep.traceFlags() & erydbSelectExecutionPlan::TRACE_LOG) != 0)
							{
								bs << jl->extendedInfo();
						...
						else // (qb > 3)
						{
							...
							tableOID = static_cast<erydbSystemCatalog::OID>(qb);
						}
					}
					catch (std::exception& ex)
					{
					...
					for (;;)
					{
						...
						msgHandler.stop();
						if (jl->status()) {
							ERYDBErrorInfo* errInfo = ERYDBErrorInfo::instance();
						...
						{
							if (csep.traceFlags() & erydbSelectExecutionPlan::TRACE_NO_ROWS3)
							{
				...
				if ((csep.traceFlags() & flagsWantOutput) && (csep.sessionID() < 0x80000000))
				{
					...
					if (csep.traceFlags() &
						erydbSelectExecutionPlan::TRACE_NO_ROWS4)
					{
						cout << endl;
						...
						cout << endl;
							...
							erydbSelectExecutionPlan::TRACE_NO_ROWS3)
					{
						cout << endl;
