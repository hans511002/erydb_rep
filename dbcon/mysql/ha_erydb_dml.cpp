...
/*
 * $Id: ha_erydb_dml.cpp 9711 2013-07-23 21:01:27Z chao $
 */
...
{
	return execplan::erydbSystemCatalog::erydb_tid2sid(tid);
}
...
int ProcessCommandStatement(THD *thd, string& dmlStatement, cal_connection_info& ci, std::string schema="")
{
	...
	erydbDMLPackage* pDMLPackage; 
	//@Bug 2721 and 2722. Log the statement before issuing commit/rollback
	if ( dmlStatement == "LOGGING" )
	{	
		VendorDMLStatement cmdStmt(erydb_mysql_query_str(thd), DML_COMMAND, sessionID);
		cmdStmt.set_Logging( false );
		pDMLPackage = erydbDMLFactory::makeerydbDMLPackageFromMysqlBuffer(cmdStmt);
...
int doProcessInsertValues ( TABLE* table, uint32_t size, cal_connection_info& ci, bool lastBatch = false )
{
		...
		VendorDMLStatement dmlStmts(erydb_mysql_query_str(thd), DML_INSERT, table->s->table_name.str,
                table->s->db.str, size, ci.colNameList.size(), ci.colNameList,
                ci.tableValuesMap, ci.nullValuesBitset, sessionID);

		erydbDMLPackage* pDMLPackage = erydbDMLFactory::makeerydbDMLPackageFromMysqlBuffer(dmlStmts);
		...
		{
			rc = -1;
			string emsg("erydb DML package cannot build. ");
            thd->get_stmt_da()->set_overwrite_status(true);
...
int ha_erydb_impl_write_last_batch(TABLE* table, cal_connection_info& ci, bool abort)
{
		int rc = 0;
		THD *thd = current_thd;
		std::string command;
...
int ha_erydb_impl_write_row_(uchar *buf, TABLE* table, cal_connection_info& ci, ha_rows& rowsInserted)
{
    int rc = 0;
...
int ha_erydb_impl_write_batch_row_(uchar *buf, TABLE* table, cal_impl_if::cal_connection_info& ci)
{
	ByteStream rowData;
	int rc = 0;
    ...
    while (colpos < ci.columnTypes.size()) //test bitmap for null values
    {
		...
		for (uint8_t i = 0; i < numLoop; i++)
		{
			...
			if (ci.columnTypes[colpos].constraintType != erydbSystemCatalog::NOTNULL_CONSTRAINT)
			{
			...
			{
				case erydbSystemCatalog::DATE: //date fetch
				{
				
					if (nullVal && (ci.columnTypes[colpos].constraintType != erydbSystemCatalog::NOTNULL_CONSTRAINT))
					{
				...
				case erydbSystemCatalog::UFLOAT:
				{
					...
					{
						float val = *((float*)buf);
						if ((fabs(val) > (1.0 / ERYDB_pow[4])) && (fabs(val) < (float) ERYDB_pow[6]))
						{
 ...
 std::string  ha_erydb_impl_viewtablelock( cal_impl_if::cal_connection_info& ci, execplan::erydbSystemCatalog::TableName& tablename)
 {
	THD *thd = current_thd;
	ulong sessionID = tid2sid(thd->thread_id);
	erydbDMLPackage* pDMLPackage; 
...
// lock is released.
//------------------------------------------------------------------------------
 std::string  ha_erydb_impl_cleartablelock(
	...
	uint64_t tableLockID)
 {
	...
	// Perform preliminary setup.  erydbDMLPackage expects schema and table
	...
	VendorDMLStatement cmdStmt(dmlStatement, DML_COMMAND, sessionID);
	erydbDMLPackage* pDMLPackage =
		erydbDMLFactory::makeerydbDMLPackageFromMysqlBuffer( cmdStmt );
...
int ha_erydb_impl_commit_ (handlerton *hton, THD *thd, bool all, cal_connection_info& ci )
{
	int rc = 0;
...
int ha_erydb_impl_rollback_ (handlerton *hton, THD *thd, bool all, cal_connection_info& ci)
{
	int rc = 0;
#ifdef ERYDB_DEBUG
	cout << "ROLLBACK" << endl;
...
int ha_erydb_impl_close_connection_ (handlerton *hton, THD *thd, cal_connection_info& ci )
{
	int rc = 0;
#ifdef ERYDB_DEBUG
	...
	rc = ProcessCommandStatement(thd, command, ci);
	// @bug 1622. remove erydbsystemcatalog and close the socket when session quit.
	// @info when erydb process a select query, an alter table phase is involved in
