...
ST_FIELD_INFO is_erydb_columns_fields[] =
{
...
static int is_erydb_columns_fill(THD *thd, TABLE_LIST *tables, COND *cond)
{
    ...
    {
        execplan::erydbSystemCatalog::RIDList column_rid_list = systemCatalogPtr->columnRIDs((*it).second);
        for (size_t col_num = 0; col_num < column_rid_list.size(); col_num++)
        {
            execplan::erydbSystemCatalog::TableColName tcn = systemCatalogPtr->colName(column_rid_list[col_num].objnum);
            execplan::erydbSystemCatalog::ColType ct = systemCatalogPtr->colType(column_rid_list[col_num].objnum);
			...
			if (ct.constraintType != execplan::erydbSystemCatalog::NOTNULL_CONSTRAINT)
			{
...
static int is_erydb_columns_plugin_init(void *p)
{
    ST_SCHEMA_TABLE *schema = (ST_SCHEMA_TABLE*) p;
    schema->fields_info = is_erydb_columns_fields;
    schema->fill_table = is_erydb_columns_fill;
    return 0;
}

static struct st_mysql_information_schema is_erydb_columns_plugin_version =
{ MYSQL_INFORMATION_SCHEMA_INTERFACE_VERSION };

maria_declare_plugin(is_erydb_columns_plugin)
{
    MYSQL_INFORMATION_SCHEMA_PLUGIN,
    &is_erydb_columns_plugin_version,
    "ERYDB_COLUMNS",
    "EryDB, Inc.",
    "An information schema plugin to list EryDB columns",
    PLUGIN_LICENSE_GPL,
    is_erydb_columns_plugin_init,
