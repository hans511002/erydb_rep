...
/*
 * $Id: ha_erydb_impl.cpp 9642 2013-06-24 14:57:42Z rdempsey $
 */
...
#define NEED_ERYDB_INTERFACE
#include "ha_erydb_impl.h"
...
namespace
{
// erydb vtable non-support error message
const string erydb_autoswitch_warning = "The query includes syntax that is not supported by EryDB distributed mode. The execution was switched to standard mode with downgraded performance.";
...
void storeNumericField(Field** f, int64_t value, erydbSystemCatalog::ColType& ct)
{
	...
	switch ((*f)->type())
	{
		case MYSQL_TYPE_NEWDECIMAL:
		{
			...
			// @bug4388 stick to EryDB's scale in case mysql gives wrong scale due
		...
		case MYSQL_TYPE_VARCHAR:
		{
			...
			char tmp[25];
			if (ct.colDataType == erydbSystemCatalog::DECIMAL)
...
//#endif
		sm_stat = sm::ERYDB_INTERNAL_ERROR;
	} catch (...) {
// @bug 2244. Always log this msg for now, as we try to track down when/why we are
//			losing socket connection with ExeMgr
//#ifdef ERYDB_DEBUG
...
//#endif
	...
	if (sm_stat == sm::STATUS_OK)
	{
		...
		std::vector<erydbSystemCatalog::ColType> &colTypes = ti.tpl_scan_ctx->ctp;
		int64_t intColVal = 0;
		...
		if (ti.tpl_scan_ctx->rowsreturned == 0 &&
			 (ti.tpl_scan_ctx->traceFlags & execplan::erydbSelectExecutionPlan::TRACE_TUPLE_OFF))
		{
		...
		for (int p = 0; p < num_attr; p++, f++)
		{
			...
			erydbSystemCatalog::ColType colType(colTypes[p]);

			// table mode handling
			if (ti.tpl_scan_ctx->traceFlags & execplan::erydbSelectExecutionPlan::TRACE_TUPLE_OFF)
			{
			...
			if (row.isNullValue(s) && colType.precision != -16)
			{
				...
				if (colType.colDataType == erydbSystemCatalog::CHAR ||
					  colType.colDataType == erydbSystemCatalog::VARCHAR ||
					  colType.colDataType == erydbSystemCatalog::VARBINARY)
				{
					Field_varstring* f2 = (Field_varstring*)*f;
			...
			switch (colType.colDataType)
			{
				...
				case erydbSystemCatalog::DATETIME:
				{
                     ...
                     * EryDB doesn't fully support mseconds yet so if
				...
				case erydbSystemCatalog::VARBINARY:
				{
					...
					if (current_thd->variables.erydb_varbin_always_hex)
					{
						uint32_t l;
	...
	else if (sm_stat == sm::ERYDB_INTERNAL_ERROR)
	{
		ti.moreRows = false;
	...
	else if ((uint32_t)sm_stat == logging::ERR_LOST_CONN_EXEMGR)
	{
		...
		sm::sm_init(tid2sid(current_thd->thread_id), &ci->cal_conn_hndl,
					current_thd->variables.erydb_local_query);
		erydbassert(ci->cal_conn_hndl != 0);
		ci->rc = rc;
	}
...
void makeUpdateScalarJoin(const ParseTree* n, void* obj)
{
	...
	erydbSystemCatalog::TableAliasName* updatedTables = reinterpret_cast<erydbSystemCatalog::TableAliasName*>(obj);
	if ( scLeft && scRight )
	{
...
uint32_t doUpdateDelete(THD *thd)
{
	...
	bool notExec = (
#ifdef WITH_WSREP
		wsrep_erydb_type && thd->isWsrepQuery && thd->erydb_vtable.isErydb);
#else
		thd->erydb_vtable.isErydb);
#endif
 
		//@bug 5660. Error out DDL/DML on slave node, or on local query node
	if (notExec || (ci->isSlaveNode && !thd->slave_thread))
	{
	...
	thd->erydb_vtable.isEryDBDML = true;
	...
	{
		string msg = string("Erydb User Priority - ") + e.what();
	...
	for (; tables; tables= tables->next_local)
	{
		if (tables->view)
		{
			...
			if (((thd->lex)->sql_command == SQLCOM_UPDATE) || ((thd->lex)->sql_command == SQLCOM_UPDATE_MULTI))
				...
				args.add("Delete");
			string emsg = logging::ERYDBErrorInfo::instance()->errorMsg(ERR_DML_VIEW, args);
...
/*
		...
		if ((strcmp((*tables->table->s->db_plugin)->name.str, "EryDB") != 0) && (strcmp((*tables->table->s->db_plugin)->name.str, "MEMORY") != 0) &&
		...
		{
			Message::Args args;
			args.add("Non erydb table(s)");
			string emsg(ERYDBErrorInfo::instance()->errorMsg(ERR_DML_NOT_SUPPORT_FEATURE, args));
			setError(current_thd, ER_CHECK_NOT_IMPLEMENTED, emsg);
			return ER_CHECK_NOT_IMPLEMENTED;
		}
*/
...
//	string dmlStmt="";
	string dmlStmt=string(erydb_mysql_query_str(thd));
	string schemaName;
	string tableName("");
	string aliasName("");
	...
	execplan::SCSEP updateCP(new execplan::erydbSelectExecutionPlan());
...
//      dmlStmt += "update ";
        updateCP->queryType(erydbSelectExecutionPlan::UPDATE);
...
//            dmlStmt += string(item->name) + "=";
            ...
            else if (strcasecmp(tableName.c_str(), tmpTableName.c_str()) != 0)
            {
                ...
                string emsg(ERYDBErrorInfo::instance()->errorMsg(ERR_UPDATE_NOT_SUPPORT_FEATURE));
...
//                }
            ...
            else if (value->type() == Item::WINDOW_FUNC_ITEM)
            {
                setError(thd, ER_INTERNAL_ERROR,
                         logging::ERYDBErrorInfo::instance()->errorMsg(ERR_WF_UPDATE));
                return ER_CHECK_NOT_IMPLEMENTED;
                //return 0;
            }
            else
...
//                dmlStmt += ", ";
	...
	{
//		dmlStmt = string(erydb_mysql_query_str(thd));
		updateCP->queryType(erydbSelectExecutionPlan::DELETE);
		ci->stats.fQueryType = updateCP->queryType();
	}
...
//		setError(thd, ER_UNKNOWN_TABLE, ie.what());
		...
		setError(thd, ER_INTERNAL_ERROR,
					logging::ERYDBErrorInfo::instance()->errorMsg(ERR_SYSTEM_CATALOG) + ex.what());
		return ER_INTERNAL_ERROR;
	}

	ci->tableOid = roPair.objnum;
	erydbDMLPackage* pDMLPackage = 0;
//	dmlStmt += ";";
	...
	if (( (thd->lex)->sql_command == SQLCOM_UPDATE ) || ( (thd->lex)->sql_command == SQLCOM_UPDATE_MULTI ) )
	{
		...
		pDMLPackage = erydbDMLFactory::makeerydbUpdatePackageFromMysqlBuffer( dmlStatement, updateStmt );
	}
	else if ((thd->lex)->sql_command == SQLCOM_DELETE_MULTI) //@Bug 6121 error out on multi tables delete.
	{
		if ( (thd->lex->select_lex.join) != 0)
		{
			...
			if (deleteTable->num_of_tables == 1)
			{
				...
				qualifiedTablName->fName = tableName;
				qualifiedTablName->fSchema = schemaName;
				pDMLPackage = erydbDMLFactory::makeerydbDMLPackageFromMysqlBuffer(dmlStatement);
			}
			else
			{
	...
	string origStmt(erydb_mysql_query_str(thd));
	origStmt += ";";
	...
	//thd->erydb_vtable.isEryDBDML = true;
	THD::erydb_state origState = thd->erydb_vtable.vtable_state;
	//if (( (thd->lex)->sql_command == SQLCOM_UPDATE ) || ( (thd->lex)->sql_command == SQLCOM_UPDATE_MULTI ) )
	{
		gp_walk_info gwi;
		thd->erydb_vtable.vtable_state = THD::ERYDB_CREATE_VTABLE;
		gwi.thd = thd;
		//updateCP->subType (erydbSelectExecutionPlan::SINGLEROW_SUBS); //set scalar
		updateCP->subType (erydbSelectExecutionPlan::SELECT_SUBS);
		//@Bug 2975.
		SessionManager sm;
		BRM::TxnID txnID;
		...
		updateCP->data(erydb_mysql_query_str(thd));
		try {
		...
		{
			if (thd->erydb_vtable.isUpdateWithDerive)
			{
				...
				thd->erydb_vtable.vtable_state = origState;
				return 0;
			}

			//check different error code
			// avoid double set ERYDB error
			string emsg;
			if (gwi.parseErrorText.find("ERYDB-") == string::npos)
			{
				Message::Args args;
				args.add(gwi.parseErrorText);
				emsg = ERYDBErrorInfo::instance()->errorMsg(ER_INTERNAL_ERROR, args);
			}
			else
			{
...
//cout<< "Plan before is " << endl << *updateCP << endl;
		...
		erydbSelectExecutionPlan::TableList tbList = updateCP->tableList();

		erydbSelectExecutionPlan::TableList::iterator iter = tbList.begin();
		bool notFirst = false;
		...
		if ( notFirst )
		{
			erydbSystemCatalog::TableAliasName tn = make_aliastable(schemaName, tableName, aliasName);
			iter = tbList.begin();
		...
		erydbSelectExecutionPlan::ReturnedColumnList returnedCols = updateCP->returnedCols();
		erydbSelectExecutionPlan::ReturnedColumnList::iterator coliter = returnedCols.begin();
		...
		if (( (thd->lex)->sql_command == SQLCOM_DELETE ) || ( (thd->lex)->sql_command == SQLCOM_DELETE_MULTI ) ) 
		{
			...
			erydbSystemCatalog::TableName deleteTableName;
			deleteTableName.schema = schemaName;
			deleteTableName.table = tableName;
			erydbSystemCatalog::RIDList colrids;
			try {
			...
			{
				erydbSystemCatalog::ColType ct = csc->colType(colrids[j].objnum);
				if (ct.colDataType == erydbSystemCatalog::VARBINARY)
					continue;
			...
			erydbSystemCatalog::TableColName tcn = csc->colName(colrids[minWidthColOffset].objnum);
			...
			try
			{
				...
				while (isTimeOut)
				{
					...
					if (b == 0 && thd->killed > 0 && isTimeOut)
					{
						...
						erydbDMLPackage* pDMLPackage = erydbDMLFactory::makeerydbDMLPackageFromMysqlBuffer(cmdStmt);
...
//						break;
	...
	if (b != dmlpackageprocessor::DMLPackageProcessor::ACTIVE_TRANSACTION_ERROR)
	{
		...
		if ((useHdfs) && (b == 0))
			...
			command = "ROLLBACK";
        else if ((b == dmlpackageprocessor::DMLPackageProcessor::ERYDBRANGE_WARNING) && thd->is_strict_mode())
            command = "ROLLBACK";
		else if ((!(current_thd->variables.option_bits & (OPTION_NOT_AUTOCOMMIT | OPTION_BEGIN))) && (( b == 0 ) || (b == dmlpackageprocessor::DMLPackageProcessor::ERYDBRANGE_WARNING)) )
			command = "COMMIT";
		else if (( b != 0 ) && (b != dmlpackageprocessor::DMLPackageProcessor::ERYDBRANGE_WARNING) )
			command = "ROLLBACK";
		else
			command = "";
	...
	if ( ( b != 0 ) && (b != dmlpackageprocessor::DMLPackageProcessor::ERYDBRANGE_WARNING))
	{
    ...
    if (b == dmlpackageprocessor::DMLPackageProcessor::ERYDBRANGE_WARNING)
    {
...
//	}
	...
	{
		string msg = string("Erydb Query Stats - ") + e.what();
		push_warning(thd, Sql_condition::WARN_LEVEL_WARN, 9999, msg.c_str());
	}
	delete ci->dmlProc;
	ci->dmlProc = NULL;
							...
							char* is_null, char* error)
{
	...
	ci->traceFlags |= (oldTrace & erydbSelectExecutionPlan::TRACE_TUPLE_OFF);
	ci->traceFlags |= (oldTrace & erydbSelectExecutionPlan::TRACE_TUPLE_AUTOSWITCH);
	return oldTrace;
}
					...
					char* is_null, char* error)
{
	...
	if (!thd->erydb_vtable.cal_conn_info)
		thd->erydb_vtable.cal_conn_info = (void*)(new cal_connection_info());
	cal_connection_info* ci = reinterpret_cast<cal_connection_info*>(thd->erydb_vtable.cal_conn_info);		erydbSystemCatalog::TableName tableName;
	if ( args->arg_count == 2 )
	{
	...
	string lockinfo = ha_erydb_impl_viewtablelock(*ci, tableName);
					...
					char* is_null, char* error)
{
	...
	cal_connection_info* ci = reinterpret_cast<cal_connection_info*>(
		thd->erydb_vtable.cal_conn_info);
	...
	string lockinfo = ha_erydb_impl_cleartablelock(*ci, uLockID);
					...
					char* is_null, char* error)
{
	...
	boost::shared_ptr<erydbSystemCatalog> csc = erydbSystemCatalog::makeerydbSystemCatalog(tid2sid(thd->thread_id));
	csc->identity(execplan::erydbSystemCatalog::FE);
	...
	{
		setError(thd, ER_INTERNAL_ERROR, ERYDBErrorInfo::instance()->errorMsg(ERR_EXCEED_LIMIT));
		return nextVal;
	}
...
int ha_erydb_impl_open(const char *name, int mode, uint32_t test_if_locked)
{
	IDEBUG ( cout << "ha_erydb_impl_open: " << name << ", " << mode << ", " << test_if_locked << endl );
	Config::makeConfig();
	return 0;
}

int ha_erydb_impl_close(void)
{
	IDEBUG( cout << "ha_erydb_impl_close" << endl );
	return 0;
}

int ha_erydb_impl_discover_existence(const char *schema, const char *name)
{
	boost::shared_ptr<erydbSystemCatalog> csc = erydbSystemCatalog::makeerydbSystemCatalog();
	try
	{
		const erydbSystemCatalog::OID oid = csc->lookupTableOID(make_table(schema, name));
		if (oid)
			return 1;
	}
	catch ( ... )
...
int ha_erydb_impl_rnd_init(TABLE* table)
{
#ifdef DEBUG_SETENV
	string home(getenv("HOME"));
	if (!getenv("ERYDB_HOME"))
	{
		string ErydbHome(home + "/erydb/etc/");
		setenv("ERYDB_HOME", ErydbHome.c_str(), 1);
	}

	if (!getenv("ERYDB_CONFIG_FILE"))
	{
		string erydbConfigFile(home + "/erydb/etc/erydb.xml");
		setenv("ERYDB_CONFIG_FILE", erydbConfigFile.c_str(), 1);
	}

	if (!getenv("ERYDB_CSC_IDENT"))
		setenv("ERYDB_CSC_IDENT", "dm", 1);
#endif
    ...
    if (bSystemQueryReady == 0)
	{
		...
		thd->erydb_vtable.vtable_state = THD::ERYDB_ERROR;
		return ER_INTERNAL_ERROR;
	}
	else
	if (bSystemQueryReady < 0)
	...
	thd->erydb_vtable.hasEryDBTable = true;

	/* If this node is the slave, ignore DML to ERYDB tables */
	bool notExec = (
#ifdef WITH_WSREP
		wsrep_erydb_type && thd->isWsrepQuery && thd->erydb_vtable.isErydb);
#else
	...
	if (thd->erydb_vtable.vtable_state == THD::ERYDB_SELECT_VTABLE &&
	    string(table->s->table_name.str).find("$vtable") != 0)
		return 0;

	// return error is error status is already set
	if (thd->erydb_vtable.vtable_state == THD::ERYDB_ERROR)
		return ER_INTERNAL_ERROR;

	// by pass the extra union trips. return 0
	if (thd->erydb_vtable.isUnion && thd->erydb_vtable.vtable_state == THD::ERYDB_CREATE_VTABLE)
		return 0;
	...
	if (thd->erydb_vtable.call_sp && (thd->lex)->sql_command != SQLCOM_ALTER_TABLE)
	{
		setError(thd, ER_CHECK_NOT_IMPLEMENTED, "This stored procedure syntax is not supported by Erydb in this version");
		thd->erydb_vtable.vtable_state = THD::ERYDB_ERROR;
		return ER_INTERNAL_ERROR;
	}

	// mysql reads table twice for order by
	if (thd->erydb_vtable.vtable_state == THD::ERYDB_REDO_PHASE1 ||
		thd->erydb_vtable.vtable_state == THD::ERYDB_ORDER_BY)
		return 0;
	...
	if (thd->erydb_vtable.vtable_state == THD::ERYDB_CREATE_VTABLE &&
	    !thd->erydb_vtable.isNewQuery)
		return 0;
	...
	if (thd->erydb_vtable.vtable_state == THD::ERYDB_DISABLE_VTABLE ||
		(thd->slave_thread && thd->erydb_vtable.vtable_state == THD::ERYDB_INIT))
	{
		ci->traceFlags |= erydbSelectExecutionPlan::TRACE_TUPLE_OFF;
		thd->erydb_vtable.vtable_state = THD::ERYDB_DISABLE_VTABLE;
	}
	else
	{
		ci->traceFlags = (ci->traceFlags | erydbSelectExecutionPlan::TRACE_TUPLE_OFF)^
		                  erydbSelectExecutionPlan::TRACE_TUPLE_OFF;
	}

	bool localQuery = (thd->variables.erydb_local_query>0 ? true : false);
	...
	if (thd->erydb_vtable.vtable_state == THD::ERYDB_DISABLE_VTABLE)
	{
		...
		try {
			...
			setError(thd, ER_INTERNAL_ERROR, ERYDBErrorInfo::instance()->errorMsg(ERR_LOST_CONN_EXEMGR));
		...
		if (ti.csep.get() == 0)
		{
			ti.csep.reset(new erydbSelectExecutionPlan());
		...
		if (ci->tableMap.size() == 1)
			ti.csep->data(erydb_mysql_query_str(thd));
	}
	// vtable mode
	else
	{
		//if (!ci->cal_conn_hndl || thd->erydb_vtable.vtable_state == THD::ERYDB_CREATE_VTABLE)
		if ( thd->erydb_vtable.vtable_state == THD::ERYDB_CREATE_VTABLE)
		{
		...
		erydbassert(ci->cal_conn_hndl->exeMgr != 0);
		...
		if (thd->erydb_vtable.vtable_state != THD::ERYDB_SELECT_VTABLE)
		{
			...
			if (!csep)
				csep.reset(new erydbSelectExecutionPlan());
			...
			if (thd->erydb_vtable.isInsertSelect)
				csep->queryType(erydbSelectExecutionPlan::INSERT_SELECT);
			...
			if (thd->erydb_vtable.impossibleWhereOnUnion)
				return 0;
			string query;
			query.assign(thd->erydb_vtable.original_query.ptr(),
	...
	thd->erydb_vtable.isNewQuery = false;
	...
	if ((thd->erydb_vtable.vtable_state == THD::ERYDB_SELECT_VTABLE)||
	    (thd->erydb_vtable.vtable_state == THD::ERYDB_DISABLE_VTABLE) ||
	    (thd->erydb_vtable.vtable_state == THD::ERYDB_REDO_QUERY))
	{
		if (ti.tpl_ctx == 0)
		{
		...
		try {
			tableid = execplan::ERYDB_VTABLE_ID;
		} catch (...) {
		...
		if ((ti.tpl_scan_ctx->ctp).size() == 0)
		{
			...
			{
				erydbSystemCatalog::ColType ctype;
			...
			if (thd->erydb_vtable.vtable_state == THD::ERYDB_DISABLE_VTABLE)
			{
				erydbSystemCatalog::RIDList oidlist = csc->columnRIDs(make_table(table->s->db.str, table->s->table_name.str), true);
				...
				{
					erydbSystemCatalog::ColType ctype = csc->colType(oidlist[j].objnum);
...
int ha_erydb_impl_rnd_next(uchar *buf, TABLE* table)
{
	THD* thd = current_thd;
	...
	{
		erydbSystemCatalog::removeerydbSystemCatalog(tid2sid(thd->thread_id));
		return ER_INTERNAL_ERROR;
	}

	erydbassert(ti.msTablePtr == table);
...
int ha_erydb_impl_rnd_end(TABLE* table)
{
	int rc = 0;
	THD* thd = current_thd;
	...
	thd->erydb_vtable.isNewQuery = true;

	if (thd->erydb_vtable.cal_conn_info)
		ci = reinterpret_cast<cal_connection_info*>(thd->erydb_vtable.cal_conn_info);

	if (thd->erydb_vtable.vtable_state == THD::ERYDB_ORDER_BY )
	{
		thd->erydb_vtable.vtable_state = THD::ERYDB_SELECT_VTABLE;	// flip back to normal state
		return rc;
	}
	if (thd->erydb_vtable.vtable_state == THD::ERYDB_REDO_PHASE1 && thd->erydb_vtable.mysql_optimizer_off)
		return rc;
...
int ha_erydb_impl_create(const char *name, TABLE *table_arg, HA_CREATE_INFO *create_info)
{
	THD *thd = current_thd;
	if (!thd->erydb_vtable.cal_conn_info)
		thd->erydb_vtable.cal_conn_info = (void*)(new cal_connection_info());
	...
	// @bug1940 Do nothing for select query. Support of set default engine to ERYDB.
	if (thd->erydb_vtable.vtable_state == THD::ERYDB_CREATE_VTABLE ||
	...
	if (!table_arg)
	{
		setError(thd, ER_INTERNAL_ERROR, "ha_erydb_impl_create_: table_arg is NULL");
		return 1;
	}
	if (!table_arg->s)
	{
		setError(thd, ER_INTERNAL_ERROR, "ha_erydb_impl_create_: table_arg->s is NULL");
		return 1;
	}

	int rc = ha_erydb_impl_create_(name, table_arg, create_info, *ci);

	return rc;
}

int ha_erydb_impl_delete_table(const char *name)
{
	THD *thd = current_thd;
	char *dbName = NULL;
	...
	//if this is an EryDB tmp table ('#sql*.frm') just leave...
	...
	if (!thd) return 0;
	if (!thd->lex) return 0;
	if (!erydb_mysql_query_str(thd)) return 0;
	...
	//@Bug 1948,2306. if alter table want to drop the old table, EryDB does not need to drop.
	if ( ci->isAlter ){
		 ci->isAlter=false;
		 return 0;
	}
	// @bug 1793. make vtable droppable in erydbsys. "$vtable" ==> "@0024vtable" passed in as name.
	if (strcmp(dbName, "erydbsys") == 0 && string(name).find("@0024vtable") == string::npos)
	{
		std::string stmt(erydb_mysql_query_str(thd));
		...
		setError(thd, ER_INTERNAL_ERROR, "erydb system tables can only be dropped with restrict.");
		return 1;
	}

	int rc = ha_erydb_impl_delete_table_(dbName, name, *ci);
	return rc;
}
int ha_erydb_impl_write_row(uchar *buf, TABLE* table)
{
	THD *thd = current_thd;
	//sleep(100);
	...
	bool notExec = (
		...
		thd->erydb_vtable.isErydb);
#endif
	if ((thd->slave_thread || notExec)  ) return 0;
	...
	if ( (ci->useCpimport > 0) && (!(thd->variables.option_bits & (OPTION_NOT_AUTOCOMMIT | OPTION_BEGIN))) && (!ci->singleInsert) && ((ci->isLoaddataInfile) ||
	    ...
	    ((thd->lex)->sql_command == SQLCOM_INSERT_SELECT)) )
	{
		rc = ha_erydb_impl_write_batch_row_(buf, table, *ci);
	}
	else
	{
		if ( !ci->dmlProc )
		...
		rc = ha_erydb_impl_write_row_(buf, table, *ci, rowsInserted);
...
void ha_erydb_impl_start_bulk_insert(ha_rows rows, TABLE* table)
{
	THD *thd = current_thd;
	if (!thd->erydb_vtable.cal_conn_info)
		thd->erydb_vtable.cal_conn_info = (void*)(new cal_connection_info());
	...
	if (thd->erydb_vtable.vtable_state != THD::ERYDB_ALTER_VTABLE)
		thd->erydb_vtable.isEryDBDML = true;
	...
	bool notExec = (
		...
		thd->erydb_vtable.isErydb);
#endif
	if (thd->slave_thread || notExec) return;

	//@bug 5660. Error out DDL/DML on slave node, or on local query node
	if (ci->isSlaveNode && thd->erydb_vtable.vtable_state != THD::ERYDB_ALTER_VTABLE)
	{
		string emsg = logging::ERYDBErrorInfo::instance()->errorMsg(ERR_DML_DDL_SLAVE);
		setError(current_thd, ER_CHECK_NOT_IMPLEMENTED, emsg);
		return;
	...
	{
		setError(current_thd, ER_CHECK_NOT_IMPLEMENTED, "REPLACE statement is not supported in Erydb.");
	}
	...
	if ((thd->lex)->sql_command == SQLCOM_INSERT)
	{
		string insertStmt = erydb_mysql_query_str(thd);
		...
		if (found!=string::npos)
		{
			setError(current_thd, ER_CHECK_NOT_IMPLEMENTED, "IGNORE option in insert statement is not supported in Erydb.");
		}
	...
	{
    	ci->useCpimport = thd->variables.erydb_use_import_for_batchinsert;
...
//				setError(thd, ER_UNKNOWN_TABLE, ie.what());
			...
			for (unsigned int j = 0; j < colrids.size(); j++)
			{
				erydbSystemCatalog::ColType ctype = csc->colType(colrids[j].objnum);
				ci->columnTypes.push_back(ctype);
				if ((( ctype.colDataType == erydbSystemCatalog::VARCHAR ) || ( ctype.colDataType == erydbSystemCatalog::VARBINARY )) && !ci->useXbit )
					ci->useXbit = true;
				if (ctype.constraintType == erydbSystemCatalog::NOTNULL_CONSTRAINT)
					numberNotNull++;
			}
			...
			ostringstream oss;
			oss << erydb_mysql_query_str(thd) << "; |" << table->s->db.str<<"|";
			args1.add(oss.str());
			msg.format( args1 );
			...
			//get delimiter
			if (char(thd->variables.erydb_import_for_batchinsert_delimiter) != '\007')
				ci->delimiter = char(thd->variables.erydb_import_for_batchinsert_delimiter);
			else
				ci->delimiter = '\007';
			//get enclosed by
			if (char(thd->variables.erydb_import_for_batchinsert_enclosed_by) != 8)
				ci->enclosed_by = char(thd->variables.erydb_import_for_batchinsert_enclosed_by);
			else
				ci->enclosed_by = 8;
			...
			if (ci->enclosed_by == 34)	// Double quotes
				strcat(escapechar, "\\");
			if (thd->variables.erydb_local_query > 0  )
			{
				OamCache * oamcache = OamCache::makeOamCache();
				int localModuleId = oamcache->getLocalPMId();
				if (localModuleId == 0)
				{
					setError(current_thd, ER_INTERNAL_ERROR, logging::ERYDBErrorInfo::instance()->errorMsg(ERR_LOCAL_QUERY_UM));
					ci->singleInsert = true;
					LoggingID logid( 24, tid2sid(thd->thread_id), 0);
					logging::Message::Args args1;
					logging::Message msg(1);
...
//				siStartInfo.StartupInfo.hStdOutput = ci->cpimport_stdout_Wr;
	...
	if ( ( ((thd->lex)->sql_command == SQLCOM_INSERT) ||  ((thd->lex)->sql_command == SQLCOM_LOAD) || (thd->lex)->sql_command == SQLCOM_INSERT_SELECT) )
	{
		...
		ci->stats.fQuery = erydb_mysql_query_str(thd);
		try {
			ci->stats.userPriority(ci->stats.fHost, ci->stats.fUser);
		} catch (std::exception& e)
		{
		...
		if ((thd->lex)->sql_command == SQLCOM_INSERT)
			ci->stats.fQueryType = erydbSelectExecutionPlan::queryTypeToString(erydbSelectExecutionPlan::INSERT);
		else if ((thd->lex)->sql_command == SQLCOM_LOAD)
			ci->stats.fQueryType = erydbSelectExecutionPlan::queryTypeToString(erydbSelectExecutionPlan::LOAD_DATA_INFILE);
		...
		tableName.table = table->s->table_name.str;
		try {
			erydbSystemCatalog::ROPair roPair = csc->tableRID( tableName );
			ci->tableOid = roPair.objnum;
		}
		catch (ERYDBExcept &ie) {
			setError(thd, ER_INTERNAL_ERROR, ie.what());
...
int ha_erydb_impl_end_bulk_insert(bool abort, TABLE* table)
{
	THD *thd = current_thd;
	if (!thd->erydb_vtable.cal_conn_info)
		thd->erydb_vtable.cal_conn_info = (void*)(new cal_connection_info());
...
/*		if ( ci->isLoaddataInfile && ((thd->variables.option_bits & (OPTION_NOT_AUTOCOMMIT | OPTION_BEGIN)) || (ci->useCpimport == 0))) {
			...
			rc = ha_erydb_impl_write_last_batch(table, *ci, abort);
		}
	   ...
	   if ((ci->useCpimport > 0) && (!(thd->variables.option_bits & (OPTION_NOT_AUTOCOMMIT | OPTION_BEGIN))) && (!ci->singleInsert) && ((ci->isLoaddataInfile) ||  
	    ...
	    ((thd->lex)->sql_command == SQLCOM_INSERT_SELECT)) )
	    {
			...
			else
			{
				...
				{
					rc = 1;
					setError(thd, ER_INTERNAL_ERROR, "load failed. The detailed error information is listed in EryDBLog.txt.");
				}
				// Close handles to the cpimport process and its primary thread.
				CloseHandle(ci->cpimportProcInfo.hProcess);
				CloseHandle(ci->cpimportProcInfo.hThread);
...
int ha_erydb_impl_commit (handlerton *hton, THD *thd, bool all)
{
	if (thd->erydb_vtable.vtable_state == THD::ERYDB_CREATE_VTABLE ||
	    thd->erydb_vtable.vtable_state == THD::ERYDB_ALTER_VTABLE ||
	    thd->erydb_vtable.vtable_state == THD::ERYDB_DROP_VTABLE ||
	    thd->erydb_vtable.vtable_state == THD::ERYDB_REDO_PHASE1)
	    return 0;
	...
	int rc = ha_erydb_impl_commit_(hton, thd, all, *ci);
...
int ha_erydb_impl_rollback (handlerton *hton, THD *thd, bool all)
{
	...
	int rc = ha_erydb_impl_rollback_(hton, thd, all, *ci);
	ci->singleInsert = true; // reset the flag
	ci->isLoaddataInfile = false;
	ci->tableOid = 0;
	ci->rowsHaveInserted = 0;
...
int ha_erydb_impl_close_connection (handlerton *hton, THD *thd)
{
	if (!thd) return 0;
	...
	execplan::erydbSystemCatalog::removeerydbSystemCatalog(tid2sid(thd->thread_id));
	...
	if ( ci->dmlProc )
	{
		rc = ha_erydb_impl_close_connection_(hton, thd, *ci);
		delete ci->dmlProc;
		ci->dmlProc = NULL;
	}
...
int ha_erydb_impl_rename_table(const char* from, const char* to)
{
	IDEBUG( cout << "ha_erydb_impl_rename_table: " << from << " => " << to << endl );
	THD *thd = current_thd;
	...
	if ( ci->alterTableState == cal_connection_info::ALTER_FIRST_RENAME )
	{
		...
		IDEBUG( cout << "ha_erydb_impl_rename_table: was in state ALTER_FIRST_RENAME, now in ALTER_SECOND_RENAME" << endl );
		return 0;
	}
	else if (ci->alterTableState == cal_connection_info::ALTER_SECOND_RENAME)
	{
		ci->alterTableState = cal_connection_info::NOT_ALTER;
		IDEBUG( cout << "ha_erydb_impl_rename_table: was in state ALTER_SECOND_RENAME, now in NOT_ALTER" << endl );
		return 0;
	}
	else if ( thd->erydb_vtable.vtable_state == THD::ERYDB_ALTER_VTABLE )
		return 0;

	int rc = ha_erydb_impl_rename_table_(from, to, *ci);
	return rc;
}

int ha_erydb_impl_delete_row(const uchar *buf)
{
	IDEBUG( cout << "ha_erydb_impl_delete_row" << endl );
	return 0;
}

COND* ha_erydb_impl_cond_push(COND *cond, TABLE* table)
{
	THD *thd = current_thd;

	if (thd->slave_thread && thd->erydb_vtable.vtable_state == THD::ERYDB_INIT)
		thd->erydb_vtable.vtable_state = THD::ERYDB_DISABLE_VTABLE;
	...
	IDEBUG( cout << "ha_erydb_impl_cond_push: " << alias << endl );
...
int ha_erydb_impl_external_lock(THD *thd, TABLE* table, int lock_type)
{
	// @bug 3014. Error out locking table command. ERYDB does not support it now.
	if (thd->lex->sql_command == SQLCOM_LOCK_TABLES)
	{
		setError(current_thd, ER_CHECK_NOT_IMPLEMENTED,
				 logging::ERYDBErrorInfo::instance()->errorMsg(ERR_LOCK_TABLE));
		return ER_CHECK_NOT_IMPLEMENTED;
	}
	...
	erydbassert((thd->erydb_vtable.vtable_state >= THD::ERYDB_INIT_CONNECT &&
	           thd->erydb_vtable.vtable_state <= THD::ERYDB_REDO_QUERY) ||
	          thd->erydb_vtable.vtable_state == THD::ERYDB_ERROR);
	if ( thd->erydb_vtable.vtable_state == THD::ERYDB_INIT  )
		return 0;
...
#endif
	{
		...
		if (thd->erydb_vtable.vtable_state == THD::ERYDB_DISABLE_VTABLE)
		{
			...
			if (ci->tableMap.size() == 1 &&
				  thd->erydb_vtable.vtable_state == THD::ERYDB_DISABLE_VTABLE && thd->erydb_vtable.autoswitch)
			{
				push_warning(thd, Sql_condition::WARN_LEVEL_WARN, 9999, erydb_autoswitch_warning.c_str());
			}
		}
		else // vtable mode
		{
			if (thd->erydb_vtable.vtable_state == THD::ERYDB_SELECT_VTABLE)
			{
				...
				ci->queryState = 0;
				thd->erydb_vtable.override_largeside_estimate = false;
			}
		}
...
int ha_erydb_impl_rnd_pos(uchar *buf, uchar *pos)
{
	IDEBUG( cout << "ha_erydb_impl_rnd_pos" << endl);
	string emsg = logging::ERYDBErrorInfo::instance()->errorMsg(ERR_ORDERBY_TOO_BIG);
	setError(current_thd, ER_INTERNAL_ERROR, emsg);
	return ER_INTERNAL_ERROR;
}

// Called from mysql parser to set ERYDB error for window functions
void ha_erydb_impl_set_error(THD* thd, uint64_t errCode, LEX_STRING* args, uint32_t argCount)
{
	IDEBUG( cout << "ha_erydb_impl_ser_error" << endl);
	Message::Args arguments;
	for (uint32_t i = 0; i < argCount; i++)
		arguments.add(args[i].str);
	string emsg = logging::ERYDBErrorInfo::instance()->errorMsg(errCode, arguments);
	setError(thd, ER_INTERNAL_ERROR, emsg);
}
// vim:sw=4 ts=4:
