...
/*
 * $Id: ha_erydb_execplan.cpp 9749 2013-08-15 04:00:39Z zzhu $
 */
...
bool nonConstFunc(Item_func* ifp)
{
	if (strcasecmp(ifp->func_name(), "rand") == 0 ||
	    ...
	    strcasecmp(ifp->func_name(), "erydblocalpm") == 0)
		return true;
...
void buildNestedTableOuterJoin(gp_walk_info& gwi, TABLE_LIST* table_ptr)
{
	...
	while ((table= li++))
	{
		...
		if (table->outer_join)
		{
			erydbSystemCatalog::TableAliasName ta = make_aliasview(
			                       (table->db ? table->db : ""),
...
uint32_t buildOuterJoin(gp_walk_info& gwi, SELECT_LEX& select_lex)
{
	...
	for (; table_ptr; table_ptr= table_ptr->next_local)
	{
		...
		erydbSystemCatalog:: TableAliasName tan = make_aliasview(
		...
		else if (table_ptr->embedding && table_ptr->embedding->outer_join && table_ptr->embedding->on_expr)
		{
			...
			set<erydbSystemCatalog::TableAliasName>::const_iterator it;
		...
		else if (table_ptr->embedding && table_ptr->embedding->nested_join)
		{
			...
			if (table_ptr->embedding->is_natural_join && table_ptr->derived)
			{
				...
				{
					gwi.thd->erydb_vtable.isUnion = false;
					gwi.thd->erydb_vtable.isUpdateWithDerive = true;
					return -1;
				}
			}
			if (embeddingSet.find(table_ptr->embedding) != embeddingSet.end())
		...
		if (gwi_outer.hasSubSelect)
		{
			...
			gwi.parseErrorText = ERYDBErrorInfo::instance()->errorMsg(ERR_OUTER_JOIN_SUBSELECT);
...
bool buildRowColumnFilter(gp_walk_info* gwip, RowColumn* rhs, RowColumn* lhs, Item_func* ifp)
{
	...
	else
	{
		...
		gwip->parseErrorText = ERYDBErrorInfo::instance()->errorMsg(ERR_FUNC_MULTI_COL);
		return false;
	}
	return true;
}
...
bool buildPredicateItem(Item_func* ifp, gp_walk_info* gwip)
{
	...
	if (!(gwip->thd->erydb_vtable.cal_conn_info))
		gwip->thd->erydb_vtable.cal_conn_info = (void*)(new cal_connection_info());
	cal_connection_info* ci = reinterpret_cast<cal_connection_info*>(gwip->thd->erydb_vtable.cal_conn_info);


	if (ifp->functype() == Item_func::BETWEEN)
	{
		erydbassert (gwip->rcWorkStack.size() >= 3);
	...
	{
		erydbassert(gwip->rcWorkStack.size() >= 2);
		ReturnedColumn* rhs = gwip->rcWorkStack.top();
		gwip->rcWorkStack.pop();
		ReturnedColumn* lhs = gwip->rcWorkStack.top();
		gwip->rcWorkStack.pop();
		...
		// Use real table name.
		ifp->print(&str, QT_ERYDB_DERIVED);
			 ...
			 ifp->functype() == Item_func::ISNOTNULL_FUNC)
	{
		...
		{
			gwip->fatalParseError = true;
			gwip->parseErrorText = ERYDBErrorInfo::instance()->errorMsg(ERR_FILTER_COND_EXP);
			return false;
		}
	}
	...
	else if (ifp->functype() == Item_func::NOT_FUNC)
	{
		...
		erydbassert(ifp->argument_count() == 1);
		ParseTree *ptp = 0;
	...
	else if (ifp->functype() == Item_func::MULT_EQUAL_FUNC)
	{
		...
		erydbassert(item_eq->fields.elements == 2);
	...
	else //std rel ops (incl "like")
	{
		...
		{
			erydbassert(ifp->argument_count() == 2);
		...
		if (correlatedCol && localCol)
		{
			...
			if (sop->op() == OP_EQ)
			{
				if (gwip->subSelectType == erydbSelectExecutionPlan::IN_SUBS ||
					gwip->subSelectType == erydbSelectExecutionPlan::EXISTS_SUBS)
					correlatedCol->joinInfo(correlatedCol->joinInfo() | JOIN_SEMI);
				else if (gwip->subSelectType == erydbSelectExecutionPlan::NOT_IN_SUBS ||
					   gwip->subSelectType == erydbSelectExecutionPlan::NOT_EXISTS_SUBS)
		...
		else
		{
			for (uint32_t i = 0; i < ifp->argument_count(); i++)
			{
				...
				{
					gwip->fatalParseError = true;
					gwip->parseErrorText = ERYDBErrorInfo::instance()->errorMsg(ERR_SUB_EXPRESSION);
				}
			}
		}
		...
		if (sop->op() == OP_EQ)
		{
			erydbSystemCatalog::TableAliasName tan_lhs;
			erydbSystemCatalog::TableAliasName tan_rhs;
			set<erydbSystemCatalog::TableAliasName>::const_iterator it;
...
bool buildConstPredicate(Item_func* ifp, ReturnedColumn* rhs, gp_walk_info* gwip)
{
	...
	erydbSystemCatalog::ColType opType = rhs->resultType();
	if ( (opType.colDataType == erydbSystemCatalog::CHAR && opType.colWidth <= 8) ||
		  (opType.colDataType == erydbSystemCatalog::VARCHAR && opType.colWidth < 8) ||
		  (opType.colDataType == erydbSystemCatalog::VARBINARY && opType.colWidth < 8) )
	{
		opType.colDataType = execplan::erydbSystemCatalog::BIGINT;
		opType.colWidth = 8;
	}
...
SimpleColumn* buildSimpleColFromDerivedTable(gp_walk_info& gwi, Item_field* ifp)
{
	...
	for (int32_t i = gwi.tbList.size() - 1; i >=0; i--)
	{
		...
		{
			erydbSystemCatalog::TableName tn(gwi.tbList[i].schema, gwi.tbList[i].table);
			erydbSystemCatalog::RIDList oidlist = gwi.csc->columnRIDs(tn, true);
			for (unsigned int j = 0; j < oidlist.size(); j++)
			{
				erydbSystemCatalog::TableColName tcn = gwi.csc->colName(oidlist[j].objnum);
				erydbSystemCatalog::ColType ct = gwi.csc->colType(oidlist[j].objnum);
				if (strcasecmp(ifp->field_name, tcn.column.c_str()) == 0)
				{
					...
					/*if (sc)
					{
						...
						Message::Args args;
						args.add(ifp->name);
						gwi.parseErrorText = ERYDBErrorInfo::instance()->errorMsg(ERR_AMBIGUOUS_COL, args);
						return NULL;
					}*/
	...
	for (int32_t i = gwi.derivedTbList.size()-1; i>=0; i--)
	{
		...
		erydbSelectExecutionPlan* csep = dynamic_cast<erydbSelectExecutionPlan*>(gwi.derivedTbList[i].get());
		string derivedName = csep->derivedTbAlias();

		if (!ifp->table_name || strcasecmp(ifp->table_name, derivedName.c_str()) == 0)
		{
			erydbSelectExecutionPlan::ReturnedColumnList cols = csep->returnedCols();
	...
	if (!sc)
	{
		...
		name += ifp->name;
		args.add(name);
		gwi.parseErrorText = ERYDBErrorInfo::instance()->errorMsg(ERR_UNKNOWN_COL, args);
		return sc;
	}
	...
	if (ifp->depended_from)
	{
		...
		if (gwi.subSelectType == erydbSelectExecutionPlan::SINGLEROW_SUBS)
			sc->joinInfo(sc->joinInfo() | JOIN_SCALAR | JOIN_SEMI);
		if (gwi.subSelectType == erydbSelectExecutionPlan::SELECT_SUBS)
...
void collectAllCols(gp_walk_info& gwi, Item_field* ifp)
{
	...
	erydbSelectExecutionPlan::SelectList::const_iterator it = gwi.derivedTbList.begin();
	for (uint32_t i = 0; i < gwi.tbList.size(); i++)
	{
		...
		{
			erydbSelectExecutionPlan* csep = dynamic_cast<erydbSelectExecutionPlan*>((*it).get());
			++it;
...
void buildSubselectFunc(Item_func* ifp, gp_walk_info* gwip)
{
	...
	{
		gwip->fatalParseError = true;
		gwip->parseErrorText = ERYDBErrorInfo::instance()->errorMsg(ERR_NON_SUPPORT_FUNC_SUB);
		return;
	}
...
void setError(THD* thd, uint32_t errcode, string errmsg)
{
	...
	thd->erydb_vtable.mysql_optimizer_off = false;
...
SimpleColumn* getSmallestColumn(boost::shared_ptr<erydbSystemCatalog> csc,
								erydbSystemCatalog::TableName& tn,
								erydbSystemCatalog::TableAliasName& tan,
								TABLE* table,
								gp_walk_info& gwi)
{
	// derived table
	...
	if (!tan.fIsEryDB)
	{
		...
		SimpleColumn* sc = new SimpleColumn(table->s->db.str, table->s->table_name.str, field->field_name, tan.fIsEryDB, gwi.sessionid);
		string alias(table->alias.ptr());
		sc->tableAlias(lower(alias));
		sc->isEryDB(false);
		sc->resultType(fieldType_MysqlToERYDB(field));
		sc->oid(field->field_index+1);
		return sc;
	}

	erydbSystemCatalog::RIDList oidlist = csc->columnRIDs(tn, true);
	erydbSystemCatalog::TableColName tcn;
	int minColWidth = -1;
	int minWidthColOffset = 0;
	for (unsigned int j = 0; j < oidlist.size(); j++)
	{
			erydbSystemCatalog::ColType ct = csc->colType(oidlist[j].objnum);
...
erydbSystemCatalog::ColType fieldType_MysqlToERYDB (const Field* field)
{
	...
	switch (field->result_type())
	{
		...
		case REAL_RESULT:
			ct.colDataType = erydbSystemCatalog::DOUBLE;
			ct.colWidth = 8;
			break;
		default:
			IDEBUG( cout << "fieldType_MysqlToERYDB:: Unknown result type of MySQL "
...
erydbSystemCatalog::ColType colType_MysqlToERYDB (const Item* item)
{
	erydbSystemCatalog::ColType ct;
	ct.precision = 4;
	switch (item->result_type())
	{
		...
		case STRING_RESULT:
			...
			{
				if (ct.colWidth < 20)
					ct.colWidth = 20; // for erydb date length
			...
			if (item->type() == Item::FIELD_ITEM)
			{
                ...
                {
                    throw runtime_error ("BLOB/TEXT data types are not supported by EryDB.");
                }
			}
			break;
/* FIXME:
		case xxxBINARY_RESULT:
			ct.colDataType = erydbSystemCatalog::VARBINARY;
			ct.colWidth = item->max_length;
			// force token
			if (item->type() == Item::FUNC_ITEM)
			{
...
*/
		...
		case REAL_RESULT:
			...
			break;
		default:
			IDEBUG( cout << "colType_MysqlToERYDB:: Unknown result type of MySQL "
...
ReturnedColumn* buildReturnedColumn(Item* item, gp_walk_info& gwi, bool& nonSupport)
{
	...
	switch (item->type())
	{
		...
		case Item::FUNC_ITEM:
		{
			...
			{
				gwi.fatalParseError = true;
				gwi.parseErrorText = ERYDBErrorInfo::instance()->errorMsg(ERR_NON_SUPPORT_SELECT_SUB);
				  ...
				  (tmpVec.size() == 0))
			{
				...
				else if (ifp->result_type() == STRING_RESULT)
				{
					...
					rc->resultType(colType_MysqlToERYDB(item));
				}
...
ArithmeticColumn* buildArithmeticColumn(Item_func* item, gp_walk_info& gwi, bool& nonSupport)
{
	...
	//aop->resultType(colType_MysqlToERYDB(item));
	// @bug5715. Use EryDB adjusted coltype for result type.
    // decimal arithmetic operation gives double result when the session variable is set.
	//erydbassert(pt->left() && pt->right() && pt->left()->data() && pt->right()->data());
	erydbSystemCatalog::ColType mysql_type = colType_MysqlToERYDB(item);
	if (current_thd->variables.erydb_double_for_decimal_math == 1)
		aop->adjustResultType(mysql_type);
	else
		aop->resultType(mysql_type);

	// adjust decimal result type according to internalDecimalScale
	if (gwi.internalDecimalScale >= 0 && aop->resultType().colDataType == erydbSystemCatalog::DECIMAL)
	{
		erydbSystemCatalog::ColType ct = aop->resultType();
...
ReturnedColumn* buildFunctionColumn(Item_func* ifp, gp_walk_info& gwi, bool& nonSupport)
{
	...
	else if ((functor = funcExp->getFunctor(funcName)))
	{
		...
		if (gwi.clauseType == SELECT || /*gwi.clauseType == HAVING || */gwi.clauseType == GROUP_BY) // select clause
		{
			for (uint32_t i = 0; i < ifp->argument_count(); i++)
			{
				...
				if (ifp->arguments()[i]->has_subquery())
				{
					nonSupport = true;
					gwi.fatalParseError = true;
					gwi.parseErrorText = ERYDBErrorInfo::instance()->errorMsg(ERR_SUB_EXPRESSION);
					return NULL;
				}
		...
		if (funcName == "extract")
		{
			...
			switch (idai->int_type) {
				case INTERVAL_DAY_MICROSECOND:
				{
					...
					args.add(info);
					gwi.parseErrorText = ERYDBErrorInfo::instance()->errorMsg(ERR_NON_SUPPORTED_FUNCTION, args);
					return NULL;
				}
		...
		fc->resultType(colType_MysqlToERYDB(ifp));
	...
	if (!fc)
		return NULL;
	if (gwi.internalDecimalScale >= 0 && fc->resultType().colDataType == erydbSystemCatalog::DECIMAL)
	{
		erydbSystemCatalog::ColType ct = fc->resultType();
		ct.scale = gwi.internalDecimalScale;
		fc->resultType(ct);
	}
	if (ifp->name)
...
FunctionColumn* buildCaseFunction(Item_func* item, gp_walk_info& gwi, bool& nonSupport)
{
	...
	fc->resultType(colType_MysqlToERYDB(item));
	fc->operationType(functor->operationType(funcParms, fc->resultType()));
	fc->functionName(funcName);
	fc->functionParms(funcParms);
	fc->expressionId(ci->expressionId++);
...
{
	Item_decimal* idp = (Item_decimal*)item;
	ERYDB_Decimal erydb_decimal;
	String val, *str = item->val_str(&val);
	string valStr;
	valStr.assign(str->ptr(), str->length());
	ostringstream erydb_decimal_val;
	uint32_t i = 0;
	if (str->ptr()[0] == '+' || str->ptr()[0] == '-')
	{
		erydb_decimal_val << str->ptr()[0];
		i = 1;
	}
	for (; i < str->length(); i++)
	{
		if (str->ptr()[i] == '.')
			continue;
		erydb_decimal_val << str->ptr()[i];
	}
	erydb_decimal.value = strtoll(erydb_decimal_val.str().c_str(), 0, 10);

	if (gwi.internalDecimalScale >= 0 && idp->decimals > (uint)gwi.internalDecimalScale)
	{
		erydb_decimal.scale = gwi.internalDecimalScale;
		double val = (double)(erydb_decimal.value / pow((double)10, idp->decimals - gwi.internalDecimalScale));
		erydb_decimal.value = (int64_t)(val > 0 ? val + 0.5 : val - 0.5);
	}
	else
		erydb_decimal.scale = idp->decimals;
	erydb_decimal.precision = idp->max_length - idp->decimals;
	return new ConstantColumn(valStr, erydb_decimal);
}
...
SimpleColumn* buildSimpleColumn(Item_field* ifp, gp_walk_info& gwi)
{
	if (!gwi.csc)
	{
		gwi.csc = erydbSystemCatalog::makeerydbSystemCatalog(gwi.sessionid);
		gwi.csc->identity(erydbSystemCatalog::FE);
	}
...
//	Item** sfitempp = isp->arguments();
	...
	{
		gwi.fatalParseError = true;
		gwi.parseErrorText = ERYDBErrorInfo::instance()->errorMsg(ERR_MUL_ARG_AGG);
		return NULL;
	}
		...
		for (uint32_t i = 0; i < isp->argument_count(); i++)
		{
			...
			switch (sfitype)
			{
				case Item::FIELD_ITEM:
				{
					...
					parm.reset(sc);
					gwi.columnMap.insert(erydbSelectExecutionPlan::ColumnMap::value_type(string(ifp->field_name), parm));
					TABLE_LIST* tmp = (ifp->cached_table ? ifp->cached_table : 0);
					gwi.tableMap[make_aliastable(sc->schemaName(), sc->tableName(), sc->tableAlias(), sc->isEryDB())] = make_pair(1, tmp);
					break;
				}
				case Item::INT_ITEM:
				case Item::STRING_ITEM:
			...
			if (gwi.fatalParseError)
			{
				if (gwi.parseErrorText.empty())
				{
					...
					if (item->name)
						...
						args.add("");
					gwi.parseErrorText = ERYDBErrorInfo::instance()->errorMsg(ERR_NON_SUPPORT_AGG_ARGS, args);
				}
				return NULL;
			}
		}
	...
	if (parm)
	{
		...
		{
			erydbSystemCatalog::ColType ct = parm->resultType();
			switch (ct.colDataType)
			{
				case erydbSystemCatalog::TINYINT:
				case erydbSystemCatalog::SMALLINT:
	...
	{
		ac->resultType(colType_MysqlToERYDB(isp));
	}

	// adjust decimal result type according to internalDecimalScale
	if (gwi.internalDecimalScale >= 0 && ac->resultType().colDataType == erydbSystemCatalog::DECIMAL)
	{
		erydbSystemCatalog::ColType ct = ac->resultType();
		ct.scale = gwi.internalDecimalScale;
		ac->resultType(ct);
	}
	...
	switch (itype)
	{
		case Item::FIELD_ITEM:
		{
			...
			if (ifp)
			{
				...
				gwip->funcName.clear();
				gwip->columnMap.insert(erydbSelectExecutionPlan::ColumnMap::value_type(string(ifp->field_name), scsp));
				...
				if (!((scp->joinInfo() & JOIN_CORRELATED) || scp->colType().colDataType == erydbSystemCatalog::VARBINARY))
				{
					TABLE_LIST* tmp = (ifp->cached_table ? ifp->cached_table : 0);
					gwip->tableMap[make_aliastable(scp->schemaName(), scp->tableName(), scp->tableAlias(), scp->isEryDB())] =
					           make_pair(1, tmp);
				}
			}
			break;
		...
		case Item::FUNC_ITEM:
		{
				  ...
				  ifp->functype() != Item_func::MULT_EQUAL_FUNC)
			{
				...
				else
				{
					...
					cc->resultType(colType_MysqlToERYDB(item));
				}
...
//				if (isPredicateFunction(ifp, gwip))
			...
			if (gwip->fatalParseError)
			{
				...
				if (gwip->parseErrorText.empty())
				{
					Message::Args args;
					args.add(funcName);
					gwip->parseErrorText = ERYDBErrorInfo::instance()->errorMsg(ERR_NON_SUPPORTED_FUNCTION, args);
				}
				return;
			}
		...
		case Item::COND_ITEM:
		{
			...
			else
			{
				...
				//if (gwip->ptWorkStack.size() < icp->argument_list()->elements)
				{
					...
					while (Item *it= li++)
					{
						...
						if (isOr)
						{
							...
							{
								gwip->fatalParseError = true;
								gwip->parseErrorText = ERYDBErrorInfo::instance()->errorMsg(ERR_CORRELATED_SUB_OR);
								return;
							}
						}
				...
				// column. EryDB will remove it and set nullmatch flag if it's NOT_IN sub.
		...
		case Item::REF_ITEM:
		{
			...
			if (rc)
			{
				if (((Item_ref*)item)->depended_from)
				{
					...
					if (gwip->subSelectType == erydbSelectExecutionPlan::SINGLEROW_SUBS)
						rc->joinInfo(rc->joinInfo() | JOIN_SCALAR | JOIN_SEMI);
					if (gwip->subSelectType == erydbSelectExecutionPlan::SELECT_SUBS)
		...
		case Item::SUBSELECT_ITEM:
		{
			...
			if (sub->substype() == Item_subselect::EXISTS_SUBS)
			{
				...
				current_thd->erydb_vtable.isUnion = true; // only temp. bypass the 2nd phase.
				// recover original
				gwip->subQuery = orig;
...
//				parse_item(sfitempp[i], field_vec, hasNonSupportItem, parseInfo);
		...
		case Item::EXPR_CACHE_ITEM:
		{
			...
			string parseErrorText = ERYDBErrorInfo::instance()->errorMsg(ERR_NON_SUPPORT_SUB_QUERY_TYPE);
...
bool isEryDB(TABLE* table_ptr)
{
	...
	if (engineName == "EryDB" || engineName == "Erydb")
		return true;
	else
		return false;
}
...
int getSelectPlan(gp_walk_info& gwi, SELECT_LEX& select_lex, SCSEP& csep, bool isUnion)
{
	...
	{
		gwi.fatalParseError = true;
		gwi.parseErrorText = ERYDBErrorInfo::instance()->errorMsg(ERR_ROLLUP_NOT_SUPPORT);
		setError(gwi.thd, ER_CHECK_NOT_IMPLEMENTED, gwi.parseErrorText, gwi);
		return ER_CHECK_NOT_IMPLEMENTED;
	}

	gwi.internalDecimalScale = (gwi.thd->variables.erydb_use_decimal_scale ? gwi.thd->variables.erydb_decimal_scale : -1);
	...
	// @bug 2123. Override large table estimate if erydb_ordered hint was used.
	// @bug 2404. Always override if the erydb_ordered_only variable is turned on.
	if (gwi.thd->erydb_vtable.override_largeside_estimate || gwi.thd->variables.erydb_ordered_only)
		csep->overrideLargeSideEstimate(true);

	// @bug 5741. Set a flag when in Local PM only query mode
	csep->localQuery(gwi.thd->variables.erydb_local_query);

	// @bug 3321. Set max number of blocks in a dictionary file to be scanned for filtering
	csep->stringScanThreshold(gwi.thd->variables.erydb_string_scan_threshold);

	csep->stringTableThreshold(gwi.thd->variables.erydb_stringtable_threshold);

	csep->djsSmallSideLimit(gwi.thd->variables.erydb_diskjoin_smallsidelimit * 1024ULL * 1024);
	csep->djsLargeSideLimit(gwi.thd->variables.erydb_diskjoin_largesidelimit * 1024ULL * 1024);
	csep->djsPartitionSize(gwi.thd->variables.erydb_diskjoin_bucketsize * 1024ULL * 1024);
	if (gwi.thd->variables.erydb_um_mem_limit == 0)
		csep->umMemLimit(numeric_limits<int64_t>::max());
	else
		csep->umMemLimit(gwi.thd->variables.erydb_um_mem_limit * 1024ULL * 1024);
...
// DEBUG
	...
	try {
		for (; table_ptr; table_ptr= table_ptr->next_local)
		{
			...
			{
				String str;
				(table_ptr->derived->first_select())->print(gwi.thd, &str, QT_ERYDB_DERIVED);
				...
				erydbSystemCatalog::TableAliasName tan = make_aliastable("", alias, alias);
			...
			else if (table_ptr->view)
			{
				...
				erydbSystemCatalog::TableAliasName tn = make_aliastable(table_ptr->db, table_ptr->table_name, table_ptr->alias);
				view->viewName(tn);
			...
			else
			{
				...
				erydbSystemCatalog::TableAliasName tn = make_aliasview(table_ptr->db, table_name, table_ptr->alias, viewName, infiniDB);
				gwi.tbList.push_back(tn);
				erydbSystemCatalog::TableAliasName tan = make_aliastable(table_ptr->db, table_name, table_ptr->alias, infiniDB);
				gwi.tableMap[tan] = make_pair(0,table_ptr);
#ifdef DEBUG_WALK_COND
				cout << tn << endl;
#endif
	...
	{
		gwi.thd->erydb_vtable.isUnion = true;
		erydbSelectExecutionPlan::SelectList unionVec;
		...
		{
			SCSEP plan(new erydbSelectExecutionPlan());
...
/*#ifdef DEBUG_WALK_COND
		...
		if (unionVec.empty())
			gwi.thd->erydb_vtable.impossibleWhereOnUnion = true;
	}
...
#ifdef DEBUG_WALK_COND
{
	...
	erydbSelectExecutionPlan::SelectList selectSubList;
		...
		switch (itype)
		{
			case Item::FIELD_ITEM:
			{
				...
				if (sc)
				{
					...
					string fullname;
					String str;
					ifp->print(&str, QT_ERYDB_NO_QUOTE);
					fullname = str.c_ptr();
					...
					gwi.columnMap.insert(erydbSelectExecutionPlan::ColumnMap::value_type(string(ifp->field_name), spsc));
					TABLE_LIST* tmp = 0;
					if (ifp->cached_table)
			...
			case Item::FUNC_ITEM:
			{
				...
				{
					gwi.fatalParseError = true;
					gwi.parseErrorText = ERYDBErrorInfo::instance()->errorMsg(ERR_SP_FUNCTION_NOT_SUPPORT);
					setError(gwi.thd, ER_CHECK_NOT_IMPLEMENTED, gwi.parseErrorText, gwi);
					return ER_CHECK_NOT_IMPLEMENTED;
				}
				...
				{
					gwi.fatalParseError = true;
					gwi.parseErrorText = ERYDBErrorInfo::instance()->errorMsg(ERR_FILTER_COND_EXP);
					setError(gwi.thd, ER_CHECK_NOT_IMPLEMENTED, gwi.parseErrorText, gwi);
					return ER_CHECK_NOT_IMPLEMENTED;
				}
				...
				if (rc)
				{
					...
					{
						if (isUnion || unionSel || gwi.subSelectType != erydbSelectExecutionPlan::MAIN_SELECT ||
				...
				else // EryDB Non support functions still go through post process for now
				{
			...
			case Item::SUBSELECT_ITEM:
			{
				...
				String str;
				sub->get_select_lex()->print(gwi.thd, &str, QT_ERYDB_NO_QUOTE);
			...
			case Item::EXPR_CACHE_ITEM:
			{
				...
				gwi.fatalParseError = true;
				gwi.parseErrorText = ERYDBErrorInfo::instance()->errorMsg(ERR_UNKNOWN_COL);
				setError(gwi.thd, ER_CHECK_NOT_IMPLEMENTED, gwi.parseErrorText, gwi);
				return ER_CHECK_NOT_IMPLEMENTED;
			}
			case Item::WINDOW_FUNC_ITEM:
	...
	if (!csep->unionVec().empty())
	{
		for (uint32_t i = 0; i < gwi.returnedCols.size(); i++)
		{
			vector<erydbSystemCatalog::ColType> coltypes;
			for (uint32_t j = 0; j < csep->unionVec().size(); j++)
			{
				coltypes.push_back(
				   dynamic_cast<erydbSelectExecutionPlan*>(csep->unionVec()[j].get())->returnedCols()[i]->resultType());

				// @bug5976. set hasAggregate true for the main column if 
				// one corresponding union column has aggregate
				if (dynamic_cast<erydbSelectExecutionPlan*>(csep->unionVec()[j].get())->returnedCols()[i]->hasAggregate())
	...
	if (isUnion || gwi.subSelectType != erydbSelectExecutionPlan::MAIN_SELECT)
	{
	...
	for (uint32_t i = 0; i < funcFieldVec.size(); i++)
	{
		...
		String str;
		funcFieldVec[i]->print(&str, QT_ERYDB_NO_QUOTE);
		...
		{
			gwi.returnedCols.push_back(srcp);
			gwi.columnMap.insert(erydbSelectExecutionPlan::ColumnMap::value_type(string(funcFieldVec[i]->field_name), srcp));
			if (sel_cols_in_create.length() != 0)
				sel_cols_in_create += ", ";
			string fullname;
			fullname = str.c_ptr();
	...
	if (gwi.thd->erydb_vtable.vtable_state == THD::ERYDB_CREATE_VTABLE && !unionSel)
	{
		gwi.clauseType = GROUP_BY;
		Item* nonSupportItem = NULL;
		ORDER* groupcol = reinterpret_cast<ORDER*>(select_lex.group_list.first);

		// check if window functions are in order by. EryDB process order by list if
		...
		{
			gwi.fatalParseError = true;
			gwi.parseErrorText = ERYDBErrorInfo::instance()->errorMsg(ERR_WF_NOT_ALLOWED, "GROUP BY clause");
			setError(gwi.thd, ER_CHECK_NOT_IMPLEMENTED, gwi.parseErrorText, gwi);
			return ER_CHECK_NOT_IMPLEMENTED;
		}
		...
		for (; groupcol; groupcol= groupcol->next)
		{
			...
			else if (groupItem->type() == Item::FIELD_ITEM)
			{
				...
				gwi.groupByCols.push_back(srcp);
				gwi.columnMap.insert(erydbSelectExecutionPlan::ColumnMap::value_type(string(ifp->field_name), srcp));
			}
		...
		if (nonSupportItem)
		{
			...
			if (nonSupportItem->name)
				...
				args.add("");
			gwi.parseErrorText = ERYDBErrorInfo::instance()->errorMsg(ERR_NON_SUPPORT_GROUP_BY, args);
			setError(gwi.thd, ER_CHECK_NOT_IMPLEMENTED, gwi.parseErrorText, gwi);
			return ER_CHECK_NOT_IMPLEMENTED;
		}
	}

	if (gwi.thd->erydb_vtable.vtable_state == THD::ERYDB_CREATE_VTABLE)
	{
		SQL_LIST order_list = select_lex.order_list;
		ORDER* ordercol = reinterpret_cast<ORDER*>(order_list.first);
		string create_query(gwi.thd->erydb_vtable.create_vtable_query.c_ptr());
		string select_query(gwi.thd->erydb_vtable.select_vtable_query.c_ptr());
		string lower_create_query(gwi.thd->erydb_vtable.create_vtable_query.c_ptr());
		string lower_select_query(gwi.thd->erydb_vtable.select_vtable_query.c_ptr());
		...
		// for subquery, order+limit by will be supported in erydb. build order by columns
		// @todo union order by and limit support
		if (gwi.hasWindowFunc || gwi.subSelectType != erydbSelectExecutionPlan::MAIN_SELECT)
		{
			for (; ordercol; ordercol= ordercol->next)
			{
				ReturnedColumn* rc = NULL;
				...
				else
				{
					...
					if (!rc)
					{
						string emsg = ERYDBErrorInfo::instance()->errorMsg(ERR_NON_SUPPORT_ORDER_BY);
						gwi.parseErrorText = emsg;
								...
								for (uint32_t i = 0; i < fc->functionParms().size(); i++)
								{
									...
									for (uint32_t j = 0; j < gwi.returnedCols.size(); j++)
									{
										if (fc->functionParms()[i]->data()->operator==(gwi.returnedCols[j].get()))
										{
											...
											if (ac)
											{
												gwi.parseErrorText = ERYDBErrorInfo::instance()->errorMsg(ERR_NON_SUPPORT_ORDER_BY);
												setError(gwi.thd, ER_CHECK_NOT_IMPLEMENTED, gwi.parseErrorText, gwi);
												return ER_CHECK_NOT_IMPLEMENTED;
											}
											addToSel = true;
					...
					else if (ord_item->type() == Item::SUM_FUNC_ITEM)
					{
						...
						if (!ac)
						{
							setError(gwi.thd, ER_CHECK_NOT_IMPLEMENTED,
							         ERYDBErrorInfo::instance()->errorMsg(ERR_NON_SUPPORT_ORDER_BY), gwi);
							return ER_CHECK_NOT_IMPLEMENTED;
						}
						// check if this order by column is on the select list
						for (uint32_t i = 0; i < gwi.returnedCols.size(); i++)
						...
						if (ac || !gwi.groupByCols.empty())
						{
							if (addToSel)
							{
								...
								String str;
								ord_item->print(&str, QT_ERYDB_NO_QUOTE);
								if (sel_cols_in_create.length() != 0)
									sel_cols_in_create += ", ";
...
//							fullname += string(field->field_name);
					...
					if (addToSel)
					{
						...
						{
							gwi.parseErrorText = ERYDBErrorInfo::instance()->errorMsg(ERR_ORDERBY_NOT_IN_DISTINCT);
							setError(gwi.thd, ER_CHECK_NOT_IMPLEMENTED, gwi.parseErrorText, gwi);
							return ER_CHECK_NOT_IMPLEMENTED;
						}
						bool hasNonSupportItem = false;
			...
			for (uint32_t i = 0; i < fieldVec.size(); i++)
			{
				...
				String str;
				fieldVec[i]->print(&str, QT_ERYDB_NO_QUOTE);
				sc->alias(string(str.c_ptr()));
				SRCP srcp(sc);
				uint32_t j = 0;
				for (; j < gwi.returnedCols.size(); j++)
				...
				if (j == gwi.returnedCols.size())
				{
					...
					gwi.returnedCols.push_back(srcp);
					gwi.columnMap.insert(erydbSelectExecutionPlan::ColumnMap::value_type(string(fieldVec[i]->field_name), srcp));
		...
		try {
				for (; tb_iter != gwi.tableMap.end(); tb_iter++)
				{
					...
					erydbSystemCatalog::TableAliasName tan = (*tb_iter).first;
					erydbSystemCatalog::TableName tn = make_table((*tb_iter).first.schema, (*tb_iter).first.table);
		     ...
		     gwi.returnedCols.empty())
		{
			...
			erydbSelectExecutionPlan::ColumnMap::const_iterator iter;
			int minColWidth = 0;
			erydbSystemCatalog::ColType ct;
			try {
		...
		if (!isUnion && !gwi.hasWindowFunc && gwi.subSelectType == erydbSelectExecutionPlan::MAIN_SELECT)
		{
			std::ostringstream vtb;
		  vtb << "erydb_vtable.$vtable_" << gwi.thd->thread_id;
		  //vtb << "$vtable_" << gwi.thd->thread_id;
			...
			if (redo)
			{
				...
				gwi.thd->erydb_vtable.create_vtable_query.free();
				gwi.thd->erydb_vtable.create_vtable_query.append(create_query.c_str(), create_query.length());
				gwi.thd->erydb_vtable.vtable_state = THD::ERYDB_REDO_PHASE1; // redo phase 1
			...
			{
				gwi.thd->erydb_vtable.has_order_by = true;
		...
		if (unionSel || gwi.subSelectType != erydbSelectExecutionPlan::MAIN_SELECT)
		{
		...
		else if (select_lex.explicit_limit)
		{
			...
			if ((gwi.thd->variables.select_limit == (uint64_t)-1 ||
				  ...
				  gwi.thd->erydb_vtable.vtable_state != THD::ERYDB_CREATE_VTABLE))&&
				  !csep->hasOrderBy())
			{
		...
		gwi.thd->erydb_vtable.select_vtable_query.free();
		gwi.thd->erydb_vtable.select_vtable_query.append(select_query.c_str(), select_query.length());
		...
		{
			gwi.fatalParseError = true;
			gwi.parseErrorText = ERYDBErrorInfo::instance()->errorMsg(ERR_NON_SUPPORT_LIMIT_SUB);
			setError(gwi.thd, ER_INTERNAL_ERROR, gwi.parseErrorText, gwi);
			return ER_CHECK_NOT_IMPLEMENTED;
		}
	}
	...
	gwi.thd->erydb_vtable.duplicate_field_name = false;
	clearStacks(gwi);
	return 0;
}
...
int cp_get_table_plan(THD* thd, SCSEP& csep, cal_table_info& ti)
{
	...
	for (f_ptr=table->field ; (field= *f_ptr) ; f_ptr++)
	{
		if (bitmap_is_set(read_set, field->field_index))
		{
			...
			boost::shared_ptr<SimpleColumn> spsc(sc);
			gwi->returnedCols.push_back(spsc);
			gwi->columnMap.insert(erydbSelectExecutionPlan::ColumnMap::value_type(string(field->field_name), spsc));
		}
	}

	if (gwi->columnMap.empty())
	{
		erydbSystemCatalog::TableName tn = make_table(table->s->db.str, table->s->table_name.str);
		erydbSystemCatalog::TableAliasName tan = make_aliastable(table->s->db.str, table->s->table_name.str, table->alias.c_ptr());
	...
	csep->returnedCols(gwi->returnedCols);
	csep->columnMap(gwi->columnMap);
	erydbSelectExecutionPlan::TableList tblist;
