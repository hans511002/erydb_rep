...
// $Id: ha_erydb_impl_if.h 9701 2013-07-17 18:51:55Z zzhu $
/** @file */
#ifndef HA_ERYDB_IMPL_IF_H__
#define HA_ERYDB_IMPL_IF_H__
...
struct JoinInfo
{
	execplan::erydbSystemCatalog::TableAliasName tn;
	uint32_t joinTimes;
...
typedef std::map<execplan::erydbSystemCatalog::TableAliasName, std::pair<int, TABLE_LIST*> > TableMap;


struct gp_walk_info
{
	std::vector <std::string> selectCols;
	execplan::erydbSelectExecutionPlan::ReturnedColumnList returnedCols;
	execplan::erydbSelectExecutionPlan::ReturnedColumnList groupByCols;
	execplan::erydbSelectExecutionPlan::ReturnedColumnList subGroupByCols;
	execplan::erydbSelectExecutionPlan::ReturnedColumnList orderByCols;
	execplan::erydbSelectExecutionPlan::ColumnMap columnMap;
	...
	execplan::erydbSelectExecutionPlan::ReturnedColumnList additionalRetCols;
	...
	std::set<execplan::erydbSystemCatalog::TableAliasName> innerTables;
	...
	SubQuery* subQuery;
	execplan::erydbSelectExecutionPlan::SelectList derivedTbList;
	execplan::erydbSelectExecutionPlan::TableList tbList;
	std::vector<execplan::erydbSystemCatalog::TableAliasName> correlatedTbNameVec;
...
struct cal_connection_info
{
	...
	char delimiter;
	char enclosed_by;
	std::vector <execplan::erydbSystemCatalog::ColType> columnTypes; 
};

typedef std::tr1::unordered_map<int, cal_connection_info> CalConnMap;

const std::string erydb_err_msg = "\nThe query includes syntax that is not supported by erydb. Use 'show warnings;' to get more information. Review the erydb Syntax guide for additional information on supported distributed syntax or consider changing the erydb Operating Mode (erydb_vtable_mode).";
...
execplan::erydbSystemCatalog::ColType fieldType_MysqlToERYDB (const Field* field);
execplan::erydbSystemCatalog::ColType colType_MysqlToERYDB (const Item* item);
