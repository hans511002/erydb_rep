...
execplan::ParseTree* ScalarSub::transform()
{
	...
	//erydbassert(fGwip.rcWorkStack.size() >= 2);
...
{
	//erydbassert(fGwip.rcWorkStack.size() >= 3);
	if (fGwip.rcWorkStack.size() < 3)
	{
		fGwip.fatalParseError = true;
		fGwip.parseErrorText = ERYDBErrorInfo::instance()->errorMsg(ERR_NON_SUPPORT_SCALAR);
		return NULL;
	}
...
{
	erydbassert(fColumn.get() && fSub && fFunc);
	...
	if (rcol)
	{
		// ERYDB only supports (c1,c2..) =/!= (subquery)
		if (fFunc->functype() != Item_func::EQ_FUNC && fFunc->functype() != Item_func::NE_FUNC)
		{
			fGwip.fatalParseError = true;
			fGwip.parseErrorText = ERYDBErrorInfo::instance()->errorMsg(ERR_INVALID_OPERATOR_WITH_LIST);
			return NULL;
		}
		cols = rcol->columnVec();
	}
	...
	csep->location(erydbSelectExecutionPlan::WHERE);
	csep->subType (erydbSelectExecutionPlan::SINGLEROW_SUBS);
	...
	if (!gwi.correlatedTbNameVec.empty())
	{
		for (uint32_t i = 0; i < cols.size(); i++)
		{
			...
			if (sc)
			{
				erydbSystemCatalog::TableAliasName tan = make_aliastable(sc->schemaName(), sc->tableName(), sc->tableAlias());
				uint32_t j = 0;
