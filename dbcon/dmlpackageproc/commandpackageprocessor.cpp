...
CommandPackageProcessor::processPackage(dmlpackage::erydbDMLPackage& cpackage)
{
    ...
    try
    {
		...
		{
			execplan::erydbSystemCatalog::removeerydbSystemCatalog
	...
	catch ( logging::ERYDBExcept& noTable) //@Bug 2606 catch no table found exception
	{
...
void CommandPackageProcessor::viewTableLock(
	const dmlpackage::erydbDMLPackage& cpackage,
	DMLPackageProcessor::DMLResult& result)
{
	...
	boost::shared_ptr<erydbSystemCatalog> systemCatalogPtr =
		erydbSystemCatalog::makeerydbSystemCatalog(fSessionID);
	...
	for (unsigned idx=0; idx<tableLocks.size(); idx++)
	{
		if (roPair.objnum == (erydbSystemCatalog::OID)
	...
	DMLPackageProcessor::DMLResult& result)
{
	...
	// lock ID attribute to the erydbDMLPackage object.
