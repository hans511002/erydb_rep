...
UpdatePackageProcessor::processPackage(dmlpackage::erydbDMLPackage& cpackage)
{
...
//#endif
    try
    {
        ...
        if (0 != tablePtr)
        {
			...
			if ((result.result != 0) && (result.result != DMLPackageProcessor::ERYDBRANGE_WARNING))
  ...
  // timer.finish();
	...
	int rc = 0;
    if (result.result == NO_ERROR || result.result == ERYDBRANGE_WARNING)
    {
...
/*	if (result.result != ERYDBRANGE_WARNING)
...
uint64_t UpdatePackageProcessor::fixUpRows(dmlpackage::erydbDMLPackage& cpackage, DMLResult& result, 
		...
		while (1)
		{
			...
			try {
				...
				else 
                {
					*bsIn >> tmp8;
					*bsIn >> errorMsg;
					if (tmp8 == ERYDBRANGE_WARNING)
					{
						result.result = ERYDBRANGE_WARNING;	
						logging::Message::Args args;
						logging::Message message(2);
						args.add(errorMsg);
						message.format(args);
