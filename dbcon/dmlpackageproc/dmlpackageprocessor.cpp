...
#if 0
boost::any DMLPackageProcessor::tokenizeData( execplan::erydbSystemCatalog::SCN txnID,
	...
	std::string table, dmlpackage::ColumnList& colList)
{
	...
	erydbSystemCatalog::RIDList ridList = systemCatalogPtr->columnRIDs(tableName, true);
	...
	while (rid_iterator != ridList.end())
	{
		...
		erydbSystemCatalog::TableColName tblColName = systemCatalogPtr->colName( roPair.objnum );
	...
	unsigned startDBRoot, unsigned dbrootCnt, const unsigned startPartitionNum)
{
	...
	erydbassert(relRidInPartition <= (uint64_t)filesPerColumnPartition * (uint64_t)extentsPerSegmentFile *
		(uint64_t)extentRows);
...
{
	if (ec < 1000) // pre ERYDB error code
	{
		ErrorCodes ecObj;
	...
	// ERYDB error
	return ERYDBErrorInfo::instance()->errorMsg(ec);
}
...
int DMLPackageProcessor::rollBackTransaction(uint64_t uniqueId, BRM::TxnID txnID, uint32_t sessionID,std::string& errorMsg)
{
...
int DMLPackageProcessor::commitBatchAutoOnTransaction(uint64_t uniqueId, BRM::TxnID txnID, const uint32_t tableOid, std::string & errorMsg)
{
	...
	{
		errorMsg = ERYDBErrorInfo::instance()->errorMsg(ERR_HARD_FAILURE);
		stateChanged = false;
	}
