...
namespace dmlpackageprocessor
{
  DMLPackageProcessor::DMLResult DeletePackageProcessor::processPackage(dmlpackage::erydbDMLPackage& cpackage)
  {
	...
	//StopWatch timer;
    VERBOSE_INFO("DeletePackageProcessor is processing erydbDMLPackage ...");
    ...
    try
    {
      ...
      if ( 0 != tablePtr )
      {
		...
		const erydbSystemCatalog::RIDList ridList = csc->columnRIDs(aTableName);
  ...
  uint64_t DeletePackageProcessor::fixUpRows (dmlpackage::erydbDMLPackage& cpackage, DMLResult& result, const uint64_t uniqueId, const uint32_t tableOid)
  {
