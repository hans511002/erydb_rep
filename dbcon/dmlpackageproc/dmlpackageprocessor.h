...
public:
    ...
    enum  ResultCode
    {
        NO_ERROR, INSERT_ERROR,  NOTNULL_VIOLATION,CHECK_VIOLATION, DELETE_ERROR, UPDATE_ERROR, INDEX_UPDATE_ERROR,
        COMMAND_ERROR, TOKEN_ERROR, NOT_ACCEPTING_PACKAGES, NETWORK_ERROR, DEAD_LOCK_ERROR, REFERENCE_VIOLATION,
        ERYDBRANGE_WARNING, VB_OVERFLOW_ERROR, ACTIVE_TRANSACTION_ERROR, TABLE_LOCK_ERROR, JOB_ERROR, JOB_CANCELED
    };
     ...
     *
     * @param cpackage the erydbDMLPackage to process
     */
    virtual DMLResult processPackage(dmlpackage::erydbDMLPackage& cpackage) = 0;
     ...
     */
    //static boost::any convertColumnData( execplan::erydbSystemCatalog::ColType colType,
...
protected:
     ...
     */
    bool updateIndexes( uint32_t sessionID, execplan::erydbSystemCatalog::SCN txnID, const std::string& schema,
    ...
    bool violatesUniqueConstraint( const dmlpackage::RowList& rows,
        const execplan::erydbSystemCatalog::ConstraintInfo& constraintInfo, 
    ...
    inline bool isDictCol ( execplan::erydbSystemCatalog::ColType colType )
    {
            if (((colType.colDataType == execplan::erydbSystemCatalog::CHAR) && (colType.colWidth > 8)) 
       || ((colType.colDataType == execplan::erydbSystemCatalog::VARCHAR) && (colType.colWidth > 7)) 
       || ((colType.colDataType == execplan::erydbSystemCatalog::DECIMAL) && (colType.precision > 18))
               || (colType.colDataType == execplan::erydbSystemCatalog::VARBINARY)) 
             {
                    return true;
             }
             else
...
//    bool validateNextValue(execplan::erydbSystemCatalog::ColType colType, int64_t value, bool & offByOne);
