...
DropTableProcessor::DDLResult DropTableProcessor::processPackage(ddlpackage::DropTableStatement& dropTableStmt)
{
	...
	erydbSystemCatalog::RIDList tableColRidList;
	erydbSystemCatalog::DictOIDList dictOIDList;
...
//	int pmNum = 1;
	...
	{	
		//check table lock
		boost::shared_ptr<erydbSystemCatalog> systemCatalogPtr = erydbSystemCatalog::makeerydbSystemCatalog(dropTableStmt.fSessionID);
		...
		catch (ERYDBExcept &ie)
		{
			if (ie.errorCode() == ERR_TABLE_NOT_IN_CATALOG)
			{
				...
				args.add("Table does not exist in erydb engine.");
				args.add("");
				args.add("");
				message.format(args);
...
TruncTableProcessor::DDLResult TruncTableProcessor::processPackage(ddlpackage::TruncTableStatement& truncTableStmt)
{
	...
	std::vector <erydbSystemCatalog::OID> columnOidList;
	std::vector <erydbSystemCatalog::OID> allOidList;
	erydbSystemCatalog::RIDList tableColRidList;
	erydbSystemCatalog::DictOIDList dictOIDList;
	execplan::erydbSystemCatalog::ROPair roPair;
	std::string  processName("DDLProc");
	uint32_t  processID = ::getpid();;
	int32_t   txnid = txnID.id;
	boost::shared_ptr<erydbSystemCatalog> systemCatalogPtr = erydbSystemCatalog::makeerydbSystemCatalog(truncTableStmt.fSessionID);
	systemCatalogPtr->identity(erydbSystemCatalog::EC);
	systemCatalogPtr->sessionID(truncTableStmt.fSessionID);
	erydbSystemCatalog::TableInfo tableInfo;
	uint64_t uniqueId = 0;
	//Bug 5070. Added exception handling
	try {
		uniqueId = fDbrm->getUnique64();
