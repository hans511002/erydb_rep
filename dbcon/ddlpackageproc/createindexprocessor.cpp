...
CreateIndexProcessor::DDLResult CreateIndexProcessor::processPackage(ddlpackage::CreateIndexStatement& createIndexStmt)
{
    ...
    boost::shared_ptr<erydbSystemCatalog> systemCatalogPtr = erydbSystemCatalog::makeerydbSystemCatalog( createIndexStmt.fSessionID );
    try {
...
//    int oerydbase = fObjectIDManager.allocOIDs(2);
//    fIdxOID.listOID = oerydbase;
//    fIdxOID.treeOID = ++oerydbase;
...
try
{
    ...
    if (createIndexStmt.fUnique)
    {
    	...
    	erydbSystemCatalog::TableName sysConsTableName;
    	sysConsTableName.schema = ERYDB_SCHEMA;
...
//         		result.message = message;
	...
	erydbSystemCatalog::TableName sysConsColTableName;
    	sysConsColTableName.schema = ERYDB_SCHEMA;
...
/*       				logging::Message::Args args;
        			logging::Message message(9);
        			args.add("Error updating: ");
        			args.add("erydb.sysconstraintcol");
        			args.add("error number: ");
        			args.add( error );
        			message.format( args );
