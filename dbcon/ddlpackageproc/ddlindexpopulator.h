...
{
	typedef ColStructList IdxStructList;
	typedef ColValueList  IdxValueList;
}
...
public:
  ...
  DDLIndexPopulator(WriteEngine::WriteEngineWrapper* writeEngine, 
		    ...
		    uint32_t sessionID, 
		    execplan::erydbSystemCatalog::SCN txnID, 
 ...
 /*   fTOKENSIZE(sizeof(WriteEngine::Token) ) {}*/
     ...
     */
	bool isDictionaryType(const execplan::erydbSystemCatalog::ColType& ctype);
 ...
 protected:
     ...
     *
     * Fills in the values from the column result and erydb system catalog for
     * the WriteEngine::IdxStruct
     */
    execplan::erydbSystemCatalog::ColType makeIdxStruct(const execplan::ColumnResult* cr, size_t cols, boost::shared_ptr<execplan::erydbSystemCatalog> csc );
     ...
     */
    void addColumnData(const execplan::ColumnResult* cr, const execplan::erydbSystemCatalog::ColType ctype, int added);
 ...
 private: 
    ...
    /** @brief makes erydb Select Execution Plan
     *
     * builds csep to select data from all columns from fColNames
     */
    void makeCsep(execplan::erydbSelectExecutionPlan&  csep);
    ...
    {
  	return (type == execplan::erydbSystemCatalog::CHAR 
		|| type == execplan::erydbSystemCatalog::VARCHAR
		|| type == execplan::erydbSystemCatalog::FLOAT
		|| type == execplan::erydbSystemCatalog::DOUBLE
        || type == execplan::erydbSystemCatalog::UFLOAT
        || type == execplan::erydbSystemCatalog::UDOUBLE );
    }
     ...
     */
    void convertColData(const execplan::ColumnResult* cr, int idx,  const execplan::erydbSystemCatalog::ColType& cType, WriteEngine::IdxTuple& tuple);

    /** @brief converts non token data to its original type
     */
    boost::any  convertData(const execplan::erydbSystemCatalog::ColType&  colType, const execplan::ColumnResult* cr, int idx );
     ...
     */    
    boost::any tokenizeData( const execplan::erydbSystemCatalog::ColType& colType, const std::string& data );
     ...
     */
    bool checkConstraints(const WriteEngine::IdxTuple& data, const execplan::erydbSystemCatalog::ColType& ctype, int i, int column); 

    /** @brief returns if data not null
     *
     * Returns false if data is null and sets result to error
     */
    bool checkNotNull(const WriteEngine::IdxTuple& data, const execplan::erydbSystemCatalog::ColType& ctype);

    /** @brief returns if data is not unique
     *
     * Returns false if data row is found more than once in columns and sets result to error
     */
    bool checkUnique(int  i, const execplan::erydbSystemCatalog::ColType& colType );

    bool checkCheck( const WriteEngine::IdxTuple& data, const execplan::erydbSystemCatalog::ColType& ctype) const  { return true; }
    ...
    uint32_t 				fSessionID;
    execplan::erydbSystemCatalog::SCN fTxnID;
    ...
    {
      DDLNullValueForType(const execplan::erydbSystemCatalog::ColType& ctype)
	: DDLPackageProcessor(new BRM::DBRM()), fType(ctype) {}
      boost::any operator()(execplan::erydbSystemCatalog::ColType& ctype)
      {
	return getNullValueForType(fType);
      }
      const execplan::erydbSystemCatalog::ColType& fType;
    };
