...
bool typesAreSame(const erydbSystemCatalog::ColType& colType, const ColumnType& newType)
{
...
AlterTableProcessor::DDLResult AlterTableProcessor::processPackage(ddlpackage::AlterTableStatement& alterTableStmt)
{
	...
	{
		//check table lock
		boost::shared_ptr<erydbSystemCatalog> systemCatalogPtr = erydbSystemCatalog::makeerydbSystemCatalog(alterTableStmt.fSessionID);
		...
		execplan::erydbSystemCatalog::ROPair roPair;
		...
		{
			throw std::runtime_error(ERYDBErrorInfo::instance()->errorMsg(ERR_HARD_FAILURE));
		}
		...
		if ( tableLockId  == 0 )
		{
			...
			if (i >= numTries) //error out
			{
				...
				args.add(sessionId);
				throw std::runtime_error(ERYDBErrorInfo::instance()->errorMsg(ERR_TABLE_LOCKED,args));
			}			
		}
	...
	catch (std::exception&)
	{
		if (result.result == NO_ERROR)
		{
			...
			args.add(ERYDBErrorInfo::instance()->errorMsg(ERR_HARD_FAILURE));
			args.add("");
			args.add("");
			message.format(args);
			result.result = ALTER_ERROR;
...
void AlterTableProcessor::addColumn (uint32_t sessionID, execplan::erydbSystemCatalog::SCN txnID, DDLResult& result, 
		ddlpackage::ColumnDef* columnDefPtr, ddlpackage::QualifiedName& inTableName, const uint64_t uniqueId)
{
	...
	boost::shared_ptr<erydbSystemCatalog> systemCatalogPtr =
		erydbSystemCatalog::makeerydbSystemCatalog(sessionID);
	systemCatalogPtr->identity(erydbSystemCatalog::FE);
	...
	erydbSystemCatalog::OID columnOid;
	try {
	...
	if ((columnDefPtr->fType->fType == erydbSystemCatalog::CHAR && columnDefPtr->fType->fLength > 8) ||
				 (columnDefPtr->fType->fType == erydbSystemCatalog::VARCHAR && columnDefPtr->fType->fLength > 7) ||
				 (columnDefPtr->fType->fType == erydbSystemCatalog::VARBINARY && columnDefPtr->fType->fLength > 7))
	{
		isDict = true;
	}
	...
	if (inTableName.fSchema != ERYDB_SCHEMA)
	{
		try {
	...
	else
	{
		...
		{
			throw std::runtime_error("Error adding column to erydbsys table");
		}
		columnDefPtr->fType->fCompressiontype = 0;
		columnDefPtr->fType->fAutoincrement   = "n";
		columnDefPtr->fType->fNextvalue       = 0;
		cerr << "updating erydbsys...using static OID " << fStartingColOID << endl;
	}
	...
	try
	{
		...
		std::vector <erydbSystemCatalog::OID> oidList;
		if (isDict)
		{
		...
		while(column_iterator != columns.end()) {
			if (column_iterator->colType.colWidth == maxColwidth)
			{
				...
				if (dataType1 == erydbSystemCatalog::DECIMAL ||
                    dataType1 == erydbSystemCatalog::UDECIMAL)
				{
					columnDefPtr->convertDecimal();
				}
				
				erydbSystemCatalog::ColDataType dataType = convertDataType(columnDefPtr->fType->fType);
	...
	if (autoincrement)
	{
		...
		{
			aDbrm.releaseAILock(fStartingColOID);
			throw std::runtime_error(ERYDBErrorInfo::instance()->errorMsg(ERR_EXCEED_LIMIT));
		}
	}
	std::vector <erydbSystemCatalog::OID> oidList;
	oidList.push_back(fStartingColOID);
...
void AlterTableProcessor::dropColumn (uint32_t sessionID, execplan::erydbSystemCatalog::SCN txnID, DDLResult& result, 
...
void AlterTableProcessor::dropColumns (uint32_t sessionID, execplan::erydbSystemCatalog::SCN txnID, DDLResult& result, 
...
void AlterTableProcessor::addTableConstraint (uint32_t sessionID, execplan::erydbSystemCatalog::SCN txnID, DDLResult& result, ddlpackage::AtaAddTableConstraint& ataAddTableConstraint, ddlpackage::QualifiedName& fTableName)
{
...
void AlterTableProcessor::setColumnDefault (uint32_t sessionID, execplan::erydbSystemCatalog::SCN txnID, DDLResult& result, 
...
void AlterTableProcessor::dropColumnDefault (uint32_t sessionID, execplan::erydbSystemCatalog::SCN txnID, DDLResult& result, 
...
#if 0
void AlterTableProcessor::dropTableConstraint (uint32_t sessionID, execplan::erydbSystemCatalog::SCN txnID, DDLResult& result, ddlpackage::AtaDropTableConstraint& ataDropTableConstraint, ddlpackage::QualifiedName& fTableName)
{
	...
	try
	{
		execplan::erydbSystemCatalog::RID constrainRid = systemCatalogPtr->constraintRID(ataDropTableConstraint.fConstraintName);
		if (constrainRid == std::numeric_limits<erydbSystemCatalog::RID>::max())
		{
		...
		execplan::erydbSystemCatalog::RIDList ridlist = systemCatalogPtr->constraintColRID(ataDropTableConstraint.fConstraintName);
		if (ridlist.size() == 0)
		{
			// build the logging message
			err = "Alter Table failed: Constraint name not found";
		...
		execplan::erydbSystemCatalog::IndexName idxName;
		idxName.schema = fTableName.fSchema;
		idxName.table = fTableName.fName;
		idxName.index = ataDropTableConstraint.fConstraintName;
		execplan::erydbSystemCatalog::ROPair ropair = systemCatalogPtr->indexRID(idxName);
		if (ropair.rid >= 0)
		{
			sysCatalogTableName.fName  = SYSINDEX_TABLE;
			removeRowFromSysCatalog(sessionID, txnID, result, sysCatalogTableName, ropair.rid);
		}
		execplan::erydbSystemCatalog::RIDList ridList = systemCatalogPtr->indexColRIDs(idxName);

		if (ridList.size() > 0)
		{
			execplan::erydbSystemCatalog::RIDList::const_iterator riditer;
...
void AlterTableProcessor::renameTable (uint32_t sessionID, execplan::erydbSystemCatalog::SCN txnID, DDLResult& result, ddlpackage::AtaRenameTable& ataRenameTable, 
			ddlpackage::QualifiedName& fTableName, const uint64_t uniqueId)
{
	...
	*/
	...
	//@Bug 4599. Check whether the new table exists in erydb
	boost::shared_ptr<erydbSystemCatalog> systemCatalogPtr =
		erydbSystemCatalog::makeerydbSystemCatalog(sessionID);
...
void AlterTableProcessor::tableComment(uint32_t sessionID, execplan::erydbSystemCatalog::SCN txnID,
	...
	ddlpackage::QualifiedName& fTableName, const uint64_t uniqueId)
{
    ...
    if (boost::regex_search(start, end, what, compat, flags) && what[0].matched)
    {
        ...
        {
            throw std::runtime_error(ERYDBErrorInfo::instance()->errorMsg(ERR_INVALID_START_VALUE));
        }
        // Checks if zero and throws appropriate error (despite message name)
        // negative checks are below
        if (nextVal == 0)
        {
            throw std::runtime_error(ERYDBErrorInfo::instance()->errorMsg(ERR_NEGATIVE_STARTVALUE));
        }
    }
    else
    {
    ...
    tableName.schema = fTableName.fSchema;
    tableName.table = fTableName.fName;
    erydbSystemCatalog::TableInfo tblInfo = systemCatalogPtr->tableInfo(tableName);
    ...
    erydbSystemCatalog::ColType type = systemCatalogPtr->colType(oid);
...
void AlterTableProcessor::renameColumn(uint32_t sessionID, execplan::erydbSystemCatalog::SCN txnID,
	...
	ddlpackage::QualifiedName& fTableName, const uint64_t uniqueId)
{
	...
	*/
	...
	try
	{
		...
		erydbSystemCatalog::ColType colType = systemCatalogPtr->colType(ropair.objnum);
