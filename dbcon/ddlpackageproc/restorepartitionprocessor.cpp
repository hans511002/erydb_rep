...
namespace ddlpackageprocessor
{
	...
	RestorePartitionProcessor::DDLResult RestorePartitionProcessor::processPackage(ddlpackage::RestorePartitionStatement& restorePartitionStmt)
	{
		...
		{
			//check table lock
			boost::shared_ptr<erydbSystemCatalog> systemCatalogPtr = erydbSystemCatalog::makeerydbSystemCatalog(restorePartitionStmt.fSessionID);
			...
			{
				result.result = DROP_ERROR;
				result.message = ERYDBErrorInfo::instance()->errorMsg(ERR_HARD_FAILURE);
				fSessionManager.rolledback(txnID);
				return result;
			}
			...
			if ( uniqueID  == 0 )
			{
				...
				if (i >= numTries) //error out
				{
					...
					result.message = Message(ERYDBErrorInfo::instance()->errorMsg(ERR_TABLE_LOCKED,args));
					fSessionManager.rolledback(txnID);
					return result;
				}
			}
