...
void projectSimpleColumn(const SimpleColumn* sc, JobStepVector& jsv, JobInfo& jobInfo)
{
	...
	erydbSystemCatalog::OID oid = sc->oid();
	erydbSystemCatalog::OID tbl_oid = tableOid(sc, jobInfo.csc);
	string alias(extractTableAlias(sc));
	string view(sc->viewName());
	erydbSystemCatalog::OID dictOid = 0;
...
const JobStepVector doProject(const RetColsVector& retCols, JobInfo& jobInfo)
{
	...
	for (unsigned i = 0; i < retCols.size(); i++)
	{
		...
		else if ((wc = dynamic_cast<const WindowFunctionColumn*>(retCols[i].get())) != NULL)
		{
			...
			erydbSystemCatalog::ColType ct = wc->resultType();
...
void checkHavingClause(erydbSelectExecutionPlan* csep, JobInfo& jobInfo)
{
	...
	if (aggInHaving == false)
	{
		...
		{
			erydbassert(jobInfo.stack.size() == 1);
...
void checkReturnedColumns(erydbSelectExecutionPlan* csep, JobInfo& jobInfo)
{
...
*/
void checkGroupByCols(erydbSelectExecutionPlan* csep, JobInfo& jobInfo)
{
	// order by columns may be not in the select and [group by] clause
	const erydbSelectExecutionPlan::OrderByColumnList& orderByCols = csep->orderByCols();
...
void checkAggregation(erydbSelectExecutionPlan* csep, JobInfo& jobInfo)
{
...
void updateAggregateColType(AggregateColumn* ac, const SRCP& srcp, int op, JobInfo& jobInfo)
{
	...
	if (op == AggregateColumn::SUM || op == AggregateColumn::DISTINCT_SUM)
	{
        ...
        if (ct.colDataType == erydbSystemCatalog::UTINYINT ||
...
const JobStepVector doAggProject(const erydbSelectExecutionPlan* csep, JobInfo& jobInfo)
{
	...
	const erydbSelectExecutionPlan::GroupByColumnList& groupByCols = csep->groupByCols();
	...
	for (uint64_t i = 0; i < groupByCols.size(); i++)
	{
		...
		if (sc != NULL)
		{
			erydbSystemCatalog::OID gbOid = sc->oid();
			...
			if (!sc->schemaName().empty())
			{
				ct = sc->colType();
//XXX use this before connector sets colType in sc correctly.
				if (sc->isEryDB() && dynamic_cast<const PseudoColumn*>(sc) == NULL)
					ct = jobInfo.csc->colType(sc->oid());
//X
				dictOid = isDictCol(ct);
			}
	...
	map<uint32_t, erydbSystemCatalog::OID> dictMap; // bug 1853, the tupleKey - dictoid map
	for (uint64_t i = 0; i < retCols.size(); i++)
	{
		srcp = retCols[i];
		const SimpleColumn* sc = dynamic_cast<const SimpleColumn*>(srcp.get());
		...
		int op = 0;
		erydbSystemCatalog::OID dictOid = 0;
		erydbSystemCatalog::ColType ct, aggCt;
		...
		if (sc == NULL)
		{
			...
			if (ac != NULL)
			{
				...
				if (sc != NULL && sc->resultType().colDataType == erydbSystemCatalog::VARBINARY)
		...
		if (sc != NULL)
		{
			...
			erydbSystemCatalog::OID retOid = sc->oid();
			erydbSystemCatalog::OID tblOid = tableOid(sc, jobInfo.csc);
...
void changePcolStepToPcolScan(JobStepVector::iterator& it, JobStepVector::iterator& end)
{
	...
	erydbassert(dynamic_cast<PseudoColStep*>(it->get()) == NULL);
...
void parseExecutionPlan(erydbSelectExecutionPlan* csep, JobInfo& jobInfo,
	...
	if (!querySteps.empty())
	{
		...
		for (; iter != end; ++iter)
		{
			...
			if (typeid(*(iter->get())) == typeid(pColStep))
			{
				...
				if (pcol->colType().colDataType == erydbSystemCatalog::VARBINARY)
				{
...
//			}
	...
	const erydbSelectExecutionPlan::ColumnMap& retCols = csep->columnMap();
	erydbSelectExecutionPlan::ColumnMap::const_iterator i = retCols.begin();
	for (; i != retCols.end(); i++)
	{
...
// v-table mode
void makeVtableModeSteps(erydbSelectExecutionPlan* csep, JobInfo& jobInfo,
	JobStepVector& querySteps, JobStepVector& projectSteps, DeliveredTableMap& deliverySteps)
{
...
//	SJSTEP ds = deliverySteps.begin()->second;
	erydbassert(deliverySteps.begin()->second.get());
//	ds->stepId(stepNo);
...
void makeJobSteps(erydbSelectExecutionPlan* csep, JobInfo& jobInfo,
	JobStepVector& querySteps, JobStepVector& projectSteps, DeliveredTableMap& deliverySteps)
{
	// v-table mode, switch to tuple methods and return the tuple joblist.
	//@Bug 1958 Build table list only for tryTuples.
	const erydbSelectExecutionPlan::SelectList& fromSubquery = csep->derivedTableList();
	int i = 0;
	for (erydbSelectExecutionPlan::TableList::const_iterator it = csep->tableList().begin();
...
void makeUnionJobSteps(erydbSelectExecutionPlan* csep, JobInfo& jobInfo,
	JobStepVector& querySteps, JobStepVector&, DeliveredTableMap& deliverySteps)
{
	erydbSelectExecutionPlan::SelectList& selectVec = csep->unionVec();
	uint8_t distinctUnionNum = csep->distinctUnionNum();
	RetColsVector unionRetCols = csep->returnedCols();
	JobStepVector unionFeeders;
	for (erydbSelectExecutionPlan::SelectList::iterator cit = selectVec.begin();
		 cit != selectVec.end();
		 cit++)
	{
		// @bug4848, enhance and unify limit handling.
...
SJLP makeJobList_(
	erydbExecutionPlan* cplan,
	ResourceManager& rm,
	bool isExeMgr,
	unsigned& errCode, string& emsg)
{
	erydbSelectExecutionPlan* csep = dynamic_cast<erydbSelectExecutionPlan*>(cplan);
	boost::shared_ptr<erydbSystemCatalog> csc = erydbSystemCatalog::makeerydbSystemCatalog(csep->sessionID());
...
//	jobInfo.tryTuples = tryTuples; // always tuples after release 3.0
	...
	*jobInfo.smallSideUsage = 0;
	...
	// set fifoSize to 1 for erydbSystemCatalog query
