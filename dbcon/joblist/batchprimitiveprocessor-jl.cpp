...
void BatchPrimitiveProcessorJL::addFilterStep(const pColScanStep &scan, vector<BRM::LBID_t> lastScannedLBID)
{
	...
	filterCount++;
	_hasScan = true;
	erydbassert(sessionID == scan.sessionId());
}
...
void BatchPrimitiveProcessorJL::addFilterStep(const PseudoColStep &pcs)
{
	...
	filterSteps.push_back(cc);
	filterCount++;
	erydbassert(sessionID == pcs.sessionId());
}
...
void BatchPrimitiveProcessorJL::addFilterStep(const pColStep &step)
{
	...
	filterSteps.push_back(cc);
	filterCount++;
	erydbassert(sessionID == step.sessionId());
}
	...
	const pDictionaryStep &dict)
{
	...
	projectCount++;
	needStrValues = true;
	erydbassert(sessionID == col.sessionId());
	erydbassert(sessionID == dict.sessionId());
}
	...
	const pDictionaryStep &dict)
{
	...
	erydbassert(sessionID == p.sessionId());
	erydbassert(sessionID == dict.sessionId());
}
...
{
	erydbassert(sessionID == ds.sessionId());
	...
	{
		oid = static_cast<erydbSystemCatalog::OID>(projectSteps[i]->getOID());
		if (oid > 0)
		{
...
// 	cout << "get Element Types uniqueID=" << uniqueID << endl;
	/* skip the header */
	erydbassert(in.length() > sizeof(ISMPacketHeader) + sizeof(PrimitiveHeader));
...
// 	rowsProcessed += l_count;
	...
	erydbassert(in.length() > (uint32_t) ((l_count << 1) + (l_count << 3)) );
...
// 			erydbassert((*out)[i].first > 1023);
	...
	if (joiner.get() != NULL) {
		in >> *preJoinRidCount;
		in >> jCount;
		erydbassert(in.length() > (jCount << 4));
...
// 		cout << "column " << i << " is " << colLengths[i] << " long at " << pos << endl;
	...
	for (i = 0; i < l_rowCount; i++) {
		...
		for (j = 0, pos = 0; j < projectCount; j++) {
			erydbassert(pos + colWidths[j] <= tupleLength);
...
//	ism.Interleave = pmNum - 1;
	...
	if (_hasScan)
		erydbassert(ridCount == 0);
	else if (!sendRowGroups)
		erydbassert(ridCount > 0 && (ridMap != 0 || sendAbsRids));
	else
		erydbassert(inputRG.getRowCount() > 0);
