...
//					cout << "Adding project step PassThruStep and pDictionaryStep to BPS" << endl;
	...
	else
	{
		...
		else
		{
			...
			if (passthru!= 0)
			{
				erydbassert(!fBPP->getFilterSteps().empty());
				if (static_cast<erydbSystemCatalog::OID>(fBPP->getFilterSteps().back()->getOID()) != passthru->oid())
				{
					SJSTEP pts;
...
void TupleBPS::storeCasualPartitionInfo(const bool estimateRowCounts)
{
	...
	const bool ignoreCP = ((fTraceFlags & erydbSelectExecutionPlan::IGNORE_CP) != 0);
...
bool TupleBPS::goodExtentCount()
{
	...
	map<erydbSystemCatalog::OID, tr1::unordered_map<int64_t, EMEntry> >
		::iterator it;
...
void TupleBPS::sendPrimitiveMessages()
{
	...
	catch(const ERYDBExcept &e) {
		//cout << "Caught ERYDBExcept" << e.what() << e.errorCode() << endl;
...
// 		cout << "thread " << threadID << " has " << size << " Bytestreams\n";
		for (i = 0; i < size && !cancelled(); i++) {
			...
			if (bs->length() == 0 || hdr->Status > 0)
			{
				...
				if (bs->length() == 0)
				{
					errorMessage(ERYDBErrorInfo::instance()->errorMsg(ERR_PRIMPROC_DOWN));
						...
						for (j = 0; j < smallSideCount; j++) {
							...
							*/
							...
							if (tjoiners[j]->scalar() && matchCount > 1) {
								errorMessage(ERYDBErrorInfo::instance()->errorMsg(ERR_MORE_THAN_1_ROW));
