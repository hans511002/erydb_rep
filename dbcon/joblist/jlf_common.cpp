...
uint32_t uniqTupleKey(JobInfo& jobInfo,
                      erydbSystemCatalog::OID& o,
                      erydbSystemCatalog::OID& t,
                      const string& cn,
                      const string& ca,
                      const string& tn,
                      const string& ta,
...
uint32_t fudgeWidth(const erydbSystemCatalog::ColType& ict, erydbSystemCatalog::OID oid)
{
	erydbSystemCatalog::OID dictOid = isDictCol(ict);
	erydbSystemCatalog::ColType ct = ict;
	if (ct.colDataType != erydbSystemCatalog::VARBINARY)
	{
		if (ct.colDataType == erydbSystemCatalog::VARCHAR)
			ct.colWidth++;
...
TupleInfo setTupleInfo_(const erydbSystemCatalog::ColType& ct,
                        erydbSystemCatalog::OID col_oid,
                        JobInfo& jobInfo,
                        erydbSystemCatalog::OID tbl_oid,
...
//------------------------------------------------------------------------------
erydbSystemCatalog::OID isDictCol(const erydbSystemCatalog::ColType& colType)
{
	if (colType.colWidth > 8) return colType.ddn.dictOID;
	if (colType.colDataType == erydbSystemCatalog::VARCHAR &&
		colType.colWidth > 7) return colType.ddn.dictOID;
	if (colType.colDataType == erydbSystemCatalog::VARBINARY)
...
//------------------------------------------------------------------------------
bool isCharCol(const erydbSystemCatalog::ColType& colType)
{
...
uint32_t getTableKey(const JobInfo& jobInfo, execplan::erydbSystemCatalog::OID tableOid,
...
{
	erydbSystemCatalog::OID tableOid = js->tableOid();
...
{
	erydbSystemCatalog::OID o = tableOid(sc, jobInfo.csc);
	return uniqTupleKey(jobInfo, o, o, "", "", sc->tableName(), extractTableAlias(sc),
	                    sc->schemaName(), sc->viewName(), 0, (sc->isEryDB() ? 0 : 1),
	                    ((sc->joinInfo() & execplan::JOIN_CORRELATED) != 0));
}
...
TupleInfo getTupleInfo(uint32_t columnKey, const JobInfo& jobInfo)
{
	...
	if ((cit == jobInfo.keyInfo->tupleInfoMap.end()) ||
	    (cit->second.dtype == erydbSystemCatalog::BIT))
	{
		ostringstream strstm;
...
TupleInfo setTupleInfo(const execplan::erydbSystemCatalog::ColType& ct,
	execplan::erydbSystemCatalog::OID col_oid,
	JobInfo& jobInfo,
	execplan::erydbSystemCatalog::OID tbl_oid,
	const execplan::SimpleColumn* sc,
	const string& alias)
{
	const PseudoColumn* pc = dynamic_cast<const execplan::PseudoColumn*>(sc);
...
TupleInfo setExpTupleInfo(const execplan::erydbSystemCatalog::ColType& ct, uint64_t expressionId,
...
void updateDerivedColumn(JobInfo& jobInfo, SimpleColumn* sc, erydbSystemCatalog::ColType& ct)
{
	sc->oid(tableOid(sc, jobInfo.csc) + 1 + sc->colPosition());

	map<UniqId, execplan::erydbSystemCatalog::ColType>::iterator i =
...
// string to string
bool compatibleColumnTypes(const erydbSystemCatalog::ColType& ct1,
                           const erydbSystemCatalog::ColType& ct2,
                           bool  forJoin)
{
...
bool compatibleColumnTypes(const erydbSystemCatalog::ColDataType& dt1, uint32_t scale1,
                           const erydbSystemCatalog::ColDataType& dt2, uint32_t scale2,
                           bool  forJoin)
{
	// disable VARBINARY used in join
	if (dt1 == erydbSystemCatalog::VARBINARY ||
		dt2 == erydbSystemCatalog::VARBINARY)
		throw runtime_error("Comparsion between VARBINARY columns is not supported.");

	switch (dt1)
	{
	case erydbSystemCatalog::BIT:
		if (dt2 != erydbSystemCatalog::BIT) return false;
		break;
    ...
    case erydbSystemCatalog::UDECIMAL:
		if (dt2 != erydbSystemCatalog::TINYINT &&
			dt2 != erydbSystemCatalog::SMALLINT &&
			dt2 != erydbSystemCatalog::MEDINT &&
			dt2 != erydbSystemCatalog::INT &&
			dt2 != erydbSystemCatalog::BIGINT &&
			dt2 != erydbSystemCatalog::DECIMAL &&
            dt2 != erydbSystemCatalog::UTINYINT &&
            dt2 != erydbSystemCatalog::USMALLINT &&
            dt2 != erydbSystemCatalog::UMEDINT &&
            dt2 != erydbSystemCatalog::UINT &&
            dt2 != erydbSystemCatalog::UBIGINT &&
            dt2 != erydbSystemCatalog::UDECIMAL) return false;
			if (scale2 != scale1) return false;
		break;
	case erydbSystemCatalog::DATE:
		if (dt2 != erydbSystemCatalog::DATE) return false;
		break;
	case erydbSystemCatalog::DATETIME:
		if (dt2 != erydbSystemCatalog::DATETIME) return false;
		break;
	case erydbSystemCatalog::CHAR:
	case erydbSystemCatalog::VARCHAR:
		// @bug 1495 compound/string join
		if (dt2 != erydbSystemCatalog::VARCHAR &&
			dt2 != erydbSystemCatalog::CHAR)
			return false;
		break;
	case erydbSystemCatalog::VARBINARY:
		if (dt2 != erydbSystemCatalog::VARBINARY) return false;
		break;
	case erydbSystemCatalog::FLOAT:
	case erydbSystemCatalog::UFLOAT:
		if (forJoin && (dt2 != erydbSystemCatalog::FLOAT &&
		                dt2 != erydbSystemCatalog::FLOAT)) return false;
		else if (dt2 != erydbSystemCatalog::FLOAT &&
				 dt2 != erydbSystemCatalog::DOUBLE &&
				 dt2 != erydbSystemCatalog::UFLOAT &&
		         dt2 != erydbSystemCatalog::UDOUBLE) return false;
		break;
	case erydbSystemCatalog::DOUBLE:
	case erydbSystemCatalog::UDOUBLE:
		if (forJoin && (dt2 != erydbSystemCatalog::DOUBLE &&
		                dt2 != erydbSystemCatalog::UDOUBLE)) return false;
		else if (dt2 != erydbSystemCatalog::FLOAT &&
				 dt2 != erydbSystemCatalog::DOUBLE &&
				 dt2 != erydbSystemCatalog::UFLOAT &&
