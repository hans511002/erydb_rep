...
int JobList::putEngineComm(DistributedEngineComm* dec)
{
	...
	if (fPmsConfigured == 0)
	{
		...
		errInfo->errMsg  = logging::ERYDBErrorInfo::instance()->errorMsg(logging::ERR_NO_PRIMPROC);
...
// -- TupleJobList
/* returns row count */
uint32_t TupleJobList::projectTable(erydbSystemCatalog::OID, messageqcpp::ByteStream& bs)
{
...
//			TupleDataList* tdl;
		...
		{
			BatchPrimitive* bps = dynamic_cast<BatchPrimitive*>(qsi->get());
			erydbSystemCatalog::OID tableOIDProject = bps->tableOid();
			if (bps->getOutputType() == TABLE_BAND || bps->getOutputType() == ROW_GROUP)
			{
				...
				for (dsi = fDeliveredTables.begin(); dsi != fDeliveredTables.end(); dsi++)
				{
					...
					if (bpsDelivery)
					{
						erydbSystemCatalog::OID tableOID = bpsDelivery->tableOid();
					...
					{
						erydbSystemCatalog::OID tableOID = thjDelivery->tableOid();
	...
	for (psi = fProject.begin(), ctn = 0; psi != fProject.end(); ctn++, psi++)
	{
		...
		erydbSystemCatalog::OID tableOIDProject = 0;
		if (bps)
		...
		for (dsi = fDeliveredTables.begin(); dsi != fDeliveredTables.end(); dsi++)
		{
			...
			if (dbps)
			{
				...
				erydbSystemCatalog::OID tableOID = dbps->tableOid();
...
//	DeliveredTableMap::const_iterator it;
	...
	erydbassert(fQuery.size() > 0);
//	for (i = 0; i < fQuery.size(); i++)
//		erydbassert(dynamic_cast<BatchPrimitiveStep *>(fQuery[i].get()) ||
...
//	erydbassert(fDeliveredTables.size() > 0);
//	for (i = 0; i < fProject.size(); i++)
//		erydbassert(dynamic_cast<BatchPrimitiveStep *>(fProject[i].get()));
//
//	/* Check that all JobSteps use the right status pointer */
//	for (i = 0; i < fQuery.size(); i++) {
//		erydbassert(fQuery[i]->errorInfo().get() == errorInfo().get());
//	}
//	for (i = 0; i < fProject.size(); i++) {
//		erydbassert(fProject[i]->errorInfo().get() == errorInfo().get());
//	}
//	for (it = fDeliveredTables.begin(); it != fDeliveredTables.end(); ++it) {
//		erydbassert(it->second->errorInfo().get() == errorInfo().get());
//	}
}
...
void TupleJobList::validate() const
{
	...
	erydbassert(fQuery.size() > 0);
	for (i = 0; i < fQuery.size(); i++) {
		erydbassert(dynamic_cast<TupleBPS *>(fQuery[i].get()) ||
	...
	for (i = 0; i < fQuery.size(); i++)
		for (j = i + 1; j < fQuery.size(); j++)
			erydbassert(fQuery[i].get() != fQuery[j].get());

	/****  XXXPAT: An indication of a possible problem: The next assertion fails
		occasionally */
// 	erydbassert(fDeliveredTables.begin()->first == 100);  //fake oid for the vtable
// 	cout << "Delivered TableOID is " << fDeliveredTables.begin()->first << endl;
	erydbassert(fProject.size() == 0);
	erydbassert(fDeliveredTables.size() == 1);
	erydbassert(dynamic_cast<TupleDeliveryStep *>(fDeliveredTables.begin()->second.get()));
