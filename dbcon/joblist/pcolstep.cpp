...
pColStep::pColStep(
	...
	const erydbSystemCatalog::ColType& ct,
	...
	fMsgBytesOut(0)
{
	...
	if ( fColType.colDataType == erydbSystemCatalog::VARCHAR )
	{
		if (8 > fColType.colWidth && 4 <= fColType.colWidth )
			fColType.colDataType = erydbSystemCatalog::CHAR;

		fColType.colWidth++;
	}

	//If this is a dictionary column, fudge the numbers...
	if (fColType.colDataType == erydbSystemCatalog::VARBINARY)
	{
		fColType.colWidth = 8;
		fIsDict = true;
	}
	else if (fColType.colWidth > 8 )
	{
		...
		fIsDict = true;
		//TODO: is this right?
		fColType.colDataType = erydbSystemCatalog::VARCHAR;
	}
	...
	if (fColType.colWidth == 3)
		...
		fColType.colWidth = 8;
	erydbassert(fColType.colWidth > 0);
...
void pColStep::addFilter(int8_t COP, int64_t value, uint8_t roundFlag)
{
	...
	switch(fColType.colWidth) {
		...
		default:
			...
			o << "pColStep: erydbSystemCatalog says OID " << fOid <<
