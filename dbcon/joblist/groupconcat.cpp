...
void GroupConcatAgUM::applyMapping(const boost::shared_array<int>& mapping, const Row& row)
{
	...
	for (uint64_t i = 0; i < fRow.getColumnCount(); i++)
	{
		if (fRow.getColumnWidth(i) > 8 &&
			(fRow.getColTypes()[i] == execplan::erydbSystemCatalog::CHAR ||
			 fRow.getColTypes()[i] == execplan::erydbSystemCatalog::VARCHAR))
		{
...
{
	const erydbSystemCatalog::ColDataType* types = row.getColTypes();
