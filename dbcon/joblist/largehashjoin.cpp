...
void logDiskIoInfo(uint64_t stepId, const AnyDataListSPtr& spdl)
{
	...
	ofstream umDiskIoFile("/var/log/erydb/trace/umdiskio.log", ios_base::app);
	...
	{
		(void)system("/bin/mv /var/log/erydb/trace/umdiskio.log /var/log/erydb/trace/umdiskio.bak");
	}
}
...
void LargeHashJoin::doHashJoin()
{
	...
	erydbassert(fOutputJobStepAssociation.outSize() >= 2);
	BucketDataList* Ap = 0;
	BucketDataList* Bp = 0;
...
{
	ostringstream oss;
	erydbSystemCatalog::OID oid1 = 0;
	erydbSystemCatalog::OID oid2 = 0;
	DataList_t* dl1;
	DataList_t* dl2;
	size_t idlsz;
