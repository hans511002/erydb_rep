...
//@bug 598 self-join
//typedef std::pair<execplan::erydbSystemCatalog::OID, std::string> OIDAliasPair;  //self-join
...
/* This looks like an inefficient way to get NULL values. Much easier ways
   to do it. */
int64_t valueNullNum(const erydbSystemCatalog::ColType& ct)
{
	int64_t n = 0;
	...
	case erydbSystemCatalog::CHAR:
		...
		if ( (ct.colDataType == erydbSystemCatalog::VARCHAR && ct.colWidth <= 7) ||
			(ct.colDataType == erydbSystemCatalog::VARBINARY && ct.colWidth <= 7) ||
			(ct.colDataType == erydbSystemCatalog::CHAR && ct.colWidth <= 8) )
		{
			 ...
			 * 8 bytes every time. */
			if (ct.colDataType == erydbSystemCatalog::CHAR)
			{
				switch (ct.colWidth)
				{
    ...
    case erydbSystemCatalog::UDECIMAL:
		if (ct.colWidth == execplan::erydbSystemCatalog::ONE_BYTE)
			...
			n = boost::any_cast<int>(anyVal);
		else if (ct.colWidth == execplan::erydbSystemCatalog::EIGHT_BYTE)
			n = boost::any_cast<long long>(anyVal);
		else
...
int64_t convertValueNum(const string& str, const erydbSystemCatalog::ColType& ct, bool isNull, uint8_t& rf)
{
								...
								const SOP& sop, SimpleFilter* sf)
{
	...
	erydbSystemCatalog::OID tableOid1 = tableOid(sc1, jobInfo.csc);
	erydbSystemCatalog::OID tableOid2 = tableOid(sc2, jobInfo.csc);
	string alias1(extractTableAlias(sc1));
	string alias2(extractTableAlias(sc2));
	erydbSystemCatalog::ColType ct1 = sc1->colType();
	erydbSystemCatalog::ColType ct2 = sc2->colType();
...
//    type of pseudo column is set by connector
			if (!sc1->schemaName().empty() && sc1->isEryDB() && !pc1)
				ct1 = jobInfo.csc->colType(sc1->oid());
			if (!sc2->schemaName().empty() && sc2->isEryDB() && !pc2)
...
//X
	...
	erydbSystemCatalog::OID dictOid1 = isDictCol(ct1);
	pcs1->alias(alias1);
	...
	erydbSystemCatalog::OID dictOid2 = isDictCol(ct2);
	pcs2->alias(alias2);
...
//X
	...
	pColStep* pcs1 = NULL;
	erydbSystemCatalog::OID oid1 = sc1->oid();
	erydbSystemCatalog::OID dictOid1 = isDictCol(ct1);
	...
	pColStep* pcs2 = NULL;
	erydbSystemCatalog::OID oid2 = sc2->oid();
	erydbSystemCatalog::OID dictOid2 = isDictCol(ct2);
...
{
	erydbSystemCatalog::OID tableOid1 = tableOid(sc, jobInfo.csc);
	erydbSystemCatalog::OID tableOid2 = execplan::CNX_VTABLE_ID;
	string alias1(extractTableAlias(sc));
	string alias2(jobInfo.subAlias);
	erydbSystemCatalog::ColType ct1 = sc->colType();
	erydbSystemCatalog::ColType ct2 = rc->resultType();
	const PseudoColumn* pc1 = dynamic_cast<const PseudoColumn*>(sc);
//XXX use this before connector sets colType in sc correctly.
//    type of pseudo column is set by connector
			if (!sc->schemaName().empty() && sc->isEryDB() && !pc1)
				ct1 = jobInfo.csc->colType(sc->oid());
//X
	JobStepVector jsv;
	SJSTEP step;

	erydbSystemCatalog::OID dictOid1 = 0;
	uint64_t tupleId1 = -1;
	uint64_t tupleId2 = -1;
...
bool optimizeIdbPatitionSimpleFilter(SimpleFilter* sf, JobStepVector& jsv, JobInfo& jobInfo)
{
	...
	// not a function or not erydbparttition
	if (fc == NULL || cc == NULL || fc->functionName().compare("erydbpartition") != 0)
		return false;
...
const JobStepVector doSimpleFilter(SimpleFilter* sf, JobInfo& jobInfo)
{
	...
	erydbSystemCatalog::OID tbl_oid = 0;
...
//XXX use this before connector sets colType in sc correctly.
//    type of pseudo column is set by connector
		if (!sc->schemaName().empty() && sc->isEryDB() && !pc)
...
//X
		...
		if ((dictOid = isDictCol(ct)) > 0  && ConstantColumn::NULLDATA != cc->type())
		{
			...
			if (filterWithDictionary(dictOid, jobInfo.stringScanThreshold))
			{
				...
				// token column
				erydbSystemCatalog::ColType tct;
				tct.colDataType = erydbSystemCatalog::BIGINT;
				tct.colWidth = 8;
				tct.scale = 0;
				tct.precision = 0;
			...
			else
			{
				...
				erydbSystemCatalog::ColType dct;
				dct.colDataType = erydbSystemCatalog::BIGINT;
				dct.colWidth = 8;
				dct.scale = 0;
				dct.precision = 0;
		...
		else
		{
			...
			try
			{
				...
				if (ct.colDataType == erydbSystemCatalog::FLOAT && !isNull)
				{
					float f = cc->getFloatVal();
					value = *(reinterpret_cast<int32_t*>(&f));
				}
				else if (ct.colDataType == erydbSystemCatalog::DOUBLE && !isNull)
				{
			...
			else
			{
				...
				erydbSystemCatalog::IndexOID indexOID = jobInfo.csc->lookupIndexNbr(indexName);
			 ...
			 lhsType == FUNCTIONCOLUMN || rhsType == FUNCTIONCOLUMN)
	{
		...
		if (rc && (!rc->joinInfo()) && rhsType == AGGREGATECOLUMN)
			throw ERYDBExcept(ERR_AGG_IN_WHERE);

		// @bug5848, CP elimination for erydbPartition(col)
		JobStepVector sv;
...
const JobStepVector doOuterJoinOnFilter(OuterJoinOnFilter* oj, JobInfo& jobInfo)
{
	...
	{
		// Due to erydb view handling, some joins may treated as expressions.
		ExpressionStep* es = new ExpressionStep(jobInfo);
		if (es == NULL)
			throw runtime_error("Failed to create ExpressionStep 1");
...
const JobStepVector doConstantFilter(const ConstantFilter* cf, JobInfo& jobInfo)
{
	...
	if (opStr == "and" || opStr == "or")
	{
		...
		erydbSystemCatalog::OID dictOid = 0;
		erydbSystemCatalog::ColType ct = sc.get()->colType();
...
//    type of pseudo column is set by connector
		if (!sc->schemaName().empty() && sc->isEryDB() && !pc)
			ct = jobInfo.csc->colType(sc->oid());
//X
		erydbSystemCatalog::OID tbOID = tableOid(sc.get(), jobInfo.csc);
		string alias(extractTableAlias(sc));
		string view(sc->viewName());
		string schema(sc->schemaName());
		...
		else
		{
			...
			erydbSystemCatalog::OID tblOid = tableOid(sc.get(), jobInfo.csc);
			string alias(extractTableAlias(sc));
			pColStep* pcs = NULL;
			if (pc == NULL)
...
const JobStepVector doFunctionFilter(const ParseTree* n, JobInfo& jobInfo)
{
	...
	if (functionName.compare("in") == 0 || functionName.compare(" in ") == 0)
	{
		...
		if (parm0Fc && parm0Fc->functionName().compare("erydbpartition") == 0)
		{
...
{
//	erydbassert(jobInfo.stack.size() >= 2);
