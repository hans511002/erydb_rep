...
void tupleKeyToProjectStep(uint32_t key, JobStepVector& jsv, JobInfo& jobInfo)
{
	...
	erydbSystemCatalog::OID oid = jobInfo.keyInfo->tupleKeyVec[key].fId;
	...
	erydbSystemCatalog::OID tableOid = jobInfo.keyInfo->tupleKeyToTableOid[key];
...
//	dummyJsa.outAdd(adl);
	...
	erydbSystemCatalog::ColType ct = jobInfo.keyInfo->colType[key];
	...
	if (sjs.get()->isDictCol() && !tokenOnly)
	{
		...
		erydbSystemCatalog::OID dictOid = jobInfo.keyInfo->tupleKeyVec[dictKey].fId;
...
inline void addColumnToRG(uint32_t cid, vector<uint32_t>& pos, vector<uint32_t>& oids,
	...
	vector<erydbSystemCatalog::ColDataType>& types, JobInfo& jobInfo)
{
...
inline void addColumnsToRG(uint32_t tid, vector<uint32_t>& pos, vector<uint32_t>& oids,
	vector<uint32_t>& keys, vector<uint32_t>& scale, vector<uint32_t>& precision,
	vector<erydbSystemCatalog::ColDataType>& types,
...
void addExpresssionStepsToBps(TableInfoMap::iterator& mit, SJSTEP& sjsp, JobInfo& jobInfo)
{
	...
	erydbSystemCatalog::OID tableOid = mit->second.fTableOid;
	...
	if (bps == NULL)
	{
		if (tableOid > 0)
		{
			...
			erydbSystemCatalog::ColType ct = jobInfo.keyInfo->colType[key0];
			map<uint32_t, erydbSystemCatalog::ColType>::iterator dkMit;
...
bool combineJobStepsByTable(TableInfoMap::iterator& mit, JobInfo& jobInfo)
{
	...
	erydbSystemCatalog::OID tableOid = tableInfo.fTableOid;
	...
	if (tableOid != CNX_VTABLE_ID)
	{
		...
		while (it != end)
		{
				 ...
				 dynamic_cast<TupleHashJoinStep*>((it + 1)->get()) != NULL))
			{
				...
				pos.push_back(2 + 8);
				erydbSystemCatalog::OID coid = jobInfo.keyInfo->dictOidToColOid[pds->oid()];
				oids.push_back(coid);
				uint32_t keyId = pds->tupleId();
				keys.push_back(keyId);
				types.push_back(erydbSystemCatalog::BIGINT);
				scale.push_back(0);
                    ...
                    TableInfoMap& tableInfoMap, JobInfo& jobInfo)
{
	...
	for (; (i != jobInfo.functionJoins.end()); i++)
	{
		...
		if (functionJoinPairs.empty())
		{
			...
			if (ti1.dtype==erydbSystemCatalog::CHAR || ti1.dtype==erydbSystemCatalog::VARCHAR)
...
void spanningTreeCheck(TableInfoMap& tableInfoMap, JobStepVector joinSteps, JobInfo& jobInfo)
{
	...
	if (!spanningTree)
	{
		cerr << boldStart << ERYDBErrorInfo::instance()->errorMsg(errcode, args) << boldStop << endl;
	...
	if (smallSides.size() > 0)
	{
		...
		if (!bps && !thjs && !tsas)
		{
			if (dynamic_cast<SubQueryStep*>(spjs.get()))
				throw ERYDBExcept(ERR_NON_SUPPORT_SUB_QUERY_TYPE);
		...
		for (vector<SP_JoinInfo>::iterator i = smallSides.begin(); i != smallSides.end(); i++)
		{
			...
			if (jobInfo.trace)
			{
				...
				{
					erydbSystemCatalog::OID oid1 = jobInfo.keyInfo->tupleKeyVec[*k1].fId;
					erydbSystemCatalog::TableColName tcn1 = jobInfo.csc->colName(oid1);
				...
				{
					erydbSystemCatalog::OID oid2 = jobInfo.keyInfo->tupleKeyVec[*k2].fId;
					erydbSystemCatalog::TableColName tcn2 = jobInfo.csc->colName(oid2);
							...
							const bool overrideLargeSideEstimate)
{
	if (jobInfo.trace)
	{
		...
		for (uint32_t i = 0; i < keyInfo->tupleKeyVec.size(); ++i)
		{
			...
			if (tid == i)
			{
				erydbSystemCatalog::OID oid = keyInfo->tupleKeyVec[i].fId;
	...
	if (jobInfo.hasAggregation && (jobInfo.correlateSteps.size() > 0))
	{
		...
		if ((exp != NULL) && (thjs != NULL) && (thjs->getJoinType() & CORRELATED))
			throw ERYDBExcept(
						ERYDBErrorInfo::instance()->errorMsg(ERR_NON_SUPPORT_NEQ_AGG_SUB),
...
SJSTEP unionQueries(JobStepVector& queries, uint64_t distinctUnionNum, JobInfo& jobInfo)
{
	...
	vector<vector<erydbSystemCatalog::ColType> > queryColTypes;
	for (uint64_t j = 0; j < colCount; ++j)
		queryColTypes.push_back(vector<erydbSystemCatalog::ColType>(queries.size()));
	...
	for (uint64_t i = 0; i < queries.size(); i++)
	{
		...
		const vector<erydbSystemCatalog::ColDataType>& typesIn = rg.getColTypes();
	...
	for (uint64_t j = 0; j < colCount; ++j)
	{
		erydbSystemCatalog::ColType colType = DataConvert::convertUnionColType(queryColTypes[j]);
	...
	{
		erydbSystemCatalog::ColType ct = jobInfo.deliveredCols[i]->resultType();
...
ct.colWidth = width[i];
		...
		if (ct.colDataType == erydbSystemCatalog::VARCHAR)
			ct.colWidth--;
		else if (ct.colDataType == erydbSystemCatalog::VARBINARY)
			ct.colWidth -= 2;
