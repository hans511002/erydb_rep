...
typedef std::vector<execplan::erydbSystemCatalog::OID> OIDVector;
typedef std::vector<std::pair<execplan::erydbSystemCatalog::OID, int> > OIDIntVector;
...
public:
    ...
    pColStep(
		...
		execplan::erydbSystemCatalog::OID tableOid,
		const execplan::erydbSystemCatalog::ColType& ct,
	...
	virtual execplan::erydbSystemCatalog::OID oid() const { return fOid; }

	virtual execplan::erydbSystemCatalog::OID tableOid() const { return fTableOid; }
	...
	const execplan::erydbSystemCatalog::ColType& colType() const { return fColType; }
...
private:
	...
	ResourceManager& fRm;
	boost::shared_ptr<execplan::erydbSystemCatalog> sysCat;
	execplan::erydbSystemCatalog::OID fOid;
	execplan::erydbSystemCatalog::OID fTableOid;
	execplan::erydbSystemCatalog::ColType fColType;
	...
	// @bug 663 - Added fSwallowRows for erydb.caltrace(16) which is TRACE_FLAGS::TRACE_NO_ROWS4.
...
public:
    ...
    pDictionaryStep(
		execplan::erydbSystemCatalog::OID oid,
		execplan::erydbSystemCatalog::OID tabelOid,
		const execplan::erydbSystemCatalog::ColType& ct,
		const JobInfo& jobInfo);
	...
	execplan::erydbSystemCatalog::ColType& colType() { return fColType; }
	execplan::erydbSystemCatalog::ColType colType() const { return fColType; }
...
private:
	...
	std::map<execplan::erydbSystemCatalog::OID, std::tr1::unordered_map<int64_t, struct BRM::EMEntry> > extentsMap;
...
public:
    ...
    FilterStep(const execplan::erydbSystemCatalog::ColType& colType, const JobInfo& jobInfo);
    ~FilterStep();
    ...
    execplan::erydbSystemCatalog::OID tableOid() const { return fTableOID; }
    void tableOid(execplan::erydbSystemCatalog::OID tableOid) { fTableOID = tableOid; }
	const execplan::erydbSystemCatalog::ColType& colType() const { return fColType; }
	void setBOP(int8_t b);
	int8_t BOP() const { return fBOP; }
...
private:
    ...
    execplan::erydbSystemCatalog::OID oid() const { return 0; }
...
private:
	...
	boost::shared_ptr<execplan::erydbSystemCatalog> catalog;
	execplan::erydbSystemCatalog::OID fOid;
	execplan::erydbSystemCatalog::OID fTableOid;
	uint8_t colWidth;
	uint16_t realWidth;
