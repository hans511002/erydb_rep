...
//X
	...
	if (j == m.end())
	{
		...
		throw ERYDBExcept(ERYDBErrorInfo::instance()->errorMsg(ERR_WF_COLUMN_MISSING, name),
...
{
	erydbassert(fRowGroupOut.usesStringTable() == fRowGroupDelivered.usesStringTable());
...
void WindowFunctionStep::checkWindowFunction(erydbSelectExecutionPlan* csep, JobInfo& jobInfo)
{
...
void WindowFunctionStep::initialize(const RowGroup& rg, JobInfo& jobInfo)
{
	...
	const vector<erydbSystemCatalog::ColDataType>& types = rg.getColTypes();
	...
	for (RetColsVector::iterator i=jobInfo.windowCols.begin(); i<jobInfo.windowCols.end(); i++)
	{
		...
		// @bug6065, window functions that will update string table
		{
			erydbSystemCatalog::ColType rt = wc->resultType();
			if ((types[ridx] == erydbSystemCatalog::CHAR || 
			     types[ridx] == erydbSystemCatalog::VARCHAR) &&
	...
	for (RetColsVector::iterator i = fExpression.begin(); i < fExpression.end(); i++)
	{
		...
		for (vector<SimpleColumn*>::const_iterator j = scols.begin(); j != scols.end(); j++)
		{
			...
			erydbSystemCatalog::OID dictOid = joblist::isDictCol((*j)->colType());
			if (dictOid > 0)
			{
...
//				(*j)->inputIndex(getColumnIndex(*j, colIndexMap, jobInfo));
	...
	vector<erydbSystemCatalog::ColDataType> types1;
	pos1.push_back(2);
...
void WindowFunctionStep::execute()
{
		...
		while (more && !cancelled())
		{
			...
			if (rowCnt > 0)
			{
				...
				if (fRm.getMemory(memAdd, fSessionMemLimit) == false)
					throw ERYDBExcept(ERR_WF_DATA_SET_TOO_BIG);
	...
	catch(const ERYDBExcept &erydb) {
		handleException(erydb.what(), erydb.errorCode());
	}
	catch(const std::exception& ex)
	{
                                                         ...
                                                         JobInfo& jobInfo)
{
	...
	if (cc != NULL)
	{
		...
		else
		{
			...
			throw ERYDBExcept(ERYDBErrorInfo::instance()->errorMsg(ERR_WF_BOUND_OUT_OF_RANGE, str),
	...
	else
	{
		...
		switch(ti.dtype)
		{
			...
			default:
			{
				...
				throw ERYDBExcept(ERYDBErrorInfo::instance()->errorMsg(ERR_WF_INVALID_BOUND_TYPE, str),
                                                     ...
                                                     bool s)
{
	...
	switch(b.fFrame)
	{
		...
		default:  //  unknown
		{
			throw ERYDBExcept(ERYDBErrorInfo::instance()->errorMsg(ERR_WF_UNKNOWN_BOUND, b.fFrame),
