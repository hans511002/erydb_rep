/* Copyright (C) 2014 EryDB, Inc.
   ...
   /***********************************************************************
    *   $Id: erydbsystemcatalog.cpp 9594 2013-06-04 18:19:21Z pleblanc $
    *
    *
    ***********************************************************************/
#include <unistd.h>
...
{ \
fExeMgr->shutdown(); \
throw runtime_error("ERYDB_INTERNAL_ERROR"); \
}
#else
#define BAIL_IF_0(m)
#endif
...
namespace execplan
{
    ...
    const string colDataTypeToString(erydbSystemCatalog::ColDataType cdt) {
        switch (cdt) {
...
namespace execplan
{
    ...
    erydbSystemCatalog::NJLSysDataList::~NJLSysDataList() {
        NJLSysDataVector::iterator it;
        for (it = sysDataVec.begin(); it != sysDataVec.end(); it++)
            delete *it;
    }

    const erydbSystemCatalog::TableColName make_tcn(const string& s, const string& t, const string& c) {
        erydbSystemCatalog::TableColName tcns;
    ...
    const erydbSystemCatalog::TableName make_table(const string& s, const string& t) {
        erydbSystemCatalog::TableName tn;
        tn.schema = s;
        tn.table = t;
    ...
    const erydbSystemCatalog::TableAliasName make_aliastable(const string& s, const string& t, const string& a, const bool isEryDB) {
        erydbSystemCatalog::TableAliasName tn;
        tn.schema = s;
        tn.table = t;
        tn.alias = a;
        tn.view = "";
    ...
    const erydbSystemCatalog::TableAliasName make_aliasview(const string& s, const string& t, const string& a, const string& v, const bool isEryDB) {
        erydbSystemCatalog::TableAliasName tn;
        tn.schema = s;
        tn.table = t;
        tn.alias = a;
    ...
    bool erydbSystemCatalog::TableColName::operator<(const TableColName& rhs) const {
    ...
    const string erydbSystemCatalog::TableColName::toString() const {
        string os;
        os = schema + '.' + table + '.' + column;
        return os;
    }

    bool erydbSystemCatalog::TableName::operator<(const TableName& rhs) const {
        if (schema < rhs.schema) {
            return true;
        } else if (schema == rhs.schema) {
            if (table < rhs.table) {
    ...
    void erydbSystemCatalog::TableAliasName::clear() {
        schema.clear();
        table.clear();
        alias.clear();
        view.clear();
    ...
    bool erydbSystemCatalog::TableAliasName::operator<(const TableAliasName& rhs) const {
        if (schema < rhs.schema) {
            return true;
        } else if (schema == rhs.schema) {
            if (table < rhs.table) {
    ...
    void erydbSystemCatalog::TableAliasName::serialize(messageqcpp::ByteStream& b) const {
        b << schema;
        b << table;
        b << alias;
        b << view;
    ...
    void erydbSystemCatalog::TableAliasName::unserialize(messageqcpp::ByteStream& b) {
        b >> schema;
        b >> table;
        b >> alias;
        b >> view;
    ...
    /*static*/
    boost::mutex erydbSystemCatalog::map_mutex;
    /*static*/
    erydbSystemCatalog::CatalogMap erydbSystemCatalog::fCatalogMap;
    /*static*/
    uint32_t erydbSystemCatalog::fModuleID = numeric_limits<uint32_t>::max();

    const erydbSystemCatalog::OID erydbSystemCatalog::lookupTableOID(const TableName& tablename) {
        ...
        if (aTableName.schema.compare(ERYDB_SCHEMA) != 0)
        ...
        SimpleColumn *c1 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSTABLE_TABLE + "." + OBJECTID_COL, fSessionID);
        SimpleColumn *c2 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSTABLE_TABLE + "." + SCHEMA_COL, fSessionID);
        SimpleColumn *c3 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSTABLE_TABLE + "." + TABLENAME_COL, fSessionID);

        SRCP srcp;
        srcp.reset(c1);
        colMap.insert(CMVT_(ERYDB_SCHEMA + "." + SYSTABLE_TABLE + "." + OBJECTID_COL, srcp));
        srcp.reset(c2);
        colMap.insert(CMVT_(ERYDB_SCHEMA + "." + SYSTABLE_TABLE + "." + SCHEMA_COL, srcp));
        srcp.reset(c3);
        colMap.insert(CMVT_(ERYDB_SCHEMA + "." + SYSTABLE_TABLE + "." + TABLENAME_COL, srcp));
    ...
    const erydbSystemCatalog::OID erydbSystemCatalog::lookupOID(const TableColName& tableColName) {
        ...
        if (aTableColName.schema.compare(ERYDB_SCHEMA) != 0)
        ...
        if (fOIDmap.size() > 0) {
            ...
            if (iter != fOIDmap.end()) {
                if (fIdentity == EC && aTableColName.schema.compare(ERYDB_SCHEMA) != 0) {
        ...
        string columnlength = ERYDB_SCHEMA + "." + SYSCOLUMN_TABLE + "." + COLUMNLEN_COL;
        string objectid = ERYDB_SCHEMA + "." + SYSCOLUMN_TABLE + "." + OBJECTID_COL;
        string datatype = ERYDB_SCHEMA + "." + SYSCOLUMN_TABLE + "." + DATATYPE_COL;
        string dictobjectid = ERYDB_SCHEMA + "." + SYSCOLUMN_TABLE + "." + DICTOID_COL;
        string listobjectid = ERYDB_SCHEMA + "." + SYSCOLUMN_TABLE + "." + LISTOBJID_COL;
        string treeobjectid = ERYDB_SCHEMA + "." + SYSCOLUMN_TABLE + "." + TREEOBJID_COL;
        string columnposition = ERYDB_SCHEMA + "." + SYSCOLUMN_TABLE + "." + COLUMNPOS_COL;
        string scale = ERYDB_SCHEMA + "." + SYSCOLUMN_TABLE + "." + SCALE_COL;
        string precision = ERYDB_SCHEMA + "." + SYSCOLUMN_TABLE + "." + PRECISION_COL;
        string defaultvalue = ERYDB_SCHEMA + "." + SYSCOLUMN_TABLE + "." + DEFAULTVAL_COL;
        // the following columns will be save in cache although it's not needed for now
        string columnname = ERYDB_SCHEMA + "." + SYSCOLUMN_TABLE + "." + COLNAME_COL;
        string tablename = ERYDB_SCHEMA + "." + SYSCOLUMN_TABLE + "." + TABLENAME_COL;
        string schemaname = ERYDB_SCHEMA + "." + SYSCOLUMN_TABLE + "." + SCHEMA_COL;
        string compressiontype = ERYDB_SCHEMA + "." + SYSCOLUMN_TABLE + "." + COMPRESSIONTYPE_COL;
        string autoincrement = ERYDB_SCHEMA + "." + SYSCOLUMN_TABLE + "." + AUTOINC_COL;
        string nextVal = ERYDB_SCHEMA + "." + SYSCOLUMN_TABLE + "." + NEXTVALUE_COL;
        string nullable = ERYDB_SCHEMA + "." + SYSCOLUMN_TABLE + "." + NULLABLE_COL;
       ...
       // delete col[9];
        ct.columnOID = coloid;
        // populate colinfomap cache and oerydbitmap
    ...
    void erydbSystemCatalog::getSysData(erydbSelectExecutionPlan& csep,
        NJLSysDataList& sysDataList,
        ...
        tablelist.push_back(make_aliastable("erydbsys", sysTableName, ""));
        csep.tableList(tablelist);

        // populate the returned column list as column map
        csep.returnedCols().clear();
        erydbSelectExecutionPlan::ColumnMap::const_iterator it;
        ...
        if (fIdentity == EC) {
            ...
            while (tryCnt < 5) {
                ...
                try {
                    ...
                    break;
                } catch (ERYDBExcept&) // error already occured. this is not a broken pipe
                {
                    throw;
                } catch (...) {
                    ...
                    try {
                        ...
                        throw ERYDBExcept(ERR_LOST_CONN_EXEMGR);
                    }
                }
            }
            ...
            if (tryCnt >= 5)
                ...
                throw ERYDBExcept(ERR_SYSTEM_CATALOG);
        }
    ...
    void erydbSystemCatalog::getSysData_EC(erydbSelectExecutionPlan& csep,
        NJLSysDataList& sysDataList,
        const string& sysTableName) {
        DEBUG << "Enter getSysData_EC " << fSessionID << endl;

        uint32_t tableOID = ERYDB_VTABLE_ID;
        ByteStream bs;
        uint32_t status;
    ...
    void erydbSystemCatalog::getSysData_FE(const erydbSelectExecutionPlan& csep,
        NJLSysDataList& sysDataList,
        const string& sysTableName) {
        ...
        // Send the erydbSelectExecutionPlan to ExeMgr.
        csep.serialize(msg);
        fExeMgr->write(msg);

        // Get the table oid for the system table being queried.
        TableName tableName;
        tableName.schema = ERYDB_SCHEMA;
        ...
        while (true) {
            ...
            if ((status = rowGroup->getStatus()) != 0) {
                if (status >= 1000) // new error system
                {
                    ...
                    bs >> emsgStr;
                    throw ERYDBExcept(emsgStr, rowGroup->getStatus());
                } else {
                    throw ERYDBExcept(ERR_SYSTEM_CATALOG);
                }
            }
    ...
    const erydbSystemCatalog::ColType erydbSystemCatalog::colType(const OID& Oid) {
        ...
        string compressionType = ERYDB_SCHEMA + "." + SYSCOLUMN_TABLE + "." + COMPRESSIONTYPE_COL;
        string autoincrement = ERYDB_SCHEMA + "." + SYSCOLUMN_TABLE + "." + AUTOINC_COL;
        string nextvalue = ERYDB_SCHEMA + "." + SYSCOLUMN_TABLE + "." + NEXTVALUE_COL;
    ...
    const erydbSystemCatalog::ColType erydbSystemCatalog::colTypeDct(const OID& dictOid) {
        if (dictOid >= USER_OBJECT_ID)
            DEBUG << "Enter colType: " << dictOid << endl;
        ColType ct;
        // invalid oid
        if (dictOid <= MAX_DBROOT)
            return ct;
    ...
    const erydbSystemCatalog::TableColName erydbSystemCatalog::colName(const OID& oid) {
        if (oid >= USER_OBJECT_ID)
        ...
        SimpleColumn *c1 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSCOLUMN_TABLE + "." + OBJECTID_COL, fSessionID);
        SimpleColumn *c2 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSCOLUMN_TABLE + "." + SCHEMA_COL, fSessionID);
        SimpleColumn *c3 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSCOLUMN_TABLE + "." + TABLENAME_COL, fSessionID);
        SimpleColumn *c4 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSCOLUMN_TABLE + "." + COLNAME_COL, fSessionID);

        SRCP srcp;
        srcp.reset(c1);
        colMap.insert(CMVT_(ERYDB_SCHEMA + "." + SYSCOLUMN_TABLE + "." + OBJECTID_COL, srcp));
        srcp.reset(c2);
        colMap.insert(CMVT_(ERYDB_SCHEMA + "." + SYSCOLUMN_TABLE + "." + SCHEMA_COL, srcp));
        srcp.reset(c3);
        colMap.insert(CMVT_(ERYDB_SCHEMA + "." + SYSCOLUMN_TABLE + "." + TABLENAME_COL, srcp));
        srcp.reset(c4);
        colMap.insert(CMVT_(ERYDB_SCHEMA + "." + SYSCOLUMN_TABLE + "." + COLNAME_COL, srcp));
        csep.columnMapNonStatic(colMap);
    ...
    const erydbSystemCatalog::TableColName erydbSystemCatalog::dictColName(const OID& oid) {
        if (oid >= USER_OBJECT_ID)
        ...
        SimpleColumn *c1 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSCOLUMN_TABLE + "." + DICTOID_COL, fSessionID);
        SimpleColumn *c2 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSCOLUMN_TABLE + "." + SCHEMA_COL, fSessionID);
        SimpleColumn *c3 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSCOLUMN_TABLE + "." + TABLENAME_COL, fSessionID);
        SimpleColumn *c4 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSCOLUMN_TABLE + "." + COLNAME_COL, fSessionID);
        ...
        SRCP srcp;
        srcp.reset(c1);
        colMap.insert(CMVT_(ERYDB_SCHEMA + "." + SYSCOLUMN_TABLE + "." + DICTOID_COL, srcp));
        srcp.reset(c2);
        colMap.insert(CMVT_(ERYDB_SCHEMA + "." + SYSCOLUMN_TABLE + "." + SCHEMA_COL, srcp));
        srcp.reset(c3);
        colMap.insert(CMVT_(ERYDB_SCHEMA + "." + SYSCOLUMN_TABLE + "." + TABLENAME_COL, srcp));
    ...
    const uint64_t erydbSystemCatalog::nextAutoIncrValue(TableName aTableName) {
        transform(aTableName.schema.begin(), aTableName.schema.end(), aTableName.schema.begin(), to_lower());
        transform(aTableName.table.begin(), aTableName.table.end(), aTableName.table.begin(), to_lower());
    ...
    int32_t erydbSystemCatalog::autoColumOid(TableName aTableName) {
        transform(aTableName.schema.begin(), aTableName.schema.end(), aTableName.schema.begin(), to_lower());
        transform(aTableName.table.begin(), aTableName.table.end(), aTableName.table.begin(), to_lower());
    ...
    const erydbSystemCatalog::ROPair erydbSystemCatalog::nextAutoIncrRid(const OID& columnoid) {
    ...
    const erydbSystemCatalog::OID erydbSystemCatalog::colBitmap(const OID& oid) const {
        return oid;
        DEBUG << "Enter colBitmap: Not implemented" << endl;
    }

    const erydbSystemCatalog::SCN erydbSystemCatalog::scn(void) const {
    ...
    /* static */
    boost::shared_ptr<erydbSystemCatalog> erydbSystemCatalog::makeerydbSystemCatalog(uint32_t sessionID) {
        boost::mutex::scoped_lock lock(map_mutex);
        boost::shared_ptr<erydbSystemCatalog> instance;
        CatalogMap::const_iterator it = fCatalogMap.find(sessionID);
        if (sessionID == 0) {
            if (it == fCatalogMap.end()) {
                instance.reset(new erydbSystemCatalog());
            ...
            if (fCatalogMap.size() != 1) {
                //throw runtime_error ("No erydb system catalog instance found.");
                ostringstream oss;
    ...
    /* static */
    void erydbSystemCatalog::removeerydbSystemCatalog(uint32_t sessionID) {
        boost::mutex::scoped_lock lock(map_mutex);
        DEBUG << "remove erydb system catalog for session " << sessionID << endl;
    ...
    erydbSystemCatalog::erydbSystemCatalog() :
        fExeMgr(new ClientRotator(0, "ExeMgr")),
        fSessionID(0) {
...
#ifdef _MSC_VER
            p = "EC";
            string cfStr = ERYDBreadRegistry("SyscatIdent");
            if (!cfStr.empty())
                p = cfStr.c_str();
#else        
            p = getenv("ERYDB_CSC_IDENT");
#endif
            if (p && *p) {
                localModuleType = p;
            } else {
...
#if 0
    const erydbSystemCatalog::RIDList erydbSystemCatalog::indexRIDs(const TableName& tableName) {
        ...
        SimpleColumn *c1 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSINDEX_TABLE + "." + INDEXNAME_COL, fSessionID);
        SimpleColumn *c2 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSINDEX_TABLE + "." + SCHEMA_COL, fSessionID);
        SimpleColumn *c3 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSINDEX_TABLE + "." + TABLENAME_COL, fSessionID);

        SRCP srcp;
        srcp.reset(c1);
        colMap.insert(CMVT_(ERYDB_SCHEMA + "." + SYSINDEX_TABLE + "." + INDEXNAME_COL, srcp));
        srcp.reset(c2);
        colMap.insert(CMVT_(ERYDB_SCHEMA + "." + SYSINDEX_TABLE + "." + SCHEMA_COL, srcp));
        srcp.reset(c3);
        colMap.insert(CMVT_(ERYDB_SCHEMA + "." + SYSINDEX_TABLE + "." + TABLENAME_COL, srcp));
        csep.columnMapNonStatic(colMap);
    ...
    const erydbSystemCatalog::RIDList erydbSystemCatalog::indexColRIDs(const TableName& tableName) {
        ...
        SimpleColumn *c1 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSINDEXCOL_TABLE + "." + INDEXNAME_COL, fSessionID);
        SimpleColumn *c2 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSINDEXCOL_TABLE + "." + SCHEMA_COL, fSessionID);
        SimpleColumn *c3 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSINDEXCOL_TABLE + "." + TABLENAME_COL, fSessionID);

        SRCP srcp;
        srcp.reset(c1);
        colMap.insert(CMVT_(ERYDB_SCHEMA + "." + SYSINDEXCOL_TABLE + "." + INDEXNAME_COL, srcp));
        srcp.reset(c2);
        colMap.insert(CMVT_(ERYDB_SCHEMA + "." + SYSINDEXCOL_TABLE + "." + SCHEMA_COL, srcp));
        srcp.reset(c3);
        colMap.insert(CMVT_(ERYDB_SCHEMA + "." + SYSINDEXCOL_TABLE + "." + TABLENAME_COL, srcp));
        csep.columnMapNonStatic(colMap);
    ...
    const erydbSystemCatalog::RIDList erydbSystemCatalog::indexColRIDs(const IndexName& indexName) {
        ...
        if (aIndexName.schema.compare(ERYDB_SCHEMA) != 0)
    ...
    const erydbSystemCatalog::RIDList erydbSystemCatalog::constraintRIDs(const TableName& tableName) {
        ...
        SimpleColumn *c1 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSCONSTRAINT_TABLE + "." + CONSTRAINTNAME_COL, fSessionID);
        SimpleColumn *c2 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSCONSTRAINT_TABLE + "." + SCHEMA_COL, fSessionID);
        SimpleColumn *c3 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSCONSTRAINT_TABLE + "." + TABLENAME_COL, fSessionID);

        SRCP srcp;
        srcp.reset(c1);
        colMap.insert(CMVT_(ERYDB_SCHEMA + "." + SYSCONSTRAINT_TABLE + "." + CONSTRAINTNAME_COL, srcp));
        srcp.reset(c2);
        colMap.insert(CMVT_(ERYDB_SCHEMA + "." + SYSCONSTRAINT_TABLE + "." + SCHEMA_COL, srcp));
        srcp.reset(c3);
        colMap.insert(CMVT_(ERYDB_SCHEMA + "." + SYSCONSTRAINT_TABLE + "." + TABLENAME_COL, srcp));
        csep.columnMapNonStatic(colMap);
    ...
    const erydbSystemCatalog::IndexNameList erydbSystemCatalog::colValueSysconstraint(const TableColName& tableColName, bool useCache) {
        ...
        SimpleColumn *c1 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSCONSTRAINTCOL_TABLE + "." + CONSTRAINTNAME_COL, fSessionID);
        SimpleColumn *c2 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSCONSTRAINTCOL_TABLE + "." + SCHEMA_COL, fSessionID);
        SimpleColumn *c3 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSCONSTRAINTCOL_TABLE + "." + TABLENAME_COL, fSessionID);
        SimpleColumn *c4 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSCONSTRAINTCOL_TABLE + "." + COLNAME_COL, fSessionID);

        SRCP srcp;
        srcp.reset(c1);
        colMap.insert(CMVT_(ERYDB_SCHEMA + "." + SYSCONSTRAINTCOL_TABLE + "." + CONSTRAINTNAME_COL, srcp));
        srcp.reset(c2);
        colMap.insert(CMVT_(ERYDB_SCHEMA + "." + SYSCONSTRAINTCOL_TABLE + "." + SCHEMA_COL, srcp));
        srcp.reset(c3);
        colMap.insert(CMVT_(ERYDB_SCHEMA + "." + SYSCONSTRAINTCOL_TABLE + "." + TABLENAME_COL, srcp));
        srcp.reset(c4);
        colMap.insert(CMVT_(ERYDB_SCHEMA + "." + SYSCONSTRAINTCOL_TABLE + "." + COLNAME_COL, srcp));
        csep.columnMapNonStatic(colMap);
    ...
    const erydbSystemCatalog::RID erydbSystemCatalog::constraintRID(const std::string constraintName) {
        ...
        string msg("erydbSystemCatalog::constraintRID: no RID found for ");
        msg += constraintName;
        throw runtime_error(msg);
    }

    const erydbSystemCatalog::RIDList erydbSystemCatalog::constraintColRID(const std::string constraintName) {
        ...
        vector<ColumnResult*>::const_iterator it;
        erydbSystemCatalog::ROPair ropair;
        for (it = sysDataList.begin(); it != sysDataList.end(); it++) {
            if ((*it)->ColumnOID() == oid) {
                for (int i = 0; i < (*it)->dataCount(); i++) {
                    if (fIdentity == EC) {
    ...
    const std::string erydbSystemCatalog::colValueSysconstraintCol(const TableColName& tableColName) {
    ...
    const erydbSystemCatalog::RIDList erydbSystemCatalog::constraintColRIDs(const TableName& tableName) {
    ...
    const erydbSystemCatalog::RID erydbSystemCatalog::constraintColRID(const TableColName& tableColName) {
        /* SQL statement: select constraintname from sysconstraintcol where schema=tableColName.schema and
    ...
    const vector< pair<erydbSystemCatalog::OID, erydbSystemCatalog::TableName> > erydbSystemCatalog::getTables(const std::string schema) {
        string schemaname = schema;
        vector < pair<OID, TableName> > tables;
        transform(schemaname.begin(), schemaname.end(), schemaname.begin(), to_lower());
        if (schemaname == ERYDB_SCHEMA) {
            // systables
            tables.push_back(make_pair(SYSTABLE_BASE, make_table(ERYDB_SCHEMA, SYSTABLE_TABLE)));
            tables.push_back(make_pair(SYSCOLUMN_BASE, make_table(ERYDB_SCHEMA, SYSCOLUMN_TABLE)));
            return tables;
        }
        ...
        SimpleColumn *c1 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSTABLE_TABLE + "." + TABLENAME_COL, fSessionID);
        SimpleColumn *c2 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSTABLE_TABLE + "." + SCHEMA_COL, fSessionID);
        SimpleColumn *c3 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSTABLE_TABLE + "." + OBJECTID_COL, fSessionID);
        SimpleColumn *c4 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSTABLE_TABLE + "." + CREATEDATE_COL, fSessionID);
        ...
        colMap.insert(CMVT_(ERYDB_SCHEMA + "." + SYSTABLE_TABLE + "." + OBJECTID_COL, srcp));
        srcp.reset(c4);
        colMap.insert(CMVT_(ERYDB_SCHEMA + "." + SYSTABLE_TABLE + "." + CREATEDATE_COL, srcp));
        csep.columnMapNonStatic(colMap);
    ...
    const int erydbSystemCatalog::getTableCount() {
        int tableCnt = 0;
     ...
     * tablename=tableColName.table and columnname=tableColName.column;*/
    const erydbSystemCatalog::ROPair erydbSystemCatalog::columnRID(const TableColName& tableColName) {
        ROPair rp;
        TableColName aTableColName;
        aTableColName.schema = tableColName.schema;
        aTableColName.table = tableColName.table;
        ...
        if (aTableColName.schema.compare(ERYDB_SCHEMA) != 0)
            ...
            << "|" << tableColName.column << endl;
        //Check whether cache needs to be flushed
        if (aTableColName.schema.compare(ERYDB_SCHEMA)) {
            checkSysCatVer();
        }
        /* SQL statement: select objectid from syscolumn where schema=tableColName.schema and
           tablename=tableColName.table and columnname=tableColName.column;*/
    ...
    const erydbSystemCatalog::RIDList erydbSystemCatalog::columnRIDs(const TableName& tableName, bool useCache) {
        ...
        if (aTableName.schema != ERYDB_SCHEMA)
...
#endif
        boost::mutex::scoped_lock lk3(fColinfomapLock);
        if (aTableName.schema == ERYDB_SCHEMA || (useCache && ti_iter != fTableInfoMap.end())) {
            if (aTableName.schema == ERYDB_SCHEMA)
                lk3.unlock();
            else
    ...
    const erydbSystemCatalog::TableName erydbSystemCatalog::tableName(const OID& tableoid) {
        //Check whether cache needs to be flushed
    ...
    const erydbSystemCatalog::ROPair erydbSystemCatalog::tableRID(const TableName& tableName) {
        TableName aTableName;
        aTableName.schema = tableName.schema;
        aTableName.table = tableName.table;
        transform(aTableName.schema.begin(), aTableName.schema.end(), aTableName.schema.begin(), to_lower());
        ...
        //     rp.rid = -1; @bug1866  use default
            ...
            // erydbsys only needs oid
        boost::mutex::scoped_lock lk1(fTableInfoMapLock);
        Tablemap::const_iterator iter = fTablemap.find(aTableName);
        if (aTableName.schema.compare("erydbsys") == 0 && iter != fTablemap.end()) {
            rp.objnum = (*iter).second;
            return rp;
        }
        lk1.unlock();
        ...
        //string msg("erydbSystemCatalog::tableRID: no OID found for ");
...
#if 0
    const erydbSystemCatalog::IndexNameList erydbSystemCatalog::indexNames(const TableName& tableName) {
        ...
        SimpleColumn *c2 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSINDEX_TABLE + "." + MULTICOLFLAG_COL, fSessionID);
        SimpleColumn *c3 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSINDEX_TABLE + "." + SCHEMA_COL, fSessionID);
        SimpleColumn *c4 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSINDEX_TABLE + "." + TABLENAME_COL, fSessionID);
        SimpleColumn *c5 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSINDEX_TABLE + "." + INDEXNAME_COL, fSessionID);

        SRCP srcp;
        srcp.reset(c2);
        colMap.insert(CMVT_(ERYDB_SCHEMA + "." + SYSINDEX_TABLE + "." + MULTICOLFLAG_COL, srcp));
        srcp.reset(c3);
        colMap.insert(CMVT_(ERYDB_SCHEMA + "." + SYSINDEX_TABLE + "." + SCHEMA_COL, srcp));
        srcp.reset(c4);
        colMap.insert(CMVT_(ERYDB_SCHEMA + "." + SYSINDEX_TABLE + "." + TABLENAME_COL, srcp));
    ...
    const erydbSystemCatalog::TableColNameList erydbSystemCatalog::indexColNames(const IndexName& indexName) {
        DEBUG << "Enter indexColNames: " << indexName.schema << "|" << indexName.table << "|" << indexName.index << endl;

        // not cached yet
        erydbSystemCatalog::TableColNameList tableColNameList;
        ...
        SimpleColumn *c1 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSINDEXCOL_TABLE + "." + COLNAME_COL, fSessionID);
        SimpleColumn *c2 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSINDEXCOL_TABLE + "." + COLUMNPOS_COL, fSessionID);
        SimpleColumn *c3 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSINDEXCOL_TABLE + "." + SCHEMA_COL, fSessionID);
        SimpleColumn *c4 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSINDEXCOL_TABLE + "." + TABLENAME_COL, fSessionID);
        SimpleColumn *c5 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSINDEXCOL_TABLE + "." + INDEXNAME_COL, fSessionID);

        SRCP srcp;
        srcp.reset(c1);
        colMap.insert(CMVT_(ERYDB_SCHEMA + "." + SYSINDEXCOL_TABLE + "." + COLNAME_COL, srcp));
        srcp.reset(c2);
        colMap.insert(CMVT_(ERYDB_SCHEMA + "." + SYSINDEXCOL_TABLE + "." + COLUMNPOS_COL, srcp));
        srcp.reset(c3);
        colMap.insert(CMVT_(ERYDB_SCHEMA + "." + SYSINDEXCOL_TABLE + "." + SCHEMA_COL, srcp));
        srcp.reset(c4);
        colMap.insert(CMVT_(ERYDB_SCHEMA + "." + SYSINDEXCOL_TABLE + "." + TABLENAME_COL, srcp));
    ...
    const erydbSystemCatalog::TableColNameList erydbSystemCatalog::constraintColNames(const std::string constraintName) {
        ...
        SimpleColumn *c1 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSCONSTRAINTCOL_TABLE + "." + COLNAME_COL, fSessionID);
        SimpleColumn *c2 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSCONSTRAINTCOL_TABLE + "." + CONSTRAINTNAME_COL, fSessionID);
        SimpleColumn *c3 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSCONSTRAINTCOL_TABLE + "." + SCHEMA_COL, fSessionID);
        SimpleColumn *c4 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSCONSTRAINTCOL_TABLE + "." + TABLENAME_COL, fSessionID);
    ...
    const erydbSystemCatalog::ROPair erydbSystemCatalog::indexRID(const IndexName& indexName) {
    ...
    const int erydbSystemCatalog::colNumbers(const TableName& tableName) {
...
#if 0
    const std::string erydbSystemCatalog::colValueSysindex(const TableColName& tableColName) {
        ...
        SimpleColumn *c2 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSINDEXCOL_TABLE + "." + SCHEMA_COL, fSessionID);
        SimpleColumn *c3 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSINDEXCOL_TABLE + "." + TABLENAME_COL, fSessionID);
        SimpleColumn *c4 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSINDEXCOL_TABLE + "." + COLNAME_COL, fSessionID);
        ...
        for (it = sysDataList.begin(); it != sysDataList.end(); it++) {
            if ((*it)->dataCount() == 0) {
                string msg("erydbSystemCatalog::colValueSysindex: no indexname found for ");
    ...
    const erydbSystemCatalog::RIDList erydbSystemCatalog::indexColRID(const IndexName& indexName) {
        /* SQL statement: select indexname from sysindexcol where schema=tableColName.schema and
         * tablename=tableColName.table and columnname = tableColName.column;
         */
        IndexName aIndexName;
    ...
    const erydbSystemCatalog::ROPair erydbSystemCatalog::indexColRID(const TableColName& tableColName) {
        /* SQL statement: select indexname from sysindexcol where schema=tableColName.schema and
         * tablename=tableColName.table and columnname = tableColName.column;
         */
        TableColName aTableColName;
    ...
    const erydbSystemCatalog::IndexNameList erydbSystemCatalog::colValueSysindexCol(const TableColName& tableColName) {
        /* SQL statement: select indexname from sysindex where schema = schema and table=table and column=column;*/
        TableColName aTableColName;
        erydbSystemCatalog::IndexNameList indexNameList;
        aTableColName.schema = tableColName.schema;
        aTableColName.table = tableColName.table;
        aTableColName.column = tableColName.column;
        transform(aTableColName.schema.begin(), aTableColName.schema.end(), aTableColName.schema.begin(), to_lower());
    ...
    const erydbSystemCatalog::TableName erydbSystemCatalog::lookupTableForIndex(const std::string indexName, const std::string schema) {
        ...
        SimpleColumn *c1 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSINDEX_TABLE + "." + SCHEMA_COL, fSessionID);
        SimpleColumn *c2 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSINDEX_TABLE + "." + TABLENAME_COL, fSessionID);
        SimpleColumn *c3 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSINDEX_TABLE + "." + INDEXNAME_COL, fSessionID);
        ...
        for (it = sysDataList.begin(); it != sysDataList.end(); it++) {
            if ((*it)->dataCount() == 0) {
                string msg("erydbSystemCatalog::lookupTableForIndex: no table name found for ");
                msg += aIndexName;
                throw runtime_error(msg);
            }
            if ((*it)->ColumnOID() == oid2)
    ...
    const erydbSystemCatalog::IndexOID erydbSystemCatalog::lookupIndexNbr(const IndexName& indexName) {
        erydbSystemCatalog::IndexOID indexoid = { -1, -1 };
        IndexName aIndexName;
        aIndexName.schema = indexName.schema;
        aIndexName.table = indexName.table;
        aIndexName.index = indexName.index;
        ...
        SimpleColumn *c1 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSINDEX_TABLE + "." + LISTOBJID_COL, fSessionID);
        SimpleColumn *c2 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSINDEX_TABLE + "." + TREEOBJID_COL, fSessionID);
        SimpleColumn *c3 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSINDEX_TABLE + "." + SCHEMA_COL, fSessionID);
        SimpleColumn *c4 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSINDEX_TABLE + "." + TABLENAME_COL, fSessionID);
        SimpleColumn *c5 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSINDEX_TABLE + "." + INDEXNAME_COL, fSessionID);
        SimpleColumn *c6 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSINDEX_TABLE + "." + MULTICOLFLAG_COL, fSessionID);

        SRCP srcp;
        srcp.reset(c1);
        colMap.insert(CMVT_(ERYDB_SCHEMA + "." + SYSINDEX_TABLE + "." + LISTOBJID_COL, srcp));
        srcp.reset(c2);
        colMap.insert(CMVT_(ERYDB_SCHEMA + "." + SYSINDEX_TABLE + "." + TREEOBJID_COL, srcp));
        srcp.reset(c3);
        colMap.insert(CMVT_(ERYDB_SCHEMA + "." + SYSINDEX_TABLE + "." + SCHEMA_COL, srcp));
        srcp.reset(c4);
        colMap.insert(CMVT_(ERYDB_SCHEMA + "." + SYSINDEX_TABLE + "." + TABLENAME_COL, srcp));
        ...
        for (it = sysDataList.begin(); it != sysDataList.end(); it++) {
            if ((*it)->dataCount() == 0) {
                string msg("erydbSystemCatalog::lookupIndexNbr: no indexid found for ");
    ...
    const erydbSystemCatalog::IndexOID erydbSystemCatalog::lookupIndexNbr(const TableColName& tableColName) {
        /*SQL statement: select indexname from sysindexcol where schema=tableColName.schema and
         *  tablename=tableColName.table and columnname=tableColName.column;
    ...
    const erydbSystemCatalog::IndexOIDList erydbSystemCatalog::indexOIDs(const TableName& tableName) {
    ...
    const erydbSystemCatalog::DictOIDList erydbSystemCatalog::dictOIDs(const TableName& tableName) {
        ...
        TableColName aTableName;
        erydbSystemCatalog::DictOID dictoid;
        ...
        SimpleColumn *c1 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSCOLUMN_TABLE + "." + LISTOBJID_COL, fSessionID);
        SimpleColumn *c2 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSCOLUMN_TABLE + "." + TREEOBJID_COL, fSessionID);
        SimpleColumn *c3 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSCOLUMN_TABLE + "." + SCHEMA_COL, fSessionID);
        SimpleColumn *c4 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSCOLUMN_TABLE + "." + TABLENAME_COL, fSessionID);
        SimpleColumn *c5 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSCOLUMN_TABLE + "." + DICTOID_COL, fSessionID);

        SRCP srcp;
        srcp.reset(c1);
        colMap.insert(CMVT_(ERYDB_SCHEMA + "." + SYSCOLUMN_TABLE + "." + LISTOBJID_COL, srcp));
        srcp.reset(c2);
        colMap.insert(CMVT_(ERYDB_SCHEMA + "." + SYSCOLUMN_TABLE + "." + TREEOBJID_COL, srcp));
        srcp.reset(c3);
        colMap.insert(CMVT_(ERYDB_SCHEMA + "." + SYSCOLUMN_TABLE + "." + SCHEMA_COL, srcp));
        srcp.reset(c4);
        colMap.insert(CMVT_(ERYDB_SCHEMA + "." + SYSCOLUMN_TABLE + "." + TABLENAME_COL, srcp));
...
#if 0 //Not implemented
    void erydbSystemCatalog::storeColOID(void) {
    }

    void erydbSystemCatalog::storeDictOID(void) {
    }

    void erydbSystemCatalog::storeIndexOID(void) {
    }

    void erydbSystemCatalog::updateColInfo(void) {
    }
#endif

    const int erydbSystemCatalog::colPosition(const OID& oid) {
        DEBUG << "Enter colPosition: " << oid << endl;
        ColType col = colType(oid);
        return col.colPosition;
    }

    const erydbSystemCatalog::TableInfo erydbSystemCatalog::tableInfo(const TableName& tb) {
        TableName aTableName;
        ...
        } catch (logging::ERYDBExcept& noTable) {
...
#if 0
    const erydbSystemCatalog::ConstraintInfo erydbSystemCatalog::constraintInfo(const IndexName& indexName) {
        ...
        SimpleColumn *c2 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSCONSTRAINT_TABLE + "." + SCHEMA_COL, fSessionID);
        SimpleColumn *c3 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSCONSTRAINT_TABLE + "." + TABLENAME_COL, fSessionID);
        SimpleColumn *c4 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSCONSTRAINT_TABLE + "." + CONSTRAINTTYPE_COL, fSessionID);
        SimpleColumn *c5 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSCONSTRAINT_TABLE + "." + CONSTRAINTTEXT_COL, fSessionID);
        SimpleColumn *c6 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSCONSTRAINT_TABLE + "." + REFERENCEDSCHEMA_COL, fSessionID);
        SimpleColumn *c7 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSCONSTRAINT_TABLE + "." + REFERENCEDTABLENAME_COL, fSessionID);
        SimpleColumn *c8 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSCONSTRAINT_TABLE + "." + REFERENCEDCONSTRAINTNAME_COL, fSessionID);
        SimpleColumn *c9 = new SimpleColumn(ERYDB_SCHEMA + "." + SYSCONSTRAINT_TABLE + "." + CONSTRAINTSTATUS_COL, fSessionID);
        ...
        colMap.insert(CMVT_(ERYDB_SCHEMA + "." + SYSCONSTRAINT_TABLE + "." + TABLENAME_COL, srcp));
        srcp.reset(c4);
        colMap.insert(CMVT_(ERYDB_SCHEMA + "." + SYSCONSTRAINT_TABLE + "." + CONSTRAINTTYPE_COL, srcp));
        srcp.reset(c5);
        colMap.insert(CMVT_(ERYDB_SCHEMA + "." + SYSCONSTRAINT_TABLE + "." + CONSTRAINTTEXT_COL, srcp));
        srcp.reset(c6);
        colMap.insert(CMVT_(ERYDB_SCHEMA + "." + SYSCONSTRAINT_TABLE + "." + REFERENCEDSCHEMA_COL, srcp));
        srcp.reset(c7);
        colMap.insert(CMVT_(ERYDB_SCHEMA + "." + SYSCONSTRAINT_TABLE + "." + REFERENCEDTABLENAME_COL, srcp));
        srcp.reset(c8);
        colMap.insert(CMVT_(ERYDB_SCHEMA + "." + SYSCONSTRAINT_TABLE + "." + REFERENCEDCONSTRAINTNAME_COL, srcp));
        srcp.reset(c9);
        colMap.insert(CMVT_(ERYDB_SCHEMA + "." + SYSCONSTRAINT_TABLE + "." + CONSTRAINTSTATUS_COL, srcp));
        csep.columnMapNonStatic(colMap);
        ...
        for (it = sysDataList.begin(); it != sysDataList.end(); it++) {
            if ((*it)->dataCount() == 0) {
                string msg("erydbSystemCatalog::constraintInfo: no constraint info found for ");
                msg += indexName.schema;
                msg += ".";
                msg += indexName.table;
                msg += ".";
    ...
    const erydbSystemCatalog::IndexNameList erydbSystemCatalog::referenceConstraints(const IndexName& referencePKName) {
    ...
    const string erydbSystemCatalog::primaryKeyName(const TableName& tableName) {
        TableName aTableName;
        aTableName.schema = tableName.schema;
        aTableName.table = tableName.table;
        transform(aTableName.schema.begin(), aTableName.schema.end(), aTableName.schema.begin(), to_lower());
        ...
        if (tableName.schema.compare(ERYDB_SCHEMA) != 0)
    ...
    void erydbSystemCatalog::getSchemaInfo(const string& in_schema) {
        string schema = in_schema;
        transform(schema.begin(), schema.end(), schema.begin(), to_lower());

        if (schema == ERYDB_SCHEMA)
            return;
        else
        ...
        string nullable = ERYDB_SCHEMA + "." + SYSCOLUMN_TABLE + "." + NULLABLE_COL;
        string compressiontype = ERYDB_SCHEMA + "." + SYSCOLUMN_TABLE + "." + COMPRESSIONTYPE_COL;
        string autoinc = ERYDB_SCHEMA + "." + SYSCOLUMN_TABLE + "." + AUTOINC_COL;
        string nextval = ERYDB_SCHEMA + "." + SYSCOLUMN_TABLE + "." + NEXTVALUE_COL;
...
#if 0
    ostream& operator<<(ostream& os, const erydbSystemCatalog::TableName& rhs) {
        os << rhs.schema << '.' << rhs.table;
        return os;
    }
#endif
    const string erydbSystemCatalog::TableName::toString() const {
        string str = schema + "." + table;
        return str;
    }
    ostream& operator<<(ostream& os, const erydbSystemCatalog::TableAliasName& rhs) {
        os << rhs.schema << '.' << rhs.table << "(" << rhs.alias << "/" << rhs.view
            << ") engineType=" << (rhs.fIsEryDB ? "EryDB" : "ForeignEngine");
        return os;
    }

    ostream& operator<<(ostream& os, const erydbSystemCatalog::TableColName& rhs) {
        os << rhs.toString();
        return os;
    }

    void erydbSystemCatalog::flushCache() {
    ...
    void erydbSystemCatalog::updateColinfoCache(erydbSystemCatalog::OIDNextvalMap & oidNextvalMap) {
        boost::mutex::scoped_lock lk(fColinfomapLock);
        erydbSystemCatalog::OIDNextvalMap::const_iterator iter = oidNextvalMap.begin();
        OID oid = 0;
        long long nextVal = 0;
    ...
    void erydbSystemCatalog::buildSysColinfomap() {
        ColType aCol;
        // aCol.defaultValue = "";
        aCol.scale = 0;
        aCol.precision = 10;
    ...
    void erydbSystemCatalog::buildSysOIDmap() {
        fOIDmap[make_tcn(ERYDB_SCHEMA, SYSTABLE_TABLE, TABLENAME_COL)] = OID_SYSTABLE_TABLENAME;
        fOIDmap[make_tcn(ERYDB_SCHEMA, SYSTABLE_TABLE, SCHEMA_COL)] = OID_SYSTABLE_SCHEMA;
        fOIDmap[make_tcn(ERYDB_SCHEMA, SYSTABLE_TABLE, OBJECTID_COL)] = OID_SYSTABLE_OBJECTID;
        fOIDmap[make_tcn(ERYDB_SCHEMA, SYSTABLE_TABLE, CREATEDATE_COL)] = OID_SYSTABLE_CREATEDATE;
        fOIDmap[make_tcn(ERYDB_SCHEMA, SYSTABLE_TABLE, LASTUPDATE_COL)] = OID_SYSTABLE_LASTUPDATE;
        fOIDmap[make_tcn(ERYDB_SCHEMA, SYSTABLE_TABLE, INIT_COL)] = OID_SYSTABLE_INIT;
        fOIDmap[make_tcn(ERYDB_SCHEMA, SYSTABLE_TABLE, NEXT_COL)] = OID_SYSTABLE_NEXT;
        fOIDmap[make_tcn(ERYDB_SCHEMA, SYSTABLE_TABLE, NUMOFROWS_COL)] = OID_SYSTABLE_NUMOFROWS;
        fOIDmap[make_tcn(ERYDB_SCHEMA, SYSTABLE_TABLE, AVGROWLEN_COL)] = OID_SYSTABLE_AVGROWLEN;
        fOIDmap[make_tcn(ERYDB_SCHEMA, SYSTABLE_TABLE, NUMOFBLOCKS_COL)] = OID_SYSTABLE_NUMOFBLOCKS;
        fOIDmap[make_tcn(ERYDB_SCHEMA, SYSTABLE_TABLE, AUTOINC_COL)] = OID_SYSTABLE_AUTOINCREMENT;
        fOIDmap[make_tcn(ERYDB_SCHEMA, SYSCOLUMN_TABLE, SCHEMA_COL)] = OID_SYSCOLUMN_SCHEMA;
        fOIDmap[make_tcn(ERYDB_SCHEMA, SYSCOLUMN_TABLE, TABLENAME_COL)] = OID_SYSCOLUMN_TABLENAME;
        fOIDmap[make_tcn(ERYDB_SCHEMA, SYSCOLUMN_TABLE, COLNAME_COL)] = OID_SYSCOLUMN_COLNAME;
        fOIDmap[make_tcn(ERYDB_SCHEMA, SYSCOLUMN_TABLE, OBJECTID_COL)] = OID_SYSCOLUMN_OBJECTID;
        fOIDmap[make_tcn(ERYDB_SCHEMA, SYSCOLUMN_TABLE, DICTOID_COL)] = OID_SYSCOLUMN_DICTOID;
        fOIDmap[make_tcn(ERYDB_SCHEMA, SYSCOLUMN_TABLE, LISTOBJID_COL)] = OID_SYSCOLUMN_LISTOBJID;
        fOIDmap[make_tcn(ERYDB_SCHEMA, SYSCOLUMN_TABLE, TREEOBJID_COL)] = OID_SYSCOLUMN_TREEOBJID;
        fOIDmap[make_tcn(ERYDB_SCHEMA, SYSCOLUMN_TABLE, DATATYPE_COL)] = OID_SYSCOLUMN_DATATYPE;
        fOIDmap[make_tcn(ERYDB_SCHEMA, SYSCOLUMN_TABLE, COLUMNLEN_COL)] = OID_SYSCOLUMN_COLUMNLEN;
        fOIDmap[make_tcn(ERYDB_SCHEMA, SYSCOLUMN_TABLE, COLUMNPOS_COL)] = OID_SYSCOLUMN_COLUMNPOS;
        fOIDmap[make_tcn(ERYDB_SCHEMA, SYSCOLUMN_TABLE, LASTUPDATE_COL)] = OID_SYSCOLUMN_LASTUPDATE;
        fOIDmap[make_tcn(ERYDB_SCHEMA, SYSCOLUMN_TABLE, DEFAULTVAL_COL)] = OID_SYSCOLUMN_DEFAULTVAL;
        fOIDmap[make_tcn(ERYDB_SCHEMA, SYSCOLUMN_TABLE, NULLABLE_COL)] = OID_SYSCOLUMN_NULLABLE;
        fOIDmap[make_tcn(ERYDB_SCHEMA, SYSCOLUMN_TABLE, SCALE_COL)] = OID_SYSCOLUMN_SCALE;
        fOIDmap[make_tcn(ERYDB_SCHEMA, SYSCOLUMN_TABLE, PRECISION_COL)] = OID_SYSCOLUMN_PRECISION;
        fOIDmap[make_tcn(ERYDB_SCHEMA, SYSCOLUMN_TABLE, AUTOINC_COL)] = OID_SYSCOLUMN_AUTOINC;
        fOIDmap[make_tcn(ERYDB_SCHEMA, SYSCOLUMN_TABLE, DISTCOUNT_COL)] = OID_SYSCOLUMN_DISTCOUNT;
        fOIDmap[make_tcn(ERYDB_SCHEMA, SYSCOLUMN_TABLE, NULLCOUNT_COL)] = OID_SYSCOLUMN_NULLCOUNT;
        fOIDmap[make_tcn(ERYDB_SCHEMA, SYSCOLUMN_TABLE, MINVALUE_COL)] = OID_SYSCOLUMN_MINVALUE;
        fOIDmap[make_tcn(ERYDB_SCHEMA, SYSCOLUMN_TABLE, MAXVALUE_COL)] = OID_SYSCOLUMN_MAXVALUE;
        fOIDmap[make_tcn(ERYDB_SCHEMA, SYSCOLUMN_TABLE, COMPRESSIONTYPE_COL)] = OID_SYSCOLUMN_COMPRESSIONTYPE;
        fOIDmap[make_tcn(ERYDB_SCHEMA, SYSCOLUMN_TABLE, NEXTVALUE_COL)] = OID_SYSCOLUMN_NEXTVALUE;
    }

    void erydbSystemCatalog::buildSysTablemap() {
        fTablemap[make_table(ERYDB_SCHEMA, SYSTABLE_TABLE)] = SYSTABLE_BASE;
        fTablemap[make_table(ERYDB_SCHEMA, SYSCOLUMN_TABLE)] = SYSCOLUMN_BASE;
    }

    void erydbSystemCatalog::buildSysDctmap() {
    ...
    void erydbSystemCatalog::checkSysCatVer() {
    ...
    const string erydbSystemCatalog::ColType::toString() const {
    ...
    /*static*/
    uint32_t erydbSystemCatalog::erydb_tid2sid(const uint32_t tid) {
    ...
    ostream& operator<<(ostream& output, const erydbSystemCatalog::ColType& rhs) {
        output << rhs.toString();
        return output;
    }

    vector<erydbSystemCatalog::OID> getAllSysCatOIDs() {
        vector<erydbSystemCatalog::OID> ret;
