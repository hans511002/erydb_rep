...
/***********************************************************************
 *   $Id: erydbsystemcatalog.h 9474 2013-05-02 15:28:09Z rdempsey $
 *
 *
 ***********************************************************************/
/** @file */

#ifndef EXECPLAN_ERYDBSYSTEMCATALOG_H
#define EXECPLAN_ERYDBSYSTEMCATALOG_H
...
// This is now set in the erydb.xml file
// Use, e.g., 0x500 for uid 500 so it is easy to spot in ipcs list
//const int32_t BRM_UID = 0x0;

namespace execplan
{
class erydbSelectExecutionPlan;
class ClientRotator;
class SessionManager;

/** MySQL $VTABLE ID */
const int32_t CNX_VTABLE_ID=100;
const int32_t ERYDB_VTABLE_ID=CNX_VTABLE_ID;

/** The erydbSystemCatalog class
 *
...
public:
	...
	/** static erydbsystemcatalog instance map. one instance per session
	 *  TODO: should be one per transaction
	 */
	typedef std::map <uint32_t, boost::shared_ptr<erydbSystemCatalog> > CatalogMap;
	...
	/** the structure for liberymysql to use. Alias is taken account to the identity of tables.
	 *
	 */
	struct TableAliasName
	{
		TableAliasName ():fIsEryDB (true) {}
		TableAliasName (std::string sch, std::string tb, std::string al) :
					schema (sch), table (tb), alias (al), fIsEryDB(true) {}
		TableAliasName (std::string sch, std::string tb, std::string al, std::string v) :
                                        schema (sch), table (tb), alias (al), view(v), fIsEryDB(true) {}
		std::string schema;
		std::string table;
		std::string alias;
		std::string view;
	...
	/** the type returned by makeerydbSystemCatalog()
	 *
	 */
	typedef boost::shared_ptr<erydbSystemCatalog> SPCSC;
	...
	*/
	 ...
	 */
	static SPCSC makeerydbSystemCatalog(uint32_t sessionID = 0);

	/** remove and delete the instance map to the sessionid
	 *  @param sessionID
	 */
	static void removeerydbSystemCatalog(uint32_t sessionID = 0);
	...
	/** Convert a MySQL thread id to an EryDB session id */
	static uint32_t erydb_tid2sid(const uint32_t tid);
...
private:
	/** Constuctors */
	explicit erydbSystemCatalog();
	explicit erydbSystemCatalog(const erydbSystemCatalog& rhs);


	erydbSystemCatalog& operator=(const erydbSystemCatalog& rhs);

	/** get system data */
	void getSysData (execplan::erydbSelectExecutionPlan&, NJLSysDataList&, const std::string& sysTableName);
	/** get system data for Front End */
	void getSysData_FE(const execplan::erydbSelectExecutionPlan&, NJLSysDataList&, const std::string& sysTableName);
	/** get system data for Engine Controller */
	void getSysData_EC(execplan::erydbSelectExecutionPlan&, NJLSysDataList&, const std::string& sysTableName);
 ...
 */
const erydbSystemCatalog::TableColName make_tcn(const std::string& s, const std::string& t, const std::string& c);

/** convenience function to make a TableName from 2 strings
 */
const erydbSystemCatalog::TableName make_table(const std::string& s, const std::string& t);
const erydbSystemCatalog::TableAliasName make_aliastable(const std::string& s, const std::string& t, const std::string& a,
						 const bool fIsEryDB = true);
const erydbSystemCatalog::TableAliasName make_aliasview(const std::string& s, const std::string& t, const std::string& a, const std::string& v, const bool fIsEryDB = true);

/** convenience function to determine if column type is a char
 *  type
 */
inline bool isCharType(const execplan::erydbSystemCatalog::ColDataType type)
{
	return (execplan::erydbSystemCatalog::VARCHAR == type || execplan::erydbSystemCatalog::CHAR == type);
}

/** convenience function to determine if column type is an
 *  unsigned type
 */
inline bool isUnsigned(const execplan::erydbSystemCatalog::ColDataType type)
{
	switch (type)
	{
...
inline bool isSignedInteger(const execplan::erydbSystemCatalog::ColDataType type)
{
	switch (type)
	{
...
inline bool isNull(int64_t val, const execplan::erydbSystemCatalog::ColType& ct)
{
	bool ret = false;
 ...
 */
const std::string ERYDB_SCHEMA = "erydbsys";
...
const std::string NEXTVALUE_COL = "nextvalue";
   ...
   ******************************************************/
const int   SYSTABLE_BASE = MAX_DBROOT + 1;		  /** @brief SYSTABLE table base */
const int   SYSTABLE_DICT_BASE	   = SYSTABLE_BASE + 100;		  /** @brief SYSTABLE table dictionary files base */
const int   SYSCOLUMN_BASE		   = SYSTABLE_BASE + 150;		  /** @brief SYSCOLUMN table base */
const int   SYSCOLUMN_DICT_BASE	  = SYSTABLE_BASE + 250;		  /** @brief SYSCOLUMN table dictionary files base */
...
std::vector<erydbSystemCatalog::OID> getAllSysCatOIDs();

/**
* stream operator
*/
std::ostream& operator<<(std::ostream& os, const erydbSystemCatalog::ColType& rhs);

const std::string colDataTypeToString(erydbSystemCatalog::ColDataType cdt);

} //namespace execplan

#endif											//EXECPLAN_ERYDBSYSTEMCATALOG_H
// vim:ts=4 sw=4:
