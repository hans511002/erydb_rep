...
const char *OERYDBitmapFilename = NULL;
int maxNewTxns=1000;
...
unlink("/tmp/erydbSessionMonitorShm");
//CPPUNIT_TEST( MonitorTestPlan_1 );
//CPPUNIT_TEST( MonitorTestPlan_1 );
unlink("/tmp/erydbSessionMonitorShm");
CPPUNIT_TEST( objectIDManager_1 );
if (OERYDBitmapFilename != NULL)
	unlink(OERYDBitmapFilename);
...
public:
    ...
    void setUp() {
    	  csc = erydbSystemCatalog::makeerydbSystemCatalog(0);
	...
	void objectIDManager_1()
	{
		...
		// fake out the objmgr...
		setenv("ERYDB_CONFIG_FILE", "/usr/local/erydb/etc/erydb.xml", 1);
		Config* cf = Config::makeConfig();
		cf->setConfig("OIDManager", "OERYDBitmapFile", "./oerydbitmap");
		
		try {
			ObjectIDManager o;
			
			OERYDBitmapFilename = strdup(o.getFilename().c_str());
			oidBase = o.allocOID();
			oid = o.allocOID();
			...
			if (OERYDBitmapFilename != NULL)
				unlink(OERYDBitmapFilename);   // XXXPAT: fix this when libstdc++ regains its sanity
			throw;
		}
		unlink(OERYDBitmapFilename);
	}
