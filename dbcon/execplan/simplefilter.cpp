...
const string SimpleFilter::data() const
{
	...
	if (dynamic_cast<ConstantColumn*>(fRhs) &&
		  (fRhs->resultType().colDataType == erydbSystemCatalog::VARCHAR ||
		   fRhs->resultType().colDataType == erydbSystemCatalog::CHAR ||
		   fRhs->resultType().colDataType == erydbSystemCatalog::VARBINARY ||
		   fRhs->resultType().colDataType == erydbSystemCatalog::DATE ||
		   fRhs->resultType().colDataType == erydbSystemCatalog::DATETIME))
		rhs = "'" + boost::replace_all_copy(fRhs->data(), "'", "\\'") + "'";
	else
		rhs = fRhs->data();
	if (dynamic_cast<ConstantColumn*>(fLhs) &&
		  (fLhs->resultType().colDataType == erydbSystemCatalog::VARCHAR ||
		   fLhs->resultType().colDataType == erydbSystemCatalog::CHAR ||
		   fLhs->resultType().colDataType == erydbSystemCatalog::VARBINARY ||
		   fLhs->resultType().colDataType == erydbSystemCatalog::DATE ||
		   fLhs->resultType().colDataType == erydbSystemCatalog::DATETIME))
...
void SimpleFilter::convertConstant()
{
	...
	if (lcc)
	{
		...
		if (fRhs->resultType().colDataType == erydbSystemCatalog::DATE)
		{
		...
		else if (fRhs->resultType().colDataType == erydbSystemCatalog::DATETIME)
		{
			if (lcc->constval().empty())
			{
	...
	if (rcc)
	{
		...
		if (fLhs->resultType().colDataType == erydbSystemCatalog::DATE)
		{
		...
		else if (fLhs->resultType().colDataType == erydbSystemCatalog::DATETIME)
		{
			if (rcc->constval().empty())
			{
...
void SimpleFilter::replaceRealCol(erydbSelectExecutionPlan::ReturnedColumnList& derivedColList)
{
