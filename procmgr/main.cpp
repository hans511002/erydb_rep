...
int main(int argc, char **argv) {
    ...
    } else { //running active after startup
...
static void startMgrProcessThread() {
    ...
    try {
        ...
        log.writeLog(__LINE__, "ERROR: Problem getting systemStartupOffline from the erydb System Configuration file", LOG_TYPE_ERROR);
        systemStartupOffline = "n";
    }

    if (systemStartupOffline == "y")
        log.writeLog(__LINE__, "SystemStartupOffline set to 'y', Not starting up erydb Database Processes", LOG_TYPE_INFO);
    ...
    for (; k < 1200; k++) {
        ...
        for (unsigned int i = 0; i < systemmoduletypeconfig.moduletypeconfig.size(); i++) {
            ...
            for (; pt != systemmoduletypeconfig.moduletypeconfig[i].ModuleNetworkList.end(); pt++) {
                ...
                if (localSoftwareInfo != moduleSoftwareInfo) {
                    // module not running on same erydb Software build as this local Director
        ...
        if (status == API_SUCCESS) {
            if (systemStartupOffline == "y") {
                processManager.setSystemState(oam::MAN_OFFLINE);
                log.writeLog(__LINE__, "SystemStartupOffline set to 'y', Not starting up erydb Database Processes", LOG_TYPE_DEBUG);
            }
            break;
        } else {
                            ...
                            if (moduleInfoList[moduleName] >= ModuleHeartbeatCount ||
                                ...
                                for (; retry < ModuleProcMonWaitCount; retry++) {
                                    ...
                                    for (int id = 1; id < AmazonElasticIPCount + 1; id++) {
                                        ...
                                        if (ELmoduleName == moduleName) {	//match found assign Elastic IP Address
                                ...
                                if (retry < ModuleProcMonWaitCount) {	// module successfully started
                                    ...
                                    //clear count
                                    moduleInfoList[moduleName] = 0;
                                } else {	// module failed to restart, place back in disabled state
