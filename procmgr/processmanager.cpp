...
namespace processmanager {
                ...
                switch (actionType) {
                    ...
                    case SHUTDOWNSYSTEM:
                    {
                        ...
                        if (HDFS) {
                            ...
                            string cmd = "pdsh -a -x " + localHostName + " '" + startup::StartUp::installDir() + "/erydb stop' > /dev/null 2>&1";
                            system(cmd.c_str());
                    ...
                    case RESTARTSYSTEM:
                    {
                        ...
                        if (status == API_SUCCESS) {
                            ...
                            if (status == 0 && ackIndicator) {
                                // BUG 4554 We don't need the join because erydb console is now looking for "Active"
    ...
    ******************************************************************************************/
    int ProcessManager::disableModule(string target, bool manualFlag) {
        ...
        //Update DBRM section of erydb.xml
    ...
    ******************************************************************************************/
    void ProcessManager::setProcessStates(std::string moduleName, uint16_t state, std::string processNameSkip) {
        ...
        for (itor = systemprocessconfig.processconfig.begin();
            ...
            } else {	//for for umwithpm apps, which is ExeMgr now
    ...
    ******************************************************************************************/
    int ProcessManager::addModule(oam::DeviceNetworkList devicenetworklist, std::string password, bool manualFlag) {
        ...
        if (packageType == "rpm")
            erydbPackage = homedir + "/erydb*" + systemsoftware.Version + "-" + systemsoftware.Release + "*.rpm.tar.gz";
        else
            if (packageType == "deb")
                erydbPackage = homedir + "/erydb*" + systemsoftware.Version + "-" + systemsoftware.Release + "*.deb.tar.gz";
            else
                erydbPackage = homedir + "/erydb*" + systemsoftware.Version + "-" + systemsoftware.Release + "*.bin.tar.gz";

        string cmd = "ls " + erydbPackage + " > /dev/null 2>&1";
        int rtnCode = system(cmd.c_str());
        if (WEXITSTATUS(rtnCode) != 0) {
            log.writeLog(__LINE__, "addModule - ERROR: Package not found: " + erydbPackage, LOG_TYPE_ERROR);
            pthread_mutex_unlock(&THREAD_LOCK);
            return API_FILE_OPEN_ERROR;
        }
        log.writeLog(__LINE__, "addModule - Erydb Package found:" + erydbPackage, LOG_TYPE_DEBUG);

        //
        // Verify Host IP and Password
        //

        if (password == "ssh" && amazon) {	// check if there is a root password stored
            ...
            for (; pt1 != (*listPT).hostConfigList.end(); pt1++) {
                ...
                if ((cloud == "amazon-ec2" && hostName == oam::UnassignedName) ||
                    ...
                    if (moduleType == "um") {
                        ...
                        if (UMStorageType == "external") {	//check if volume already assigned or need to create a new one
                            ...
                            } else { // one exist, detach and reattach it
        ...
        if (moduleType == "um" ||
            ...
            for (; listPT != devicenetworklist.end(); listPT++) {
                ...
                if (PMwithUM == "y") { // then go check for next available ID
                    exemgrID = 0;
                    for (int id = 2; ; id++) {
        ...
        //write erydb.xml Module section
        try {
        ...
        for (; listPT != devicenetworklist.end(); listPT++) {
            ...
            if (remoteModuleType == "um" ||
                ...
                if (packageType != "binary") {
                    ...
                    string cmd = installDir + "/bin/binary_installer.sh " + remoteModuleName + " " + remoteModuleIP + " " + password + " " + erydbPackage + " " + remoteModuleType + " initial " + binservertype + " " + MySQLPort + " 1 " + binaryInstallDir + " > /tmp/binary_installer.log";
        ...
        for (; listPT != devicenetworklist.end(); listPT++) {
            ...
            string cmd = installDir + "/bin/remote_command.sh " + remoteModuleIP + " " + password + " '" + installDir + "/bin/erydb restart;" + installDir + "/mysql/mysqld-erydb restart' 0";
            system(cmd.c_str());
            log.writeLog(__LINE__, "addModule - restart erydb service " + remoteModuleName, LOG_TYPE_DEBUG);
            ...
            if (amazon) {
                ...
                for (int id = 1; id < AmazonElasticIPCount + 1; id++) {
                    ...
                    if (ELmoduleName == remoteModuleName) {	//match found assign Elastic IP Address
            ...
            for (; listPT != devicenetworklist.end(); listPT++) {
                ...
                for (int id = 1; ; id++) {
                    ...
                    try {
                        ...
                        if (moduleName == (*listPT).DeviceName) { // match
                            sysConfig->setConfig(Section, "IPAddr", oam::UnassignedName);
        ...
        try {
            ...
            if (timeout > 0) {
                ...
                while (true) {
                    ...
                    } else {	//api timeout occurred, check if retry should be done
                        // get current time in seconds
                        time_t endTimeSec;
                        time(&endTimeSec);
                                    ...
                                    try {
                                        ...
                                        for (unsigned int j = 0; j < systemprocessconfig.processconfig.size(); j++) {
                                            if (systemprocessconfig.processconfig[j].ModuleType == moduletype &&
                                                ...
                                                } else { // if found ACTIVE, skip to next process
                                                    if (state == oam::ACTIVE)
                                                        return;
                                                }
                                            }
    ...
    ******************************************************************************************/
    int ProcessManager::updateWorkerNodeconfig() {
        ...
        erydbassert(pt0 != moduleconfig.hostConfigList.end());
    ...
    *
    * purpose:	update Extent Map section in erydb.xml
    *
    ******************************************************************************************/
    bool ProcessManager::updateExtentMap() {
        string fileName = startup::StartUp::installDir() + "/etc/erydb.xml";
    ...
    ******************************************************************************************/
    bool ProcessManager::makeXMInittab(std::string moduleName, std::string systemID, std::string parentOAMModuleHostName) {
        string fileName = startup::StartUp::installDir() + "/local/etc/" + moduleName + "/inittab.erydb";
    ...
    * @brief	setPMProcIPs
    *
    * purpose:	Updates the erydb.xml file for DDL/DMLProc IPs during PM switchover
    *
    *
    ******************************************************************************************/
    ...
    *
    * purpose:	Distribute Erydb Config File to system modules
    *
    ******************************************************************************************/
    ...
    ******************************************************************************************/
    int ProcessManager::getDBRMData(messageqcpp::IOSocket fIos, std::string moduleName) {
        ...
        string oidFile;
        oam.getSystemConfig("OERYDBitmapFile", oidFile);

        string currentDbrmFile;
        ifstream oldFile(currentFileName.c_str());
        if (oldFile) {
            // current file found, check for OERYDBitmapFile
            ifstream mapFile(oidFile.c_str());
            if (!mapFile) {
                // no OERYDBitmapFile, with current file, dbrm files are hosed
                log.writeLog(__LINE__, "getDBRMData: DBRM data files error, current file exist without OERYDBitmapFile", LOG_TYPE_CRITICAL);
                pthread_mutex_unlock(&THREAD_LOCK);
    ...
    ******************************************************************************************/
    int ProcessManager::switchParentOAMModule(std::string newActiveModuleName) {
        ...
        try {
            ...
            log.writeLog(__LINE__, "erydb.xml entries update to local IP address of " + newActiveIPaddr, LOG_TYPE_DEBUG);
        ...
        //DOING THIS JUST TO UPDATE THE TIMESTAMP OF THE ERYDB.XML FILE AS A WORK-AROUND FIX 
    ...
    ******************************************************************************************/
    int ProcessManager::OAMParentModuleChange() {
        ...
        while (!failover) {
            ...
            if (!failover) {
                ...
                if (amazon && AmazonPMFailover == "n") {
                    ...
                    if (currentIPAddr == "stopped") { // start instance
                        ...
                        for (; retry < retryCount; retry++) {
                            if (oam.startEC2Instance(downOAMParentHostname)) {
                                ...
                                if (currentIPAddr == "stopped" || currentIPAddr == "terminated") {
                                    ...
                                    string cmd = "sed -i s/" + downOAMParentIPAddress + "/" + currentIPAddr + "/g " + startup::StartUp::installDir() + "/etc/erydb.xml";
                                    system(cmd.c_str());
        ...
        try {
            ...
            log.writeLog(__LINE__, "erydb.xml Standby OAM updated : " + newStandbyModule + ":" + standbyIPaddr, LOG_TYPE_DEBUG);
            log.writeLog(__LINE__, "erydb.xml entries update to local IP address of " + localIPaddr, LOG_TYPE_DEBUG);
        } catch (exception& ex) {
            pthread_mutex_unlock(&THREAD_LOCK);
            string error = ex.what();
            log.writeLog(__LINE__, "EXCEPTION ERROR on getSystemConfig: " + error, LOG_TYPE_ERROR);
    ...
    *
    * purpose:	set Standby Module info in erydb.xml
    *
    *
    ******************************************************************************************/
    bool ProcessManager::setStandbyModule(std::string newStandbyModule, bool send) {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            try {
                ...
                log.writeLog(__LINE__, "erydb.xml Standby OAM updated to : " + newStandbyModule + ":" + standbyIPaddr, LOG_TYPE_DEBUG);
    ...
    *
    * purpose:	clear Standby Module info in erydb.xml
    *
    *
    ******************************************************************************************/
    bool ProcessManager::clearStandbyModule() {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            try {
                ...
                oam.setHotStandbyPM(" ");
                log.writeLog(__LINE__, "Clear erydb.xml Standby OAM", LOG_TYPE_DEBUG);
    ...
    *
    * purpose:	set Enable State info in erydb.xml
    *
    *
    ******************************************************************************************/
    ...
    ******************************************************************************************/
    int ProcessManager::setMySQLReplication(oam::DeviceNetworkList devicenetworklist, std::string masterModule, bool failover, bool distributeDB, std::string password, bool enable) {
        ...
        //send distubute DB
        if (distributeDB) {
            if (devicenetworklist.size() == 0) { //dist to all slaves
                ByteStream msg;
