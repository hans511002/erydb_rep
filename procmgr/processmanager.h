...
namespace processmanager{
	...
	*/
	...
	const std::string DEFAULT_LOG_FILE = "/var/log/erydb/ProcessManager.log";
...
//    void	processMSG(messageqcpp::IOSocket fIos, messageqcpp::ByteStream msg);
   ...
   // void		sendRequestToMonitor(ByteStream::byte target, ByteStream request);
	...
	/*
	* Updates the erydb.xml file for DDL/DMLProc IPs during PM switchover
	*/
	...
	*/
	...
	/** @brief set Standby Module info in erydb.xml
		*/
	bool setStandbyModule(std::string newStandbyModule, bool send = true);

	/** @brief clear Standby Module info in erydb.xml
		*/
	bool clearStandbyModule();

	int setEnableState(std::string target, std::string state);

	/** @brief Distribute erydb Config File to system modules
		*/
	int distributeConfigFile(std::string name, std::string file = "erydb.xml");
...
private:
    ...
    /**
     *@brief pdate Extent Map section in erydb.xml
     */
