...
// matter.
void OIDServer::flipOERYDBlock(int blockStart, int num, int mode) const
{
	...
	if (blockStart + num - 1 > 0x00ffffff)
		throw logic_error("flipOERYDBlock: request overruns oid space");
...
retry:
	try {
		...
		if (++i == MaxRetries)
			throw;
		cerr << "flipOERYDBlock: mem alloc failed (retrying)" << endl;
		goto retry;
	}
	oidCount = 0;
	...
	while (mask != 0 && oidCount < num) {
		if ((buf[0] & mask) != (mode ? mask : 0)) {
			delete [] buf;
			throw logic_error("flipOERYDBlock: bad allocation or deallocation attempted (1)");
		}
		if (mode)
			buf[0] &= ~mask;
		else
	...
	while (mask != 0 && oidCount < num) {
		if ((buf[byteSize-1] & mask) != (mode ? mask : 0)) {
			delete [] buf;
			if ( mode )
				throw logic_error("flipOERYDBlock: bad deallocation attempted");
			else
				throw logic_error("flipOERYDBlock: bad allocation attempted");
		}
		if (mode)
			buf[byteSize-1] &= ~mask;
		else
	...
	delete [] buf;
	throw logic_error("logic error in flipOERYDBlock detected");
}
...
countZeros:		while ((buf[i] & mask) == 0 && bitOffset < 8 && blockCount < num) {
				...
				if (blockCount == num) {   //found a match
					...
					flipOERYDBlock(blockStart, blockCount, 0);
					return blockStart;
				}
				if (bitOffset == 8)
					continue;
...
//	    "Old Reliable"
	...
	try {
		...
		if (changed)
			writeData(reinterpret_cast<uint8_t*>(freelist), 0, HeaderSize);
		flipOERYDBlock(start, end - start + 1, 1);
	}
	catch (...) {
		flock(fFd, LOCK_UN);
		throw;
