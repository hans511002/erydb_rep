...
void errString(int rc, string& errMsg)
{
	switch (rc)
	{
		...
		{
			errMsg = ERYDBErrorInfo::instance()->errorMsg(ERR_INVALID_LAST_PARTITION);
			break;
		}
		case ERR_PARTITION_DISABLED:
		{
			errMsg = ERYDBErrorInfo::instance()->errorMsg(ERR_PARTITION_ALREADY_DISABLED);
			break;
		}
		case ERR_NOT_EXIST_PARTITION:
		{
			errMsg = ERYDBErrorInfo::instance()->errorMsg(ERR_PARTITION_NOT_EXIST);
			break;
		}
		case ERR_PARTITION_ENABLED:
		{
			errMsg = ERYDBErrorInfo::instance()->errorMsg(ERR_PARTITION_ALREADY_ENABLED);
			break;
		}
...
void TableLockInfo::serialize(ERYDBDataFile* o) const
{
	uint16_t nameLen = ownerName.length();
	uint16_t dbrootListSize = dbrootList.size();
...
void TableLockInfo::deserialize(ERYDBDataFile* i)
{
	uint16_t nameLen;
	uint16_t dbrootListSize;
	boost::scoped_array<char> buf;
