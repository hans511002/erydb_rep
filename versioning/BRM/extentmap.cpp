...
unsigned extentDBRreplicateSize   = 0;
...
int ExtentMap::_markInvalid(const LBID_t lbid, const execplan::erydbSystemCatalog::ColDataType colDataType)
{
	int entries;
	int i;
	LBID_t lastBlock;
...
//------------------------------------------------------------------------------
bool ExtentMap::isValidCPRange(int64_t max, int64_t min, execplan::erydbSystemCatalog::ColDataType type) const
{
    if (isUnsigned(type))
    {
...
void ExtentMap::reserveLBIDRange(LBID_t start, uint8_t size)
{
	...
	for (i = 0; i < flEntries; i++) {
		...
		if (start == fFreeList[i].start) {
			...
			//erydbassert(size > fFreeList[i].size);
...
void ExtentMap::loadVersion4(ERYDBDataFile* in)
{
...
void ExtentMap::load(const string& filename, bool fixFL)
{
	...
	if (ERYDBPolicy::useHdfs()) {
		...
		scoped_ptr<ERYDBDataFile>  in(ERYDBDataFile::open(
									ERYDBPolicy::getType(filename_p, ERYDBPolicy::WRITEENG),
									filename_p, "r", 0));
		if (!in) {
...
void ExtentMap::save(const string& filename)
{
	...
	if (ERYDBPolicy::useHdfs()) {
		utmp = ::umask(0);
		const char* filename_p = filename.c_str();
		scoped_ptr<ERYDBDataFile> out(ERYDBDataFile::open(
									ERYDBPolicy::getType(filename_p, ERYDBPolicy::WRITEENG),
									filename_p, "wb", ERYDBDataFile::USE_VBUF));
		::umask(utmp);
		if (!out) {
...
int ExtentMap::lookupLocal_DBroot(int OID, DBROOTS_struct& dbRoot, uint32_t partitionNum, uint16_t segmentNum, uint32_t fileBlockOffset, LBID_t& LBID)
{
	...
	for (i = 0; i < entries; i++) {
		// TODO:  Blockoffset logic.
		if (fExtentMap[i].range.size != 0 && fExtentMap[i].fileID == OID && fExtentMap[i].dbRoots[0] == dbRoot[0] &&
...
// file block offset.
//------------------------------------------------------------------------------
int ExtentMap::lookupLocalStartLbid(int OID,uint32_t partitionNum,uint16_t segmentNum,uint32_t fileBlockOffset,LBID_t&  LBID)
{
#ifdef BRM_INFO
	if (fDebug)
	{
...
LBID_t ExtentMap::_createDictStoreExtent(uint32_t size, int OID, DBROOTS_struct&  dbRoot, uint32_t  partitionNum, uint16_t  segmentNum)
{
	...
	if (lastExtentIndex == -1) {
		...
		e->dbRoots[0]   = fExtentMap[lastExtentIndex].dbRoots[0];
	...
	for (int i = emEntries-1; i >= 0; i--) {
		if ((fExtentMap[i].range.size != 0) && (fExtentMap[i].fileID == OID) && (fExtentMap[i].dbRoots == dbRoot) &&
			...
			if ( (fExtentMap[i].partitionNum >  partitionNum) || ((fExtentMap[i].partitionNum == partitionNum) &&
				 ...
				 (fExtentMap[i].blockOffset  == lastExtent) && (fExtentMap[i].segmentNum   >= segmentNum)) ){
...
void ExtentMap::getDbRootHWMInfo(int OID, uint16_t pmNumber, EmDbRootHWMInfo_v& emDbRootHwmInfos)
{
	...
	for (int i = emEntries-1; i >= 0; i--) {
		if ((fExtentMap[i].range.size != 0)   && 
			...
			emIter = emDbRootMap.find( fExtentMap[i].dbRoots[0] );
...
void ExtentMap::getExtents(int OID, vector<struct EMEntry>& entries, bool sorted, bool notFoundErr, bool incOutOfService)
{
#ifdef BRM_INFO
	if (fDebug)
	{
	...
	if (incOutOfService) {
		...
		for (i = 0 ; i < emEntries; i++)
			if ((fExtentMap[i].fileID == OID) && (fExtentMap[i].range.size != 0) && (fExtentMap[i].status != EXTENTOUTOFSERVICE))
				entries.push_back(fExtentMap[i]);
	}
	releaseEMEntryTable(READ);
...
//------------------------------------------------------------------------------
void ExtentMap::getExtentCount_dbroot(int OID, uint16_t dbr, bool incOutOfService, uint64_t& numExtents)
{
	int i, emEntries;
	...
	if (incOutOfService) {
		...
		for (i = 0 ; i < emEntries; i++) {
			if ((fExtentMap[i].fileID == OID) && (fExtentMap[i].range.size != 0) && (fExtentMap[i].dbRoots[0] == dbr) && (fExtentMap[i].status != EXTENTOUTOFSERVICE))
				numExtents++;
		}
	}
	releaseEMEntryTable(READ);
...
void ExtentMap::deletePartition(const set<OID_t>& oids, const set<LogicalPartition>& partitionNums, string& emsg)
{
	...
	if (foundPartitions.size() != partitionNums.size())
	{
		...
		args.add(oss.str());
		emsg = ERYDBErrorInfo::instance()->errorMsg(ERR_PARTITION_NOT_EXIST,args);
...
void ExtentMap::markPartitionForDeletion(const set<OID_t>& oids, const set<LogicalPartition>& partitionNums, string& emsg)
{
	...
	if (foundPartitions.size() != partitionNums.size())
	{
		...
		args.add(oss.str());
		emsg = emsg + string("\n") + ERYDBErrorInfo::instance()->errorMsg(
...
void ExtentMap::restorePartition(const set<OID_t>& oids, const set<LogicalPartition>& partitionNums, string& emsg)
{
	...
	if (foundPartitions.size() != partitionNums.size())
	{
		...
		args.add(oss.str());
		emsg = ERYDBErrorInfo::instance()->errorMsg(ERR_PARTITION_NOT_EXIST, args);
		throw ERYDBExcept(emsg, ERR_PARTITION_NOT_EXIST);
	}
	...
	{
		emsg = ERYDBErrorInfo::instance()->errorMsg(ERR_PARTITION_ALREADY_ENABLED);
		throw ERYDBExcept(emsg, ERR_PARTITION_ALREADY_ENABLED);
	}
}
...
//------------------------------------------------------------------------------
void ExtentMap::deleteDBRoot(uint16_t dbr) {
#ifdef BRM_INFO
    if (fDebug) {
    ...
    for (unsigned i = 0; i < fEMShminfo->allocdSize / sizeof(struct EMEntry); i++){
        if (fExtentMap[i].range.size != 0) {
            if (fExtentMap[i].dbRoots[0] == dbr && fExtentMap[i].dbRoots[1] == 0) { // only one rep 	        
                deleteExtent(i); // 
            } else {
                fExtentMap[i].dbRoots.remove(dbr);
                //int n=-1,j=0;
                //while (++n < extentDBRreplicateSize ) {
                //    if(fExtentMap[i].dbRoots[n] == dbr){
...
//------------------------------------------------------------------------------
bool ExtentMap::isDBRootEmpty(uint16_t dbr)
{
#ifdef BRM_INFO
	if (fDebug)
	{
...
void ExtentMap::checkReloadConfig()
{
	...
	// Immediately return if erydb.xml timestamp has not changed
...
//	}
	...
	// Get latest erydb.xml timestamp after first access forced a reload
...
/*int ExtentMap::physicalPartitionNum(const set<OID_t>& oids,const set<uint32_t>& partitionNums, vector<PartitionInfo>& partitionInfos)
{
#ifdef BRM_INFO
	if (fDebug)
	{
