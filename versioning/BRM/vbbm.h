...
public:
	...
	inline void grow(unsigned key, off_t size)
		...
		{ fVBBM.grow(key, size); }
#else
		{ int rc=fVBBM.grow(key, size); erydbassert(rc==0); }
#endif
...
class VBBM : public Undoable {
	public:
		...
		EXPORT void getBlocks(int num, const DBROOTS_struct &vbOID,uint8_t dbrIndex, VBRange_v &vbRanges, VSS& vss,bool flushPMCache);
		...
		EXPORT void loadVersion1(erydbdatafile::ERYDBDataFile* in);
		EXPORT void loadVersion2(erydbdatafile::ERYDBDataFile* in);
