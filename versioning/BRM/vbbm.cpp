...
namespace BRM {
    ...
    VBBMImpl* VBBMImpl::makeVBBMImpl(unsigned key, off_t size, bool readOnly) {
        ...
        if (fInstance) {
            ...
            erydbassert(key == fInstance->fVBBM.key());
            return fInstance;
        }
    ...
    //assumes write lock
    void VBBM::getBlocks(int num, const DBROOTS_struct &vbOID,uint8_t dbrIndex, VBRange_v & freeRanges, VSS& vss, bool flushPMCache) {
        uint32_t fileIndex;
        ...
        freeRanges.clear();
        int oidIndex = 0;
        uint16_t dbr = vbOID.get(dbrIndex);
    ...
    */
    ...
    void VBBM::loadVersion2(ERYDBDataFile* in) {
        int vbbmEntries;
        int nFiles;
        int i;
        VBBMEntry entry;
