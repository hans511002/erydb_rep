...
VSSImpl* VSSImpl::makeVSSImpl(unsigned key, off_t size, bool readOnly)
{
	...
	if (fInstance)
	{
		...
		erydbassert(key == fInstance->fVSS.key());
		return fInstance;
	}
...
void VSS::growVSS()
{
	...
	erydbassert((allocSize == VSS_INITIAL_SIZE && !fPVSSImpl) || fPVSSImpl);
	if (fPVSSImpl)
	{
...
void VSS::clear()
{
	...
	erydbassert(fPVSSImpl);
	erydbassert(fPVSSImpl->key() != (unsigned)newshmkey);
