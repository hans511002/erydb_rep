...
namespace BRM {
...
#endif
    {
        ...
        if (ERYDBPolicy::useHdfs()) {
            journalh = ERYDBDataFile::open(
                ERYDBPolicy::getType(filename, ERYDBPolicy::WRITEENG), filename, "r+b", 0);
        } else {
...
#endif
    {
        ...
        release = false;
        die = false;
        firstSlave = false; // 标识是否写同步事务 全局节点同步将降低性能 
        server = NULL;
        standalone = true;
        printOnly = false;
    ...
    void SlaveComm::do_createColumnExtent_DBroot(ByteStream &msg) {
       ...
       // dbRoot = tmp16;
        ...
        segmentNum = tmp16;
        msg >> tmp8;
        colDataType = (execplan::erydbSystemCatalog::ColDataType)tmp8;
    ...
    void SlaveComm::do_markInvalid(ByteStream &msg) {
        ...
        err = slave->markExtentInvalid(lbid, (execplan::erydbSystemCatalog::ColDataType)colDataType);
        reply << (uint8_t)err;
#ifdef BRM_VERBOSE
    ...
    void SlaveComm::do_markManyExtentsInvalid(ByteStream &msg) {
        ...
        ByteStream reply;
        vector<LBID_t> lbids;
        vector<execplan::erydbSystemCatalog::ColDataType> colDataTypes;
        uint32_t size, i;
        ...
        for (i = 0; i < size; ++i) {
            ...
            colDataTypes.push_back((execplan::erydbSystemCatalog::ColDataType)colDataType);
            if (printOnly)
    ...
    void SlaveComm::do_mergeExtentsMaxMin(ByteStream &msg) {
        ...
        for (int64_t i = 0; i < mergeCount; i++) {
            ...
            msg >> tmp32;
            cpMaxMin.type = (execplan::erydbSystemCatalog::ColDataType)tmp32;
    ...
    void SlaveComm::do_confirm() {
        ...
        if (firstSlave && (takeSnapshot ||
            ...
            if (!ERYDBPolicy::useHdfs() && currentSaveFD < 0) {
                currentSaveFD = open(filename, O_WRONLY | O_CREAT, 0664);
            } else if (ERYDBPolicy::useHdfs() && !currentSaveFile) {
                currentSaveFile = ERYDBDataFile::open(
                    ERYDBPolicy::getType(filename, ERYDBPolicy::WRITEENG), filename, "wb", 0);
            }
            ...
            if (currentSaveFile) {
                ...
                delete journalh;
                journalh = ERYDBDataFile::open(
                    ERYDBPolicy::getType(filename, ERYDBPolicy::WRITEENG), filename, "w+b", 0);
                ::umask(utmp);
                if (!journalh)
                    throw runtime_error("Could not open the BRM journal for writing!");
            } else {
    ...
    int SlaveComm::replayJournal(string prefix) {
        ...
        const char* filename = journalName.c_str();
        if (ERYDBPolicy::useHdfs()) {
            ERYDBDataFile* journalf = ERYDBDataFile::open(
                ERYDBPolicy::getType(filename, ERYDBPolicy::WRITEENG), filename, "rb", 0);
            if (!journalf) {
