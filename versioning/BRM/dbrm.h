 ...
 *
 * erydb configuration file entries are necessary for the interface, Master,
...
// dmc-getExtentSize() to be deprecated, replaced by getExtentRows()
	 ...
	 * @return 0 on success, non-0 on error (see brmtypes.h)
	 */
	EXPORT int beginVBCopy(VER_t transID,const DBROOTS_struct& dbRoot,uint8_t dbrIndex, const LBIDRange_v& ranges, VBRange_v& freeList) DBRM_THROW;
	...
	/** @brief Persistence API.  Saves all BRM structures using the filenames from erydb.xml.
	 *
	 * Saves all <b>local</b> BRM structures to files.
	 *
	 * @param filename The filename prefix to use.  Saves 4 files with that prefix.
	 ...
	 * 
	 * Suspended is caused by user at erydb-console
	 ...
	 * 
	 * If the user at erydb-console asks for suspension, but ddl, 
	 ...
	 * 
	 * If the user at erydb-console asks for shutdown, but ddl, 
