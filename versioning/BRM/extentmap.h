...
#else
		{ int rc=fExtMap.grow(key, size); erydbassert(rc==0); }
#endif
...
#else
		{ int rc=fFreeList.grow(key, size); erydbassert(rc==0); }
#endif
...
public:
	...
	*/
	...
	EXPORT int markInvalid(const std::vector<LBID_t> &lbids, 
                           const std::vector<execplan::erydbSystemCatalog::ColDataType>& colDataTypes);
...
private:
	...
	int _markInvalid(const LBID_t lbid, const execplan::erydbSystemCatalog::ColDataType colDataType);

	void loadVersion4(std::ifstream &in);
	void loadVersion4(erydbdatafile::ERYDBDataFile* in);
