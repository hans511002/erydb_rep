            ...
            for( size_t i = 0; i < columns.size(); i++ ) 
            {
                ...
                if (k >= dbRootHWMInfo.size()) // logic error; should not happen
                {
                    ...
                    oss << "Error creating meta file; DBRoot" << dbRoots[m] <<
                       " listed in erydb config file, but not in extentmap"
...
std::string RBMetaWriter::openMetaFile ( uint16_t dbRoot )
{
    ...
    if( !ERYDBPolicy::exists( bulkRollbackPath.c_str() ) )
    {
...
void RBMetaWriter::renameMetaFile ( )
{
        ...
        fMetaFileNames.begin(); iter != fMetaFileNames.end(); ++iter)
    {
        ...
        if (!metaFileName.empty())
        {
            ...
            if ( ERYDBPolicy::rename(tmpMetaFileName.c_str(), metaFileName.c_str()) )
            {
                int errRc = errno;
                std::ostringstream oss;
                std::string eMsg;
...
void RBMetaWriter::createSubDir( const std::string& metaFileName )
{
    ...
    if( ERYDBPolicy::mkdir( bulkRollbackSubPath.c_str() ) != 0 )
    {
        std::ostringstream oss;
    ...
    HWM       startingHWM) // starting HWM for db segment file
{
    ...
    ERYDBFileSystem& fs = ERYDBPolicy::getFs( backupFileNameTmp.c_str() );
    ...
    std::string&         errMsg) const// error msg if error occurs
{
    ...
    ERYDBDataFile*  backupFile = ERYDBDataFile::open(
        ERYDBPolicy::getType( fileNameTmp.c_str(), ERYDBPolicy::WRITEENG ),
        fileNameTmp.c_str(),
        "w+b",
        0 );
    if (!backupFile)
    ...
    ERYDBFileSystem& fs = ERYDBPolicy::getFs( fileNameTmp.c_str() );
    ...
    backupFile->flush();
//   ERYDBDataFile flush() does a sync where appropriate
    delete backupFile;
