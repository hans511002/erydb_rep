...
namespace WriteEngine
{
  ...
  */
     ...
     *    none
     ***********************************************************/
    void FileOp::closeFile(ERYDBDataFile* pFile) const
    {
        delete pFile;
    }
    ...
    int FileOp::createDir(const char* dirName, mode_t mode) const
    {
        ...
        int rc = ERYDBPolicy::mkdir(dirName);
        if (rc != 0)
        {
            int errRc = errno;
            if (errRc == EEXIST)
     ...
     ***********************************************************/
    int FileOp::createFile(const char* fileName, int numOfBlock,uint64_t emptyVal, int width,uint16_t dbr)
    {
        ERYDBDataFile* pFile =ERYDBDataFile::open(ERYDBPolicy::getType(fileName, ERYDBPolicy::WRITEENG),fileName,"w+b",ERYDBDataFile::USE_VBUF,width);
        int rc = 0;
        if (pFile != NULL)
        { 
            ...
            {
                rc = initAbbrevCompColumnExtent(pFile, dbr, numOfBlock, emptyVal, width);
            } else
            {
                rc = initColumnExtent(pFile, dbr, numOfBlock, emptyVal, width,true, false,true);  //new file   don't expand; add new extent add abbreviated extent
            }
            closeFile(pFile);
        } else
    ...
    int FileOp::deleteFile(const char* fileName) const
    {
        ...
        return (ERYDBPolicy::remove(fileName) == -1) ? ERR_FILE_DELETE : NO_ERROR;
    }
    ...
    {
        return ERYDBPolicy::exists(fileName);
    }
     ...
     *    pFile     - ERYDBDataFile ptr to the file where the extent is added.
     ...
     ***********************************************************/
    int FileOp::initColumnExtent(ERYDBDataFile* pFile, uint16_t dbr,int nBlocks,uint64_t emptyVal,int width,bool bNewFile,bool bExpandExtent,bool bAbbrevExtent)
    {
     ...
     *    else returns NO_ERROR.
     ***********************************************************/
    int FileOp::initAbbrevCompColumnExtent(ERYDBDataFile* pFile,uint16_t dbr,int nBlocks,uint64_t emptyVal,int width)
    {
        // Reserve disk space for full abbreviated extent
        int rc = initColumnExtent(pFile,dbr,nBlocks,emptyVal,width,true,false,true); // add abbreviated extent
        if (rc != NO_ERROR)
        {
            return rc;
        }
        ...
        char hdrs[ERYDBCompressInterface::HDR_BUF_LEN * 2];
        rc = writeInitialCompColumnChunk(pFile,nBlocks,INITIAL_EXTENT_ROWS_TO_DISK,emptyVal,width,hdrs);
        if (rc != NO_ERROR)
        {
            return rc;
        }
     ...
     * PARAMETERS:
     *    pFile    - ERYDBDataFile* of column segment file to be written to
    ...
    int FileOp::writeInitialCompColumnChunk(ERYDBDataFile* pFile,int nBlocksAllocated,int nRows,uint64_t emptyVal,int width,char* hdrs)
    {
        ...
        ptrs.push_back(ERYDBCompressInterface::HDR_BUF_LEN * 2);
        ptrs.push_back(outputLen + (ERYDBCompressInterface::HDR_BUF_LEN * 2));
    ...
    int FileOp::expandAbbrevColumnChunk(ERYDBDataFile* pFile,uint64_t emptyVal,int colWidth,const CompChunkPtr& chunkInPtr,CompChunkPtr& chunkOutPtr)
    {
        ...
        //logging::Message::Args args;
        //args.add("chunkInPtr.first=");
        //args.add(chunkInPtr.first);
        //args.add("  chunkInPtr.second=");
        //args.add(chunkInPtr.second);
        //args.add("  chunkOutPtr.second=");
        //args.add((uint64_t)outputLen);
        //args.add(chunkInPtr.first);
        //args.add(chunkInPtr.first);
        //message.format(args);
     ...
     *    error code
     ***********************************************************/
    int FileOp::getFileSize(ERYDBDataFile* pFile, long long& fileSize) const
    {
        fileSize = 0;
        if (pFile == NULL)
    ...
    {
        return ERYDBPolicy::isDir(dirName);
    }
    ...
    int FileOp::oid2FileName(FID fid, char* fullFileName, bool bCreateDir, uint16_t dbr, uint32_t partition, uint16_t segment) const
    {
        ...
        if (dbr > 0)
        {
           ...
           //@Bug 5397
            if (ERYDBPolicy::exists(fullFileName))
                return NO_ERROR;
    ...
    int FileOp::oid2DirName(FID fid, char* oidDirName) const
    {
        ...
        for (unsigned i = 0; i < dbRootPathList.size(); i++)
        {
            ...
            if (ERYDBPolicy::exists(oidDirName))
                return NO_ERROR;
        }
    ...
    ERYDBDataFile* FileOp::openFile(const char* fileName,const char* mode,const int ioColSize,bool useTmpSuffix) const
    {
        ...
        unsigned opts;
        if (ioColSize > 0)
            opts = ERYDBDataFile::USE_VBUF;
        else
            opts = ERYDBDataFile::USE_NOVBUF;
        if ((useTmpSuffix) && erydbdatafile::ERYDBPolicy::useHdfs())
            opts |= ERYDBDataFile::USE_TMPFILE;
     ...
     ***********************************************************/
    int FileOp::readHeaders(ERYDBDataFile* pFile, char* hdr1, char* hdr2) const
    {
        unsigned char* hdrPtr = reinterpret_cast<unsigned char*>(hdr1);
        RETURN_ON_ERROR(setFileOffset(pFile, 0));
        RETURN_ON_ERROR(readFile(pFile, hdrPtr,
                        ERYDBCompressInterface::HDR_BUF_LEN));
     ...
     ***********************************************************/
    int FileOp::setFileOffset(ERYDBDataFile* pFile, long long offset, int origin) const
    {
        int rc;
     ...
     *    ERR_FILE_SEEK if something wrong in setting the position
     ***********************************************************/
    int FileOp::setFileOffsetBlock(ERYDBDataFile* pFile, uint64_t lbid, int origin) const
    {
     ...
     *    ERR_FILE_SEEK if something wrong in setting the position
     ***********************************************************/
    int FileOp::truncateFile(ERYDBDataFile* pFile, long long fileSize) const
    {
        if (pFile == NULL)
            return ERR_FILE_NULL;
     ...
     ***********************************************************/
    int FileOp::writeFile(ERYDBDataFile* pFile, const unsigned char* writeBuf,
                          int writeSize) const
    {
        if (pFile != NULL)
        {
    ...
    int FileOp::updateColumnExtent(ERYDBDataFile* pFile, int nBlocks)
    {
        return NO_ERROR;
    }

    int FileOp::updateDctnryExtent(ERYDBDataFile* pFile, int nBlocks)
    {
        return NO_ERROR;
    }
