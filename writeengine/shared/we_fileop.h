...
public:
    ...
    * @brief Close a file
    */
    EXPORT void         closeFile( ERYDBDataFile* pFile ) const;
    ...
    */
    EXPORT int          writeHeaders(ERYDBDataFile* pFile, const char* hdr) const;
    ...
    * ERYDBDataFile* points to start of data when function returns.
    * @param pFile (in) ERYDBDataFile* of column segment file to be read.
    * @param hdrs (out) Contents of headers that are read.
    */
    EXPORT int          readHeaders( ERYDBDataFile* pFile, char* hdrs ) const;
    EXPORT int          readHeaders( ERYDBDataFile* pFile, char* hdr1, char* hdr2 )const;

   /**
    * @brief Reinitialize a partial extent in a column segment file
    * @param pFile (in) ERYDBDataFile* of column segment file to be written to
...
protected:
    EXPORT virtual int         updateColumnExtent(ERYDBDataFile* pFile, int nBlocks);
    EXPORT virtual int         updateDctnryExtent(ERYDBDataFile* pFile, int nBlocks);
