...
/*******************************************************************************
 * DESCRIPTION:
 *    Reload local cache using contents of erydb.xml file.
 * PARAMETERS:
 *    none
 ******************************************************************************/
void Config::checkReload( )
{
...
#if 0  // common code, moved to ERYDBPolicy
    //--------------------------------------------------------------------------
    // ERYDBDataFile logging
    //--------------------------------------------------------------------------
    bool erydblog = false;
    string erydblogstr = cf->getConfig("SystemConfig", "DataFileLog");
    if ( erydblogstr.length() != 0 )
    {
    	boost::to_upper(erydblogstr);
    	erydblog = ( erydblogstr == "ON" );
    }

    //--------------------------------------------------------------------------
    // Optional File System Plugin - if a HDFS type plugin is loaded
    // then the system will use HDFS for all ERYDB data files
    //--------------------------------------------------------------------------
    string fsplugin = cf->getConfig("SystemConfig", "DataFilePlugin");
    if ( fsplugin.length() != 0 )
    {
        ERYDBPolicy::installPlugin(fsplugin);
    }
	...
	ERYDBPolicy::init( erydblog, bUseRdwrMemBuffer, hdfsRdwrScratch, hdfsRdwrBufferMaxSize );
#endif

	ERYDBPolicy::configERYDBPolicy();
