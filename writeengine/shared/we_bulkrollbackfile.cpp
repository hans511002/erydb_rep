    ...
    long long fileSizeBlocks )
{
    ...
    std::string segFile;
    ERYDBDataFile* pFile = fDbFile.openFile(columnOID, dbRoot, partNum, segNum, segFile);
    ...
    int         nBlocks )
{
    ...
    std::string segFile;
    ERYDBDataFile* pFile = fDbFile.openFile(dStoreOID, dbRoot, partNum, segNum, segFile);
    if (pFile == 0)
    {
        std::ostringstream oss;
