...
namespace WriteEngine
{
    ...
    ERYDBDataFile* BRMWrapper::m_curVBFile = NULL;
    ...
    struct fileInfoCompare // lt operator
    {
        bool operator()(const File& lhs, const File& rhs) const
        {
            ...
            if ((lhs.oid == rhs.oid) && (lhs.fDbRoot.get(0) < rhs.fDbRoot.get(0)))
            {
                return true;
            }
    ...
    typedef std::map< File, ERYDBDataFile*, fileInfoCompare > FileOpenMap;
        ...
        std::string& errMsg)
    {
        ...
        try
        {
            dbrm->startAISequence(colOID, startNextValue, colWidth, colDataType);
        ...
        std::string& errMsg)
    {
        ...
        try
        {
            ...
            bool gotFullRange = dbrm->getAIRange(colOID, count, &firstNumArg);
            if (gotFullRange)
            {
    ...
    {
        int rc = dbrm->createStripeColumnExtents(cols, dbRoot, partition, segmentNum, extents);
    ...
    int   BRMWrapper::allocateColExtentExactFile(
        ...
        uint32_t&  startBlock)
    {
        int rc = dbrm->createColumnExtentExactFile((int)oid, colWidth, dbRoot, partition, segment, colDataType, startLbid, allocSize, startBlock);
    ...
    //------------------------------------------------------------------------------
    int   BRMWrapper::allocateDictStoreExtent(const OID   oid,DBROOTS_struct&   dbRoot,uint32_t   partition,uint16_t   segment,LBID_t& startLbid,int& allocSize)
    {
        int rc = dbrm->createDictStoreExtent((int)oid, dbRoot, partition, segment, startLbid, allocSize);
    ...
    int   BRMWrapper::getBrmInfo(const OID oid, const uint32_t partition, const uint16_t segment, const int fbo, LBID_t& lbid)
    {
        ...
        int rc = dbrm->lookupLocal((int)oid, partition, segment, (uint32_t)fbo, lbid); return getRC(rc, ERR_BRM_LOOKUP_LBID);
    };
    ...
    int   BRMWrapper::getStartLbid(const OID oid, const uint32_t partition, const uint16_t segment, const int fbo, LBID_t& lbid)
    {
        ...
        int rc = dbrm->lookupLocalStartLbid((int)oid, partition, segment, (uint32_t)fbo, lbid);
    ...
    //  int rc = dbrm->lookup((uint64_t)lbid, 0, false, oid, (uint32_t&)fbo);
    ...
    int BRMWrapper::getDbRootHWMInfo(const OID oid, BRM::EmDbRootHWMInfo_v& emDbRootHwmInfos)
    {
        ...
        rc = dbrm->getDbRootHWMInfo(oid, localModuleID, emDbRootHwmInfos);
    ...
    {
        int rc = dbrm->getSystemShutdownPending(bRollback, bForce);
        if (rc < 0)
    ...
    {
        bool bRollback;
        int rc = dbrm->getSystemSuspendPending(bRollback);
        if (rc < 0)
            return ERR_BRM_GET_SUSPEND;
        else if (rc > 0)
            return ERR_BRM_SUSPEND;

        rc = dbrm->getSystemSuspended();
        if (rc < 0)
            return ERR_BRM_GET_SUSPEND;
        else if (rc > 0)
            return ERR_BRM_SUSPEND;
    ...
    //------------------------------------------------------------------------------
    int BRMWrapper::getTableLock(OID tableOid, std::string& ownerName, uint32_t&   processID, int32_t&     sessionID, int32_t&     transID, uint64_t&   lockID, std::string& errMsg)
    {
        int rc = NO_ERROR;
        lockID = 0;
        ...
        try
        {
            lockID = dbrm->getTableLock(pmList, tableOid, &ownerName, &processID, &sessionID, &transID, BRM::LOADING);
        } catch (std::exception& ex)
        {
            errMsg = ex.what();
    ...
    //------------------------------------------------------------------------------
    int BRMWrapper::changeTableLockState(uint64_t lockID, BRM::LockState lockState, bool& bChanged, std::string& errMsg)
    {
        int rc = NO_ERROR;
        bChanged = false;
        ...
        try
        {
            bChanged = dbrm->changeState(lockID, lockState);
        } catch (std::exception& ex)
        {
            errMsg = ex.what();
    ...
    int BRMWrapper::releaseTableLock(uint64_t lockID, bool& bReleased, std::string& errMsg)
    {
        ...
        try
        {
            bReleased = dbrm->releaseTableLock(lockID);
        } catch (std::exception& ex)
        {
            errMsg = ex.what();
    ...
    //------------------------------------------------------------------------------
    int BRMWrapper::getTableLockInfo(uint64_t lockID,BRM::TableLockInfo* lockInfo,bool& bLockExists,std::string& errMsg)
    {
        int rc = NO_ERROR;
        try
        {
            bLockExists = dbrm->getTableLockInfo(lockID, lockInfo);
        } catch (std::exception& ex)
        {
            errMsg = ex.what();
...
#define MAX_VERSION_BUFFER_SIZE 1024
    ...
    int BRMWrapper::copyVBBlock(ERYDBDataFile *pSourceFile, const OID sourceOid,
                                ERYDBDataFile *pTargetFile, const OID targetOid,
                                ...
                                const size_t fboCurrentOffset)
    {
        ...
        while (outputFileWritePointer < freeList.size)
        {
            ...
            {  // There was nothing in the buffer, either source list or free list is finished
                if (buffer) free(buffer);
                return 0;
            }
        }
    ...
    int BRMWrapper::copyVBBlock(ERYDBDataFile* pSourceFile, ERYDBDataFile* pTargetFile,
    ...
    {
        int rc = dbrm->vbCommit(transID);
        return getRC(rc, ERR_BRM_COMMIT);
    }

    ERYDBDataFile* BRMWrapper::openFile(const File& fileInfo, const char* mode, const bool bCache)
    {
        ERYDBDataFile* pFile;
        ...
        FileOp fileOp;
        if (fileInfo.oid <= MAX_DBROOT)
        { //Cannot have more than 999 version buffer files tp prevent oid collision
    ...
    int BRMWrapper::rollBack(const VER_t transID, int sessionId)
    {
        ...
        rc = dbrm->getUncommittedLBIDs(transID, lbidList);
        if (rc != 0)
        {
        ...
        //RETURN_ON_WE_ERROR(dbrm->getUncommittedLBIDs(transID, lbidList), ERR_BRM_GET_UNCOMM_LBID);
        ...
        try
        {
            for (i = 0; i < lbidList.size(); i++)
            {
                ...
                // get version id
                RETURN_ON_WE_ERROR(dbrm->vssLookup(lbidList[i], verID, transID, &outVer, &vbFlag, true), ERR_BRM_LOOKUP_VERSION);
                //timer.stop("vssLookup");
                // copy buffer back
                //look for the block in extentmap
                //timer.start("lookupLocalEX");
                RETURN_ON_WE_ERROR(dbrm->lookupLocal(lbidList[i], outVer, false, weOid, weDbRoot, wePartitionNum, weSegmentNum, weFbo), ERR_EXTENTMAP_LOOKUP);
                ...
                if (colType.columnOID == 0)
                {
                    ...
                    erydbassert(colType.columnOID != 0);
                    erydbassert(colType.ddn.dictOID == weOid);
                }
                erydbSystemCatalog::ColDataType colDataType = colType.colDataType;
                ColType weColType;
        ...
        //timer.start("vbRollback");
           // rc =  dbrm->vbRollback(transID, lbidRangeList);
...
cleanup:
    ...
    int BRMWrapper::rollBackBlocks(const VER_t transID, int sessionId)
    {
        ...
        ERYDBDataFile* pSourceFile = 0;
        ERYDBDataFile* pTargetFile = 0;
        ...
        for (i = 0; i < lbidList.size(); i++)
        {
            ...
            verID = dbrm->getHighestVerInVB(lbidList[i], transID);
            if (verID < 0)
            {
                std::ostringstream oss;
            ...
            //look for the block in the version buffer
            //timer.start("lookupLocalVB");
            rc = dbrm->lookupLocal(lbidList[i], verID, true, vbOid, vbDbRoot, vbPartitionNum, vbSegmentNum, vbFbo);
            if (rc != 0)
            {
                std::ostringstream oss;
                BRM::errString(rc, errorMsg);
            ...
            range.size = 1;
            lbidRangeList.push_back(range);
            rc = dbrm->dmlLockLBIDRanges(lbidRangeList, transID);
            if (rc != 0)
            {
                BRM::errString(rc, errorMsg);
                goto cleanup;
            ...
            pTargetFile->flush();
            rc = dbrm->dmlReleaseLBIDRanges(lbidRangeList);
            if (rc != 0)
            {
                BRM::errString(rc, errorMsg);
                goto cleanup;
...
cleanup:
    ...
    //	timer.start("flushChunks");
        ...
        if ((erydbdatafile::ERYDBPolicy::useHdfs()) && (files.size() > 0))
    ...
    void BRMWrapper::pruneLBIDList(VER_t transID, vector<LBIDRange> *rangeList, vector<uint32_t> *fboList) const
    {
        ...
        rc = dbrm->bulkVSSLookup(lbids, verID, transID, &vssData);
        if (rc != 0)
        ...
        */
        if (!skipBeginVBCopy)
        {
            ...
            */
            ...
            //rc = dbrm->lookupLocal(rangeList[0].start, transID, false, oid, dbRoot, partitionNum, segmentNum, fileBlockOffset);
            //if (rc != NO_ERROR)
            //	return rc;
