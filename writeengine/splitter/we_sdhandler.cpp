...
// getModuleStatus will take too long. Also to test in development
	...
	{
		ostringstream oss;
		oss << "System is not ready (" << rtn << ").  Verify that EryDB is up and ready ";
...
void WESDHandler::onBrmReport(int PmId, messageqcpp::SBS& Sbs) {
	...
	int64_t aInsRows = 0;
	int aColNum = 0;
	erydbSystemCatalog::ColDataType aColType = erydbSystemCatalog::INT;
	int aOorVal = 0;
...
int WESDHandler::getTableOID(std::string Schema, std::string Table) {
	...
	erydbSystemCatalog::TableName tableName(Schema, Table);
	boost::shared_ptr<erydbSystemCatalog> systemCatalogPtr =
			erydbSystemCatalog::makeerydbSystemCatalog();
...
unsigned int WESDHandler::calcTableRecLen(
	...
	boost::shared_ptr<erydbSystemCatalog> systemCatalogPtr =
		erydbSystemCatalog::makeerydbSystemCatalog();
	erydbSystemCatalog::RIDList colRidList =
		systemCatalogPtr->columnRIDs(tableName, true);
	erydbSystemCatalog::RIDList::const_iterator rid_iterator =
		colRidList.begin();
	...
	while (rid_iterator != colRidList.end())
	{
		...
		erydbSystemCatalog::OID       oid = roPair.objnum;
		...
		{
			erydbSystemCatalog::TableColName colName;
