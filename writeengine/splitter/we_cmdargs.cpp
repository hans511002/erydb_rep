...
void WECmdArgs::appTestFunction()
{
	...
	// testing begins
	//std::string aJobFile("/home/bpaul/erydb/data/bulk/job/Job_1234.xml");
	//std::string aJobFile("/usr/local/erydb/data/bulk/job/Job_1234.xml");
...
std::string WECmdArgs::getBrmRptFileName()
{
	...
	{
		//cout << "ERROR: Could not find TempFileDir in erydb.xml" << endl;
		throw(runtime_error("Could not find TempFileDir in erydb.xml"));
	}
...
void WECmdArgs::checkJobIdCase()
{
	...
	if(fJobPath.empty())
	{
		...
		if(!bulkRootPath.empty())
			...
			throw runtime_error("Config Error: BulkRoot not found in erydb.xml");
	}
	char aBuff[256];
	if(!fJobPath.empty())
