...
int RedistributeControlThread::makeRedistributePlan()
{
	...
	try
	{
		...
		vector<pair<erydbSystemCatalog::OID, erydbSystemCatalog::TableName> >
			tables = csc->getTables();
		vector<pair<erydbSystemCatalog::OID, erydbSystemCatalog::TableName> >::iterator i;
		for (i = tables.begin(); i != tables.end(); i++)
		{
			...
			// column oids
			erydbSystemCatalog::RIDList cols = csc->columnRIDs(i->second, true);
