...
int RedistributeWorkerThread::grabTableLock()
{
	...
	try
	{
		...
		while (fTableLockId == 0 && !fStopAction)
		{
			...
			{
				fErrorCode = RED_EC_ERYDB_HARD_FAIL;
			...
			catch (...)
			{
				...
				// no need to throw
				// throw ERYDBExcept(ERR_HARD_FAILURE);
			}
		}
	}
	catch (const std::exception& ex)
...
int RedistributeWorkerThread::buildEntryList()
{
	...
	{
		// get all column oids
		boost::shared_ptr<erydbSystemCatalog> csc = erydbSystemCatalog::makeerydbSystemCatalog(0);
		const erydbSystemCatalog::TableName table = csc->tableName(fPlanEntry.table);
		erydbSystemCatalog::RIDList cols = csc->columnRIDs(table, true);
		for (erydbSystemCatalog::RIDList::iterator i = cols.begin(); i != cols.end(); i++)
			fOids.push_back(i->objnum);

		erydbSystemCatalog::DictOIDList dicts = csc->dictOIDs(table);
		for (erydbSystemCatalog::DictOIDList::iterator i = dicts.begin(); i != dicts.end(); i++)
...
int RedistributeWorkerThread::sendData()
{
	...
	ERYDBDataFile::Types fileType = 
		(ERYDBPolicy::useHdfs() ? ERYDBDataFile::HDFS : ERYDBDataFile::UNBUFFERED);
	ERYDBFileSystem& fs = ERYDBFileSystem::getFs( fileType );

	if ((remotePM) && (fileType != ERYDBDataFile::HDFS))
	{
	...
	else                                           // local or HDFS file copy
	{
		...
		for (vector<int64_t>::iterator i = fOids.begin(); i != fOids.end(); i++)
		{
			for (set<int16_t>::iterator j = fSegments.begin(); j != fSegments.end(); ++j)
			{
				...
				if (fileType == ERYDBDataFile::HDFS) // HDFS file copy
				{
					string sourceName;
				...
				else                               // local file copy
				{
					...
					// Using boost::copy_file() instead of ERYDBFileSystem::copy-
					// File() so we can capture/report any boost exception error
					// msg that ERYDBFileSystem::copyFile() currently swallows.
					try
					{
...
void RedistributeWorkerThread::confirmToPeer()
{
	...
	ERYDBFileSystem& fs = ERYDBFileSystem::getFs(
		(ERYDBPolicy::useHdfs() ? ERYDBDataFile::HDFS : ERYDBDataFile::UNBUFFERED) );
