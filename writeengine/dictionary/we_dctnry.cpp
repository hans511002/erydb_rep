...
int Dctnry::closeDctnry(bool realClose)
{
    ...
    m_hwm = (HWM)m_lastFbo;
    erydbassert(m_dctnryOID>=0);
	
	if (erydbdatafile::ERYDBPolicy::useHdfs() && realClose) 
	{
		BRM::FileInfo aFile;
...
void  Dctnry::getBlockOpCount( const DataBlock &fileBlock, int & op_count)
{
    ...
    bs >> dbyte;
    bs >> dbyte;
    erydbassert(dbyte == BYTE_PER_BLOCK);
    bs >> offset;
 ...
 ******************************************************************************/
int  Dctnry::getEndOp(ERYDBDataFile* dFile, int lbid, int &endOp)
{
    DataBlock fileBlock;
    Offset newOffset;
    int rc;
 ...
 * open dictionary file
 ******************************************************************************/
ERYDBDataFile* Dctnry::createDctnryFile(
