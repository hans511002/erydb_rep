...
void TableInfo::reportTotals(double elapsedTime)
{
    ...
    std::vector<boost::tuple<erydbSystemCatalog::ColDataType,uint64_t,uint64_t> > satCounts;
    for (unsigned i=0; i < fColumns.size(); ++i)
    {
       ...
       // colName += fColumns[i].column.colName;
        ...
        {   // @bug 3375: report invalid dates/times set to null
            ...
            if (fColumns[i].column.dataType == erydbSystemCatalog::DATE)
			{
				ossSatCnt <<
					"invalid dates replaced with zero value : ";
			}
            else if (fColumns[i].column.dataType ==
                     erydbSystemCatalog::DATETIME)
			{
				//bug5383
				ossSatCnt <<
					"invalid date/times replaced with zero value : ";
			}
            else if (fColumns[i].column.dataType == erydbSystemCatalog::CHAR)
                ossSatCnt <<
                    "character strings truncated: ";
            else if (fColumns[i].column.dataType ==
                     erydbSystemCatalog::VARCHAR)
                ossSatCnt <<
                    "character strings truncated: ";
            else
                ossSatCnt <<
