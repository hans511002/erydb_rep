    ...
    switch( column.weType )
    {
        ...
        case WriteEngine::WR_SHORT :
        {
            ...
            else
            {
                ...
                {
                    if( (column.dataType == erydbSystemCatalog::DECIMAL ) ||
                        (column.dataType == erydbSystemCatalog::UDECIMAL) )
                    {
        ...
        case WriteEngine::WR_LONGLONG:
        {
            ...
            if( column.dataType != erydbSystemCatalog::DATETIME )
            {
                if (nullFlag)
                {
                    if (!column.autoIncFlag)
                ...
                else
                {
                    ...
                    else
                    {
                        llDate =dataconvert::DataConvert::convertColumnDatetime(
                            field, dataconvert::ERYDBDATETIME_ENUM,
                            rc, fieldLength );
                    }
                }
        ...
        default  :
        {
            if( column.dataType != erydbSystemCatalog::DATE ) 
            {
                long long origVal;
                bool bSatVal = false;
                ...
                else
                {
                    ...
                    else
                    {
                        iDate = dataconvert::DataConvert::convertColumnDate(
                            field, dataconvert::ERYDBDATE_ENUM,
                            rc, fieldLength );
                    }
                }
    ...
    while( p < pEndOfData )
    {
            ...
            (fFieldList[curFld].fFldColType == BULK_FLDCOL_COLUMN_FIELD))
        {
            ...
            if (curCol < fNumColsInFile)
            {
                ...
                if (jobCol.dataType ==erydbSystemCatalog::CHAR) 
                {
					//cout << "triming ... " << endl;
					char * tmp = p;
                ...
                if (offset)
                {
                    ...
                    if (getTruncationAsError() && bValidRow &&
                        (fTokens[curRowNum1][curCol].offset !=
                            COLPOSPAIR_NULL_TOKEN_OFFSET))
                    {
                        if ((jobCol.dataType == erydbSystemCatalog::VARCHAR ||
                             jobCol.dataType == erydbSystemCatalog::CHAR)   &&
                            (fTokens[curRowNum1][curCol].offset >
                             jobCol.definedWidth))
                        {
                            bValidRow = false;
...
bool BulkLoadBuffer::isBinaryFieldNull(void* val,
    ...
    execplan::erydbSystemCatalog::ColDataType dt)
{
    ...
    switch (ct)
    {
        ...
        case WriteEngine::WR_INT:
        {
            if (dt == execplan::erydbSystemCatalog::DATE)
            {
        ...
        case WriteEngine::WR_LONGLONG:
        {
            if (dt == execplan::erydbSystemCatalog::DATETIME)
            {
