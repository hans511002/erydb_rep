 ...
 *      Author: Boby Paul: bpaul@erydb.com
 */
 ...
 *
 **/
std::string WEDataLoader::getErydbHome()
{
	string erydbDir = config::Config::makeConfig()->getConfig(
			"SystemConfig", "ErydbHome");
	if(0 == erydbDir.length())
	{
		erydbDir = startup::StartUp::installDir() + "/bin";
	}
	else
	{
		erydbDir += "/bin";
...
{
	std::string cpimportPath = getErydbHome();
	cpimportPath += "/";
