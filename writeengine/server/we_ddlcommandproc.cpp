...
uint8_t WE_DDLCommandProc::writeSystable(ByteStream& bs, std::string &err)
{
	...
	erydbSystemCatalog::ROPair sysTableROPair;
	...
	try
	{
		...
		if (0 != colStructs.size())
		{
			...
			{
				WErrorCodes ec;
				throw std::runtime_error("WE: Error updating erydbsys.systable:" + ec.errorString(error));
			}
			}
			if (erydbdatafile::ERYDBPolicy::useHdfs())
			{		
...
uint8_t WE_DDLCommandProc::writeCreateSyscolumn(ByteStream& bs, std::string &err)
{
	...
	try
	{
		...
		while (iter != tableDefCols.end())
		{
			...
			else if (dataType == erydbSystemCatalog::VARBINARY && colDefPtr->fType->fLength <= 7)
			{
				ostringstream os;
		...
		if (0 != colStructs.size())
		{
			...
			if (error != WriteEngine::NO_ERROR)
			{
				...
				else
				{
					WErrorCodes ec;
					throw std::runtime_error(
						"WE: Error updating erydbsys.syscolumn. " + ec.errorString(error));
				}
			}
			else
				error = rc1;		
...
uint8_t WE_DDLCommandProc::createtablefiles(ByteStream& bs, std::string & err)
{
	...
	for (i = 0; i < size; ++i) {
		...
		dataOid = tmp32;
		bs >> tmp8;
		colDataType = (erydbSystemCatalog::ColDataType)tmp8;
		bs >> tmp8;
		tokenFlag = (tmp8 != 0);
		bs >> tmp32;
		colWidth = tmp32;
...
uint8_t WE_DDLCommandProc::deleteSyscolumn(ByteStream& bs, std::string & err)
{
	...
	erydbSystemCatalog::TableName userTableName;
	...
	try
	{
		erydbSystemCatalog::RIDList colRidList = systemCatalogPtr->columnRIDs(userTableName);
		...
		DDLColumn column;
		erydbSystemCatalog::RIDList::const_iterator colrid_iterator = colRidList.begin();
...
uint8_t WE_DDLCommandProc::deleteSyscolumnRow(ByteStream& bs, std::string & err)
{
	...
	try
	{
		erydbSystemCatalog::ROPair colRO = systemCatalogPtr->columnRID(tableColName);
		if (colRO.objnum < 0)
		{
...
uint8_t WE_DDLCommandProc::deleteSystable(ByteStream& bs, std::string & err)
{
	...
	try
	{
		erydbSystemCatalog::ROPair userTableROPair = systemCatalogPtr->tableRID(userTableName);
...
uint8_t WE_DDLCommandProc::updateSyscolumnTablename(ByteStream& bs, std::string & err)
{
	...
	colStruct.colWidth = column.colType.colWidth > 8 ? 8 : column.colType.colWidth;
	colStruct.tokenFlag = false;
	if ( (column.colType.colDataType == erydbSystemCatalog::CHAR
				&& column.colType.colWidth > 8)
			|| (column.colType.colDataType == erydbSystemCatalog::VARCHAR
				&& column.colType.colWidth > 7)
			|| (column.colType.colDataType == erydbSystemCatalog::VARBINARY
				&& column.colType.colWidth > 7)
			|| (column.colType.colDataType == erydbSystemCatalog::DECIMAL
				&& column.colType.precision > 18)
			|| (column.colType.colDataType == erydbSystemCatalog::UDECIMAL
...
uint8_t WE_DDLCommandProc::updateSystableAuto(ByteStream& bs, std::string & err)
{
	...
	std::vector<extentInfo> extentsinfo;
	extentInfo aExtentinfo;
	erydbSystemCatalog::OID oid = 1003;
...
uint8_t WE_DDLCommandProc::updateSyscolumnColumnposCol(messageqcpp::ByteStream& bs, std::string & err)
{
	...
	systemCatalogPtr->identity(erydbSystemCatalog::EC);
	erydbSystemCatalog::RIDList rids;
	try {
	...
	erydbSystemCatalog::RIDList::const_iterator rid_iter = rids.begin();
	boost::any value;
	WriteEngine::ColTupleList colTuples;
	erydbSystemCatalog::ColType columnType;
	erydbSystemCatalog::ROPair colRO;
...
uint8_t WE_DDLCommandProc::fillNewColumn(ByteStream& bs, std::string& err)
{
	...
	erydbSystemCatalog::ColDataType dataType, refColDataType;
	bool autoincrement;
	...
	dictOid = tmp32;
	bs >> tmp8;
	dataType = (erydbSystemCatalog::ColDataType) tmp8;
	bs >> tmp8;
	...
	refColOID = tmp32;
	bs >> tmp8;
	refColDataType = (erydbSystemCatalog::ColDataType) tmp8;
	bs >> tmp32;
	refColWidth = tmp32;
	bs >> tmp8;
	...
	colType.colDataType = static_cast<erydbSystemCatalog::ColDataType>(dataType);
...
uint8_t WE_DDLCommandProc::writeTruncateLog(ByteStream& bs, std::string& err)
{
	...
	if (prefix.length() == 0) {
		err = "Need a valid DBRMRoot entry in erydb configuation file";
		rc =1;
		return rc;
	}
...
uint8_t WE_DDLCommandProc::deleteDDLLog(ByteStream& bs, std::string& err)
{
	...
	ERYDBPolicy::remove(DDLLogFileName.c_str());
...
uint8_t WE_DDLCommandProc::fetchDDLLog(ByteStream& bs, std::string& err)
{
	...
	erydbSystemCatalog::OID fileoid;
	string tableName;
	bs.restart();
	for ( unsigned i=0; i < fileNames.size(); i++ )
	{
		...
		else //Find drop partition log file
		{
			...
			if ( pos != string::npos )
			{
				...
				std::vector<erydbSystemCatalog::OID> oidPartList;
				while (strbuf >> fileoid)
...
uint8_t WE_DDLCommandProc::updateSyscolumnSetDefault(messageqcpp::ByteStream& bs, std::string & err)
{
	...
	//Build colStructs for SYSCOLUMN
	atableName.schema = ERYDB_SCHEMA;
...
uint8_t WE_DDLCommandProc::updateSyscolumnRenameColumn(messageqcpp::ByteStream& bs, std::string & err)
{
	...
	colStruct.tokenFlag = false;
	if ( (column1.colType.colDataType == erydbSystemCatalog::CHAR
			&& column1.colType.colWidth > 8)
		|| (column1.colType.colDataType == erydbSystemCatalog::VARCHAR
			&& column1.colType.colWidth > 7)
		|| (column1.colType.colDataType == erydbSystemCatalog::VARBINARY
			&& column1.colType.colWidth > 7)
		|| (column1.colType.colDataType == erydbSystemCatalog::DECIMAL
			&& column1.colType.precision > 18)
		|| (column1.colType.colDataType == erydbSystemCatalog::UDECIMAL
	...
	colStruct.tokenFlag = false;
	if ( (column5.colType.colDataType == erydbSystemCatalog::CHAR
			&& column5.colType.colWidth > 8)
		|| (column5.colType.colDataType == erydbSystemCatalog::VARCHAR
			&& column5.colType.colWidth > 7)
		|| (column5.colType.colDataType == erydbSystemCatalog::VARBINARY
			&& column5.colType.colWidth > 7)
		|| (column5.colType.colDataType == erydbSystemCatalog::DECIMAL
			&& column5.colType.precision > 18)
		 || (column5.colType.colDataType == erydbSystemCatalog::UDECIMAL
