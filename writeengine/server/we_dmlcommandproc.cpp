...
uint8_t WE_DMLCommandProc::processSingleInsert(messageqcpp::ByteStream& bs, std::string & err)
{
	...
	erydbSystemCatalog::ROPair tableRoPair;
	...
	try
	{
		...
		if (rows.size())
		{
			...
			while (column_iterator != columns.end())
			{
				...
				erydbSystemCatalog::ROPair roPair = systemCatalogPtr->columnRID(tableColName);

				erydbSystemCatalog::OID oid = systemCatalogPtr->lookupOID(tableColName);
				...
				while (row_iterator != rows.end())
				{
					...
					if ( isDictCol(colType) )
					{
						for ( uint32_t i=0; i < origVals.size(); i++ )
						{
							...
							if (colType.constraintType == erydbSystemCatalog::NOTNULL_CONSTRAINT)
							{
								if (isNULL && colType.defaultValue.empty()) //error out
								{
									Message::Args args;
									args.add(tableColName.column);
									err = ERYDBErrorInfo::instance()->errorMsg(ERR_NOT_NULL_CONSTRAINTS, args);
									rc = 1;
									return rc;
								}
							...
							if ( tmpStr.length() > (unsigned int)colType.colWidth )
							{
								...
								if ( !pushWarning )
								{
									pushWarning = true;
									isWarningSet = true;
									if ((rc != NO_ERROR) && (rc != dmlpackageprocessor::DMLPackageProcessor::ERYDBRANGE_WARNING))
										rc = dmlpackageprocessor::DMLPackageProcessor::ERYDBRANGE_WARNING;
						...
						for ( uint32_t i=0; i < origVals.size(); i++ )
						{
							...
							if (colType.autoincrement && ( isNULL || (indata.compare("0")==0)))
							{
								try
								{
									...
									if (!reserved)
									{
										err = ERYDBErrorInfo::instance()->errorMsg(ERR_EXCEED_LIMIT);
										rc = 1;
										return rc;
									}
								}
							...
							catch(exception&)
							{
								...
								err = ERYDBErrorInfo::instance()->errorMsg(ERR_NON_NUMERIC_DATA, args);
							}
							//@Bug 1806
							if (rc != NO_ERROR && rc != dmlpackageprocessor::DMLPackageProcessor::ERYDBRANGE_WARNING)
							{
							...
							if ( pushWarning) 
							{
								if (!isWarningSet)
									isWarningSet = true;
								if ( rc != dmlpackageprocessor::DMLPackageProcessor::ERYDBRANGE_WARNING )
									rc = dmlpackageprocessor::DMLPackageProcessor::ERYDBRANGE_WARNING;
	...
	if (erydbdatafile::ERYDBPolicy::useHdfs())
	{
		...
		erydbSystemCatalog::RIDList ridList;
		try
		{
			...
			erydbSystemCatalog::ColType colType2 = systemCatalogPtr->colType(ridList[i].objnum);
	...
	if (colValuesList[0].size() > 0)
	{
		...
		(error = fWEWrapper.insertColumnRec_Single(txnid.id, colStructs, colValuesList, dctnryStructList, dicStringList, tableRoPair.objnum)))
		{
			...
			else if ( error == ERR_BRM_VB_OVERFLOW )
			{
				...
				err = ERYDBErrorInfo::instance()->errorMsg(ERR_VERSIONBUFFER_OVERFLOW);
			}
			else
			{
	...
	if ((rc == dmlpackageprocessor::DMLPackageProcessor::ERYDBRANGE_WARNING) || isWarningSet)
	{
		if (rc == NO_ERROR)
			rc = dmlpackageprocessor::DMLPackageProcessor::ERYDBRANGE_WARNING;
		Message::Args args;
		...
		args.add(cols);
		err = ERYDBErrorInfo::instance()->errorMsg(WARN_DATA_TRUNC,args);
				...
				while (row_iterator != rows.end())
				{
					...
					else
					{
						...
						for ( uint32_t i=0; i < origVals.size(); i++ )
						{
							...
							if (colType.constraintType == erydbSystemCatalog::NOTNULL_CONSTRAINT)
							{
								if (((colType.colDataType == execplan::erydbSystemCatalog::DATE) && (indata =="0000-00-00")) ||
									((colType.colDataType == execplan::erydbSystemCatalog::DATETIME) && (indata =="0000-00-00 00:00:00")))
								{
									isNULL = true;
								}
							...
							if ( pushWarning && ( rc != dmlpackageprocessor::DMLPackageProcessor::ERYDBRANGE_WARNING ) )
								rc = dmlpackageprocessor::DMLPackageProcessor::ERYDBRANGE_WARNING;
...
uint8_t WE_DMLCommandProc::commitBatchAutoOn(messageqcpp::ByteStream& bs, std::string & err)
{
	...
	erydbSystemCatalog::TableName aTableName =  systemCatalogPtr->tableName(tableOid);
	erydbSystemCatalog::RIDList ridList;
	try
	{
...
uint8_t WE_DMLCommandProc::processBatchInsertHwm(messageqcpp::ByteStream& bs, std::string & err)
{
	...
	boost::shared_ptr<erydbSystemCatalog> systemCatalogPtr = erydbSystemCatalog::makeerydbSystemCatalog(0);
	systemCatalogPtr->identity(erydbSystemCatalog::EC);
	try {
		erydbSystemCatalog::TableName tableName = systemCatalogPtr->tableName(tableOid);
	...
	std::string& err)
{
	...
	{
		boost::shared_ptr<erydbSystemCatalog> systemCatalogPtr =
			erydbSystemCatalog::makeerydbSystemCatalog(txnID);
										 ...
										 uint64_t& blocksChanged)
{
	...
	std::vector<execplan::erydbSystemCatalog::ColDataType> fetchColTypes =  rowGroups[txnId]->getColTypes();
	...
	boost::shared_ptr<erydbSystemCatalog> systemCatalogPtr =
		erydbSystemCatalog::makeerydbSystemCatalog(sessionID);
	erydbSystemCatalog::OID oid = 0;
	erydbSystemCatalog::ROPair tableRO;
	try
	{
	...
	// for query stats
	boost::scoped_array<erydbSystemCatalog::ColType> colTypes(new erydbSystemCatalog::ColType[columnsUpdated.size()]);
	...
	/*		WriteEngine::ColTupleList colTupleList;
			...
			*/
			WriteEngine::ColTupleList colTupleList;
			erydbSystemCatalog::ColType colType = colTypes[j];
			oid = oids[j];
			...
			if (isDictCol(colType))
			{
				...
				if (columnsUpdated[j]->get_isFromCol())
				{
					for (unsigned i = 0; i < rowsThisRowgroup; i++)
					{
						...
						{
							if ((colType.defaultValue.length() <= 0) && (colType.constraintType == erydbSystemCatalog::NOTNULL_CONSTRAINT))
							{
								rc = 1;
								Message::Args args;
								args.add(tableColName.column);
						...
						switch (fetchColTypes[fetchColPos])
						{
							...
							{
								{
									...
									if (fetchColTypes[fetchColPos] == erydbSystemCatalog::UDECIMAL
										&& intColVal < 0)
									{
										intColVal = 0;
									}
							...
							case erydbSystemCatalog::UFLOAT:
							{
								...
								if (dl == std::numeric_limits<float>::infinity())
									continue;
								if (fetchColTypes[fetchColPos] == erydbSystemCatalog::UFLOAT && dl < 0.0)
								{
									dl = 0.0;
								}
								ostringstream os;
							...
							case erydbSystemCatalog::UDOUBLE:
							{
								...
								if (dl == std::numeric_limits<double>::infinity())
									continue;
								if (fetchColTypes[fetchColPos] == erydbSystemCatalog::UDOUBLE && dl < 0.0)
								{
									dl = 0.0;
								}
								ostringstream os;
			...
			else //Non dictionary column
			{
				...
				if (columnsUpdated[j]->get_isFromCol())
				{
					for (unsigned i = 0; i < rowsThisRowgroup; i++)
					{
						...
						if ((isNull) && (colType.defaultValue.length() <= 0) && (colType.constraintType == erydbSystemCatalog::NOTNULL_CONSTRAINT))
						{
							rc = 1;
							Message::Args args;
							args.add(tableColName.column);
				...
				else //constant column
				{
					...
					else if (isNull && (colType.defaultValue.length() <= 0) && (colType.constraintType == erydbSystemCatalog::NOTNULL_CONSTRAINT))
					{
						rc = 1;
						Message::Args args;
						args.add(tableColName.column);
...
//No need to close files, flushDataFile will close them.
	//if (((colType.compressionType > 0 ) && (dictOids.size() > 0)) || (erydbdatafile::ERYDBPolicy::useHdfs()))
	//	fWEWrapper.closeDctnry(txnId, colType.compressionType, true);
	if (error != NO_ERROR)
	{
			rc = error;
										 ...
										 uint64_t& blocksChanged)
{
	...
	erydbSystemCatalog::RIDList tableRidList;
	...
	std::string& err)
{
	...
	try
	{
		...
		{
			boost::shared_ptr<erydbSystemCatalog> systemCatalogPtr =
			erydbSystemCatalog::makeerydbSystemCatalog(0);
			erydbSystemCatalog::TableName aTableName = systemCatalogPtr->tableName(tableOID);
...
//  "; wid-" << jobColK.width << " ; hwm-"<<segFileInfo[k].fLocalHwm<<std::endl;
        ...
        {
			erydbSystemCatalog::ColType colType2;
...
errorCheck:
    if (rc != NO_ERROR)
    {
		erydbSystemCatalog::ColType colType1, colType2;
