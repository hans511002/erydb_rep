...

{
    ...
    */
    /* static */ void WriteEngineWrapper::init(unsigned subSystemID) {
        ...
        //--------------------------------------------------------------------------
        if (maxPct > 0 || ERYDBPolicy::useHdfs()) {
    ...
    int WriteEngineWrapper::fillColumn(const TxnID& txnid, const OID& dataOid,const erydbSystemCatalog::ColDataType dataType, int dataWidth,ColTuple defaultVal,
       ...
       // dctnry.reset(dctOp);
        ...
        //Bug 1703,1705
        bool isToken = false;
        if (((dataType == erydbSystemCatalog::VARCHAR) && (dataWidth > 7)) ||
            ((dataType == erydbSystemCatalog::CHAR) && (dataWidth > 8)) || (dataType == erydbSystemCatalog::VARBINARY)) {
            isToken = true;
        }
        Convertor::convertColType(dataType, newColType, isToken);

        if (((refColDataType == erydbSystemCatalog::VARCHAR) && (refColWidth > 7)) ||
            ((refColDataType == erydbSystemCatalog::CHAR) && (refColWidth > 8)) || (refColDataType == erydbSystemCatalog::VARBINARY)) {
            isToken = true;
        }
    ...
    */
     ...
     *    others if something wrong in inserting the value
     ***********************************************************/
    int WriteEngineWrapper::processVersionBuffer(ERYDBDataFile* pFile, const TxnID& txnid,const ColStruct& colStruct, int width,
    ...
    int WriteEngineWrapper::processVersionBuffers(ERYDBDataFile* pFile, const TxnID& txnid,const ColStruct& colStruct, int width,
    ...
    */
    ...
    int WriteEngineWrapper::writeColumnRec(const TxnID& txnid,const ColStructList& colStructList, const ColValueList& colValueList,vector<void *>& colOldValueList,
        const RIDList& ridList,const int32_t tableOid,bool convertStructFlag,ColTupleList::size_type nRows) {
        bool           bExcp;
        int            rc = 0;
        void*          valArray = NULL;
        Column         curCol;
    ...
    */
    ...
    int WriteEngineWrapper::rollbackCommon(const TxnID& txnid, int sessionId) {
        ...
        if (prefix.length() == 0) {
            cerr << "Need a valid DBRMRoot entry in erydb configuation file";
            return -1;
        }
        ...
        if (ERYDBPolicy::exists(aDMLLogFileName.c_str())) {
            // TODO-for now the DML log file will always be in a local
            // filesystem since ERYDBDataFile doesn't have any support for
            // a cpp iostream interface.  need to decide if this is ok.
            boost::scoped_ptr<ERYDBDataFile> aDMLLogFile(ERYDBDataFile::open(
                ERYDBPolicy::getType(aDMLLogFileName.c_str(),
            ...
            if (aDMLLogFile) //need recover
            {
                ...
                while (strstream >> backUpFileType >> filename >> size >> offset) {
                    ...
                    if (backUpFileType.compare("rlc") == 0) {
                        ...
                        ERYDBPolicy::remove(filename.c_str());
                        ...
                        if (ERYDBPolicy::exists(orig.c_str())) {
                            // not likely both cdf and tmp exist
                            if (ERYDBPolicy::exists(filename.c_str()) &&
                                ERYDBPolicy::remove(filename.c_str()) != 0)
                                rc = ERR_COMP_REMOVE_FILE;

                            if (rc == NO_ERROR && ERYDBPolicy::rename(orig.c_str(), filename.c_str()) != 0)
                                rc = ERR_COMP_RENAME_FILE;
                        }

                        // remove the tmp file
                        string tmp(filename + ".tmp");
                        if (rc == NO_ERROR && ERYDBPolicy::exists(tmp.c_str()) &&
                            ERYDBPolicy::remove(tmp.c_str()) != 0)
                            rc = ERR_COMP_REMOVE_FILE;

                        // remove the chunk shifting helper
                        string rlc(filename + ".rlc");
                        if (rc == NO_ERROR && ERYDBPolicy::exists(rlc.c_str()) &&
                            ERYDBPolicy::remove(rlc.c_str()) != 0)
                            rc = ERR_COMP_REMOVE_FILE;
                        ...
                        ERYDBDataFile* sourceFile = ERYDBDataFile::open(
                            ERYDBPolicy::getType(backFileName.c_str(), ERYDBPolicy::WRITEENG),
                            backFileName.c_str(), "r", 0);
                        ERYDBDataFile* targetFile = ERYDBDataFile::open(
                            ERYDBPolicy::getType(filename.c_str(), ERYDBPolicy::WRITEENG),
                            filename.c_str(), "r+", 0);
                        ...
                        delete targetFile;
                        delete sourceFile;
                        ERYDBPolicy::remove(backFileName.c_str());
            ...
            ERYDBPolicy::remove(aDMLLogFileName.c_str());
        }
    ...
    int WriteEngineWrapper::AddLBIDtoList(const TxnID     txnid,
        ...
        std::vector<erydbSystemCatalog::ColDataType>& colDataTypes,
