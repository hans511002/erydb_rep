...
public:
   ...
   */
   ERYDBDataFile*  openFile(const Column& column, uint16_t dbRoot, uint32_t partition,
   ...
   * @brief virtual method in ColumnOp
   */
   bool abbreviatedExtent(ERYDBDataFile* pFile, int colWidth) const;

   /**
   * @brief virtual method in ColumnOp
   */
   int blocksInFile(ERYDBDataFile* pFile) const;
...
protected:
   ...
   * @brief virtual method in ColumnOp
   */
   int readBlock(ERYDBDataFile* pFile, unsigned char* readBuf, const uint64_t fbo);

   /**
   * @brief virtual method in ColumnOp
   */
   int saveBlock(ERYDBDataFile* pFile, const unsigned char* writeBuf, const uint64_t fbo);
...
protected:
   ...
   * @brief virtual method in FileOp
   */
   int updateColumnExtent(ERYDBDataFile* pFile, int nBlocks);
