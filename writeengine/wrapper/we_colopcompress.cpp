...
bool ColumnOpCompress0::abbreviatedExtent(ERYDBDataFile* pFile, int colWidth) const
{
    long long fsize;
...
int ColumnOpCompress0::blocksInFile(ERYDBDataFile* pFile) const
{
    long long fsize;
    if (getFileSize(pFile, fsize) == NO_ERROR)
    {
...
int ColumnOpCompress0::readBlock(ERYDBDataFile* pFile, unsigned char* readBuf, const uint64_t fbo)
{
   return readDBFile(pFile, readBuf, fbo, true);
}


int ColumnOpCompress0::saveBlock(ERYDBDataFile* pFile, const unsigned char* writeBuf, const uint64_t fbo)
{
...
bool ColumnOpCompress1::abbreviatedExtent(ERYDBDataFile* pFile, int colWidth) const
{
   return (blocksInFile(pFile) == INITIAL_EXTENT_ROWS_TO_DISK*colWidth/BYTE_PER_BLOCK);
}


int ColumnOpCompress1::blocksInFile(ERYDBDataFile* pFile) const
{
...
int ColumnOpCompress1::readBlock(ERYDBDataFile* pFile, unsigned char* readBuf, const uint64_t fbo)
{
   return m_chunkManager->readBlock(pFile, readBuf, fbo);
}


int ColumnOpCompress1::saveBlock(ERYDBDataFile* pFile, const unsigned char* writeBuf, const uint64_t fbo)
{
...
int ColumnOpCompress1::updateColumnExtent(ERYDBDataFile* pFile, int nBlocks)
{
