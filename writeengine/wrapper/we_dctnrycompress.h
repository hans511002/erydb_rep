...
public:
   ...
   */
   EXPORT int readDBFile(ERYDBDataFile* pFile, unsigned char* readBuf, const uint64_t lbid,
                         const bool isFbo = false );

   /**
   * @brief virtual method in DBFileOp
   */
   EXPORT int writeDBFile(ERYDBDataFile* pFile, const unsigned char* writeBuf, const uint64_t lbid,
                          const int numOfBlock = 1);

	/**
   * @brief virtual method in DBFileOp
   */
   EXPORT int writeDBFileNoVBCache(ERYDBDataFile* pFile,
   ...
   */
   ERYDBDataFile* createDctnryFile(const char *name, int width, const char *mode, int ioBuffSize);

   /**
   * @brief virtual method in Dctnry
   */
   ERYDBDataFile* openDctnryFile(bool useTmpSuffix);
   ...
   */
   ...
   int checkFixLastDictChunk () { return m_chunkManager->checkFixLastDictChunk(m_dctnryOID, m_dbRoot.getPmDbr(), m_partition, m_segment); };
...
protected:
   ...
   * @brief virtual method in FileOp
   */
   int updateDctnryExtent(ERYDBDataFile* pFile, int nBlocks);
