...
int ColumnOp::addExtent(const Column& column,DBROOTS_struct& dbRoot,uint32_t partition,uint16_t segment,std::string& segFile,BRM::LBID_t& startLbid,bool& newFile,int& allocSize,char* hdrs)
{
   ...
   int ColumnOp::expandAbbrevExtent(const Column& column)
   {
	  ...
	  int rc = expandAbbrevColumnExtent(column.dataFile.pFile,column.dataFile.fdbr,emptyVal,column.colWidth);
    ...
    ***********************************************************/
   bool ColumnOp::getColDataType(const char* name, erydbSystemCatalog::ColDataType& colDataType) const
   {
      bool bFound = false;
      for(int i = 0; i < erydbSystemCatalog::NUM_OF_COL_DATA_TYPE; i++)
         if (strcmp(name, ColDataTypeStr[i]) == 0) {
            colDataType = static_cast<erydbSystemCatalog::ColDataType>(i);
            bFound = true;
            break;
         }
