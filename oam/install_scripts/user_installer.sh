...
#
if { $PKGTYPE == "rpm" } {
	set PKGERASE "rpm -e --nodeps \$(rpm -qa | grep '^erydb')"
	set PKGERASE1 "rpm -e --nodeps "

	set PKGINSTALL "rpm -ivh $NODEPS --force erydb*$VERSION*rpm"
	set PKGUPGRADE "rpm -Uvh --noscripts erydb*$VERSION*rpm"
} else {
	if { $PKGTYPE == "deb" } {
		set PKGERASE "dpkg -P \$(dpkg --get-selections | grep '^erydb')"
		set PKGERASE1 "dpkg -P "
		set PKGINSTALL "dpkg -i --force-confnew erydb*$VERSION*deb"
		set PKGUPGRADE "dpkg -i --force-confnew erydb*$VERSION*deb"
	} else {
...
set timeout 30
#expect -re {[$#] }
send_user "Copy new erydb Packages to Module              "
send "ssh $USERNAME@$SERVER 'rm -f /root/erydb-*.$PKGTYPE'\n"
if { $PASSWORD != "ssh" } {
	set timeout 30
	expect {
		"word: " { send "$PASSWORD\n" }
...
send "scp $HOME/erydb*$VERSION*$PKGTYPE $USERNAME@$SERVER:.;$PKGERASE1 dummy\n"
if { $PASSWORD != "ssh" } {
	set timeout 30
	expect {
		"word: " { send "$PASSWORD\n" }
