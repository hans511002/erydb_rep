...
#define CALDB_AVAILABLE      	// Comment out to use against non-erydb db (commits instead of calcommits).
...
/***************************************************************************
*
	* Function:  getNexterydbLogEntry
*
* Purpose:   Reads the passed filestream until a erydb log entry is found.
*	     The log entry is assigned in the pass by reference argument.
*	     A bool is returned indicating whether a erydb entry was found.
*
****************************************************************************/
bool ReplayTxnLog::getNexterydbLogEntry(ifstream& dataFile, LogEntry& logEntry) {
	bool found = false;
	string text;
	...
	/*
	Sample log entries:
	Mar 11 01:54:04 srvswdev3 erydb[29720]: 04.372658 |5|1|0| C 00 CAL0018: DDL|WALT|CREATE TABLE TEST (XXX CHAR(8));
	Mar 11 02:40:22 srvswdev3 erydb[29720]: 04.373627 |7|2|0| C 00 CAL0017: DML|WALT|INSERT INTO TEST (XXX) values ('TEST');
	Mar 11 02:40:24 srvswdev3 erydb[29720]: 04.373429 |5|1|0| C 00 CAL0019: CMD|COMMIT;
	*/
	...
	while(!found && getline(dataFile, text)) {
		...
		if(v.size() >= 5) {
			...
			if(vFirstSection.size() < 3) {
				...
				// Mar  7 21:28:36 srvperf6 DDLProc[7964]: 36.179609 |1|86|0| C 15 CAL0018: DDL ||CREATE TABLE Y (K NUMERIC)ENGINE=ERYDB;
...
/*
	...
	#else
*/
		fullLogFileName = "/var/log/erydb/data/data_mods.log";
/*
	#endif
*/
	...
	if(dbFileNames.size() > 1) {
		...
		while(!done && curFile != dbFileNames.end()) {
			...
			validLogEntry = getNexterydbLogEntry(dataFile, logEntry);
			dataFile.close();
...
//	cout << "Searching files for first transaction to process." << endl << endl;
	...
	while (!done && curFile != fileNames.end()) {
		...
		while(!done && getNexterydbLogEntry(dataFile, logEntry)) {
