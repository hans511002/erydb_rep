...
$ERYDB_INSTALL_DIR/MCSgetCredentials.sh >/dev/null 2>&1 

#get Region
Region=`$ERYDB_INSTALL_DIR/bin/MCSInstanceCmds.sh getRegion`
...
createvolume() {
	# get zone
	zone=`$ERYDB_INSTALL_DIR/bin/MCSInstanceCmds.sh getZone`

	if [ $moduleType == "um" ]; then
		# get type
		volumeType=`$ERYDB_INSTALL_DIR/bin/getConfig Installation UMVolumeType`
		if [ $volumeType == "io1" ]; then
			# get IOPS
			volumeIOPS=`$ERYDB_INSTALL_DIR/bin/getConfig Installation UMVolumeIOPS`
		fi
	else	# pm
		# get type
		volumeType=`$ERYDB_INSTALL_DIR/bin/getConfig Installation PMVolumeType`
		if [ $volumeType == "io1" ]; then
			# get IOPS
			volumeIOPS=`$ERYDB_INSTALL_DIR/bin/getConfig Installation PMVolumeIOPS`
		fi
	fi
...
detachvolume() {
	...
	if [ $STATUS == "detaching" ]; then
		...
		test -f /usr/local/erydb/post/functions && . /usr/local/erydb/post/functions
		$ERYDB_INSTALL_DIR/bin/cplogger -w 100 "detachvolume failed: $STATUS"
		echo "failed"
		exit 1
	fi
	...
	test -f $ERYDB_INSTALL_DIR/post/functions && . $ERYDB_INSTALL_DIR/post/functions
	$ERYDB_INSTALL_DIR/bin/cplogger -w 100 "detachvolume failed status: $STATUS"
	echo $STATUS
	exit 1
}
...
attachvolume() {
	...
	if [ $STATUS == "attaching" -o $STATUS == "already-attached" ]; then
		...
		test -f $ERYDB_INSTALL_DIR/post/functions && . $ERYDB_INSTALL_DIR/post/functions
		$ERYDB_INSTALL_DIR/bin/cplogger -w 100 "attachvolume failed: $STATUS"
		echo "failed"
		exit 1
	fi
