...
namespace processmonitor {
    ...
    ******************************************************************************************/
    ...
    MonitorConfig::MonitorConfig() {
        ...
        //	}
            ...
            //get erydb software version and release
    ...
    */
        ...
        if (listPtr == aPtr->end()) {	// not in list, add it
            processInfo proInfo;
    ...
    ******************************************************************************************/
                ...
                switch (requestID) {
                    ...
                    case START:
                    {
                        ...
                        if (processstatus.ProcessOpState != oam::ACTIVE) {
                            ...
                            if (actIndicator == oam::GRACEFUL_STANDBY) {
                                ...
                                unlink("/var/log/erydb/activeAlarms");
                            }
                    ...
                    case RESTART:
                    {
                        ...
                        if (processName == "mysql") {
                            try {
                                oam.actionMysqlerydb(MYSQL_RESTART);
                            } catch (...) {
                            }
                    ...
                    case STOPALL:
                    {
                        ...
                        if (requestStatus == oam::API_SUCCESS) {
                            ...
                            try {
                                oam.actionMysqlerydb(MYSQL_STOP);
                        ...
                        //Remove erydb RPM on REMOVE option
                        if (actIndicator == oam::REMOVE) {
                            log.writeLog(__LINE__, "STOPALL: uninstall erydb RPMs", LOG_TYPE_DEBUG);
                            if (config.moduleType() == "um") {
                                system("rpm -e --nodeps $(rpm -qa | grep '^erydb')");
                                system("dpkg -P $(dpkg --get-selections | grep '^erydb')");
                            } else	// pm
                            {
                                //Flush the cache
                            ...
                            string cmd = startup::StartUp::installDir() + "/bin/erydb stop > /dev/null 2>&1";
                            system(cmd.c_str());
                            exit(0);
                        }
                    ...
                    case STARTALL:
                    {
                        ...
                        try {
                            oam.actionMysqlerydb(MYSQL_START);
                        } catch (...) {	// mysql didn't start, return with error
                        ...
                        if (requestStatus == oam::API_SUCCESS) {
                            ...
                            for (listPtr = aPtr->begin(); listPtr != aPtr->end(); ++listPtr) {
                                ...
                                if ((*listPtr).state == oam::MAN_OFFLINE ||
                                    ...
                                    //								sleep(2);
                                } else {	// if DBRMWorkerNode and ACTIVE, run runStartupTest
                                    if (gOAMParentModuleFlag
                                        && (*listPtr).ProcessName == "DBRMWorkerNode"
    ...
    /*					processList* aPtr = config.monitoredListPtr();
                        ...
                        //stop the mysql daemon and then erydb
                        try {
                            oam.actionMysqlerydb(MYSQL_STOP);
                        }
                        catch(...)
    ...
    */
            ...
            case RECONFIGURE:
            {
                ...
                if (reconfigureModuleName.find("um") != string::npos) {
                    ...
                    system(cmd.c_str());
                    cmd = startup::StartUp::installDir() + "/mysql/mysql-erydb start > /tmp/mysqldstart";
                    system(cmd.c_str());
            ...
            {
                log.writeLog(__LINE__, "MSG RECEIVED: Get erydb Software Info");
            ...
            {
                log.writeLog(__LINE__, "MSG RECEIVED: Update erydb Config file");
...
//				system(cmd.c_str());
    ...
    //				DBRMroot = tempDBRMDir + "/BRM_saves";
                ...
                ssize_t fileSize = ERYDBPolicy::size(fileName.c_str());
                boost::scoped_ptr<ERYDBDataFile> oldFile(ERYDBDataFile::open(
                    ERYDBPolicy::getType(fileName.c_str(),
        ...
        if (newProcessID != 0) {
            ...
            if (processLocation.find("DecomSvr") != string::npos) {	// DecomSvr app is special
    ...
    ******************************************************************************************/
    int ProcessMonitor::updateLog(std::string action, std::string level) {
        ...
        if (configFile.length() == 0)
            configFile = "syslog";
        cf->setConfig("MessageLog", "MessageLogType", configFile);
        if (configFile == "syslog") {
            logging::messageLogType = 0;
            cf->write();
        } else {
            logging::messageLogType = 1;
            logging::messageLogDir = cf->getConfig("MessageLog", "MessageLogDir");
            if (messageLogDir.length() == 0)
                logging::messageLogDir = "/var/log/erydb";
            cf->setConfig("MessageLog", "MessageLogDir", logging::messageLogDir);
            transform(level.begin(), level.end(), level.begin(), to_lower());
            string levl = level;
            transform(levl.begin(), levl.end(), levl.begin(), to_lower());
            if (levl == "err" || levl == "error") {
                logging::messageLogLevel = 3;
            } else if (levl == "warning" || levl == "warn") {
                logging::messageLogLevel = 4;
            } else if (levl == "notice") {
                logging::messageLogLevel = 5;
            } else if (levl == "info") {
                logging::messageLogLevel = 6;
            } else if (levl == "debug" || levl == "all") {
                logging::messageLogLevel = 7;
            } else {
                levl = "info";
                logging::messageLogLevel = 6;
                log.writeLog(__LINE__, "level not int [error warning notice info debug], set default to info ", LOG_TYPE_ERROR);
            }
            cf->setConfig("MessageLog", "messageLogLevel", level);
            cf->write();
            return 0;
        }
    ...
    ******************************************************************************************/
    int ProcessMonitor::buildSystemTables() {
        ...
        if (!ERYDBPolicy::exists(fileName.c_str())) {
            string logdir("/var/log/erydb");
            if (access(logdir.c_str(), W_OK) != 0) logdir = "/tmp";
    ...
    ******************************************************************************************/
    int ProcessMonitor::reconfigureModule(std::string reconfigureModuleName) {
        ...
        //copy and apply new rc.local.erydb from pm1
        cmd = "rm -f " + installDir + "/local/rc.local.erydb";
        system(cmd.c_str());
        cmd = "cp " + installDir + "/local/etc/" + reconfigureModuleName + "/rc.local.erydb " + installDir + "/local/.";
        system(cmd.c_str());
        cmd = "rm -f /etc/rc.d/rc.local";
        system(cmd.c_str());
        cmd = "cp /etc/rc.d/rc.local.erydbSave /etc/rc.d/rc.local >/dev/null 2>&1";
        system(cmd.c_str());
        if (geteuid() == 0)
            cmd = "cat " + installDir + "/local/rc.local.erydb >> /etc/rc.d/rc.local >/dev/null 2>&1";
        system(cmd.c_str());
    ...
    *
    * purpose:	Create the erydb Data directories
    *
    *
    ******************************************************************************************/
    int ProcessMonitor::createDataDirs(std::string cloud) {
        ...
        if (config.moduleType() == "pm") {
            ...
            try {
                ...
                if (boost::get<1>(t) == 0) {
                    log.writeLog(__LINE__, "No dbroots are configured in erydb.xml file at proc mon startup time", LOG_TYPE_WARNING);
    ...
    ******************************************************************************************/
    ...
    //							}
                                ...
                                boost::scoped_ptr<ERYDBDataFile> out(ERYDBDataFile::open(
                                    ERYDBPolicy::getType(fileName.c_str(),
                                        ERYDBPolicy::WRITEENG),
                                    fileName.c_str(), "w", 0));
    ...
    ******************************************************************************************/
    int ProcessMonitor::runHDFSTest() {
        ...
        if (!File) {
            ...
            // retry since erydb can startup before hadoop is full up
            int retry = 0;
            for (; retry < 12; retry++) {
    ...
    *
    * purpose:	Update local erydb Config File
    *
    ******************************************************************************************/
    ...
    ******************************************************************************************/
    int ProcessMonitor::runUpgrade(std::string mysqlpw) {
        ...
        for (int i = 0; i < 10; i++) {
            //run upgrade script
            string cmd = startup::StartUp::installDir() + "/bin/upgrade-erydb.sh doupgrade --password=" +
                mysqlpw + " --installdir=" + startup::StartUp::installDir() + "  > " + "/tmp/upgrade-erydb.log 2>&1";
            system(cmd.c_str());

            cmd = "/tmp/upgrade-erydb.log";
            if (oam.checkLogStatus(cmd, "OK")) {
                log.writeLog(__LINE__, "upgrade-erydb.sh: Successful return", LOG_TYPE_DEBUG);
                return oam::API_SUCCESS;
            } else {
                if (oam.checkLogStatus(cmd, "ERROR 1045")) {
                    log.writeLog(__LINE__, "upgrade-erydb.sh: Missing Password error, return success", LOG_TYPE_DEBUG);
                    return oam::API_SUCCESS;
                }

                log.writeLog(__LINE__, "upgrade-erydb.sh: Error return, check log /tmp/upgrade-status.log", LOG_TYPE_ERROR);
    ...
    int ProcessMonitor::changeMyCnf(std::string type) {
        ...
        if (type == "master") {
            ...
            while (file.getline(line, 200)) {
                ...
                pos = buf.find("erydb_local_query=1", 0);
                if (pos != string::npos && pos == 0) {
                    buf = "# erydb_local_query=1";
                }
            ...
            for (; pt != systemModuleTypeConfig.moduletypeconfig[i].ModuleNetworkList.end(); pt++) {
                ...
                while (true)	// need in case there is a password retry
                {
                    for (; pt1 != (*pt).hostConfigList.end(); pt1++) {
                        ...
                        string logFile = "/tmp/master-rep-erydb-" + moduleName + ".log";
                        string cmd = startup::StartUp::installDir() + "/bin/master-rep-erydb.sh --password=" +
                        ...
                        if (oam.checkLogStatus(logFile, "ERROR 1045")) {
                            if (passwordError) {
                                log.writeLog(__LINE__, "master-rep-erydb.sh:  MySQL Password Error", LOG_TYPE_ERROR);
                                return oam::API_FAILURE;
                            }

                            log.writeLog(__LINE__, "master-rep-erydb.sh: Missing Password error, go check for a password and retry", LOG_TYPE_DEBUG);
                            passwordError = true;
                            break;
                        } else {
                            if (oam.checkLogStatus(logFile, "OK"))
                                log.writeLog(__LINE__, "master-rep-erydb.sh: Successful return for node " + moduleName, LOG_TYPE_DEBUG);
                            else {
                                log.writeLog(__LINE__, "master-rep-erydb.sh: Error return, check log " + logFile, LOG_TYPE_ERROR);
                                return oam::API_FAILURE;
                            }
                        }
                    }
    ...
    ******************************************************************************************/
    int ProcessMonitor::runSlaveRep(std::string& masterLogFile, std::string& masterLogPos, std::string& port) {
        ...
        bool passwordError = false;
        while (true) {
            string cmd = startup::StartUp::installDir() + "/bin/slave-rep-erydb.sh --password=" +
                mysqlpw + " --installdir=" + startup::StartUp::installDir() + " --masteripaddr=" + masterIPAddress + " --masterlogfile=" + masterLogFile + " --masterlogpos=" + masterLogPos + +" --port=" + port + "  >   /tmp/slave-rep-erydb.log 2>&1";

            log.writeLog(__LINE__, "cmd = " + cmd, LOG_TYPE_DEBUG);

            system(cmd.c_str());

            string logFile = "/tmp/slave-rep-erydb.log";

            if (oam.checkLogStatus(logFile, "ERROR 1045")) {
                if (passwordError) {
                    log.writeLog(__LINE__, "slave-rep-erydb.sh:  MySQL Password Error", LOG_TYPE_ERROR);
                    return oam::API_FAILURE;
                }

                log.writeLog(__LINE__, "slave-rep-erydb.sh: Missing Password error, go check for a password and retry", LOG_TYPE_DEBUG);
                passwordError = true;
            } else {
                if (oam.checkLogStatus(logFile, "OK")) {
                    log.writeLog(__LINE__, "slave-rep-erydb.sh: Successful return", LOG_TYPE_DEBUG);
                    return oam::API_SUCCESS;
                } else {
                    log.writeLog(__LINE__, "slave-rep-erydb.sh: Error return, check log /tmp/slave-rep-erydb.log", LOG_TYPE_ERROR);
                    return oam::API_FAILURE;
                }
            }
    ...
    ******************************************************************************************/
    int ProcessMonitor::runDisableRep() {
        ...
        string cmd = startup::StartUp::installDir() + "/bin/disable-rep-erydb.sh --password=" +
            mysqlpw + " --installdir=" + startup::StartUp::installDir() + "  >   /tmp/disable-rep-erydb.log 2>&1";

        log.writeLog(__LINE__, "cmd = " + cmd, LOG_TYPE_DEBUG);

        system(cmd.c_str());

        cmd = "/tmp/disable-rep-erydb.log";
        if (oam.checkLogStatus(cmd, "OK")) {
            log.writeLog(__LINE__, "disable-rep-erydb.sh: Successful return", LOG_TYPE_DEBUG);
            return oam::API_SUCCESS;
        } else {
            if (oam.checkLogStatus(cmd, "ERROR 1045")) {
                log.writeLog(__LINE__, "disable-rep-erydb.sh: Missing Password error, return success", LOG_TYPE_DEBUG);
                return oam::API_SUCCESS;
            }

            log.writeLog(__LINE__, "disable-rep-erydb.sh: Error return, check log /tmp/disable-rep-erydb.log", LOG_TYPE_ERROR);
            return oam::API_FAILURE;
        }
                        ...
                        if (currentIPAddr == "stopped") {
                            ...
                            if (currentIPAddr == "terminated") {
                                ...
                                if (currentIPAddr != IPAddr) {
                                    log.writeLog(__LINE__, "Module is Running: '" + moduleName + "' / Instance '" + instanceID + "' current IP being reconfigured in erydb.xml. old = " + IPAddr + ", new = " + currentIPAddr, LOG_TYPE_DEBUG);

                                    // update the erydb.xml with the new IP Address
                                    string cmd = "sed -i s/" + IPAddr + "/" + currentIPAddr + "/g /usr/local/erydb/etc/erydb.xml";
                                    system(cmd.c_str());
                                } else
    ...
    int ProcessMonitor::checkDataMount() {
        ...
        for (int retry = 0; retry < 10; retry++) {
            try {
                ...
                if (boost::get<1>(t) == 0) {
                    log.writeLog(__LINE__, "getStorageConfig return: No dbroots are configured in erydb.xml file", LOG_TYPE_WARNING);
                    return API_INVALID_PARAMETER;
                }
        ...
        if (dbrootList.size() == 0) {
            log.writeLog(__LINE__, "No dbroots are configured in erydb.xml file", LOG_TYPE_WARNING);
            return API_INVALID_PARAMETER;
        }
        ...
        while (p != dbrootList.end()) {
            ...
            if (GlusterConfig == "n") {
                ...
                int status = system(cmd.c_str());
                if (WEXITSTATUS(status) != 0) {	// not mounted, mount
