...
int main(int argc, char **argv) {
    ...
    if (gOAMParentModuleFlag) {
        if ((config.OAMStandbyName() != oam::UnassignedName) &&
            ...
            if (count >= 120) {
                log.writeLog(__LINE__, "Standby PM not responding, erydb shutting down", LOG_TYPE_CRITICAL);
                //Set the alarm
            ...
            if (parentOAMModule != config.moduleName()) {
                ...
                // get updated erydb.xml, retry in case ProcMgr isn't up yet
                if (!HDFS) {
                    int count = 0;
                    while (true) {
                        try {
                    ...
                    //re-read local system info with new erydb.xml
                    sleep(1);
                    MonitorConfig config;
                }
        ...
        // not active Parent, get updated erydb.xml, retry in case ProcMgr isn't up yet
        if (!HDFS) {
            int count = 0;
            while (true) {
                try {
    ...
    if (config.moduleType() == "pm") {
        ...
        if (retry == 20) {
            log.writeLog(__LINE__, "Check DB mounts failed, erydb shutting down", LOG_TYPE_CRITICAL);
            //Set the alarm
            aMonitor.sendAlarm(config.moduleName().c_str(), STARTUP_DIAGNOTICS_FAILURE, SET);
            sleep(1);
            string cmd = startup::StartUp::installDir() + "/bin/erydb stop > /dev/null 2>&1";
    ...
    if (gOAMParentModuleFlag &&
        ...
        if (standbyPM != "") {
            ...
            log.writeLog(__LINE__, "erydb.xml Standby OAM updated : " + standbyPM + ":" + standbyIPaddr, LOG_TYPE_DEBUG);
    ...
    if (!gOAMParentModuleFlag && config.moduleType() == "pm") {
        ...
        while (true) {
            try {
                ...
                log.writeLog(__LINE__, "Problem getting the ParentOAMModuleName key from the Erydb System Configuration file", LOG_TYPE_CRITICAL);
                exit(1);
            }
        }
...
static void mysqlMonitorThread(MonitorConfig config) {
    ...
    while (true) {
        ...
        try {
            oam.actionMysqlerydb(MYSQL_STATUS);
        } catch (...) {
        }
    ...
    while (true) {
        ...
        for (; listPtr != aPtr->end(); ++listPtr) {
            ...
            if (((kill((*listPtr).processID, 0)) != 0 && (*listPtr).state == oam::ACTIVE) ||
                ...
                log.writeLog(__LINE__, "*****erydb Process Restarting: " + (*listPtr).ProcessName + ", old PID = " + oam.itoa((*listPtr).processID), LOG_TYPE_CRITICAL);
                ...
                if ((*listPtr).dieCounter >= processRestartCount ||
                    ...
                    //Log this event 
                    log.writeLog(__LINE__, "erydb Process " + (*listPtr).ProcessName + restartStatus, LOG_TYPE_INFO);
                }
            }
        }
        delayCount++;
