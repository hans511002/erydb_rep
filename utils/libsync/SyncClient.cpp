...
namespace SYNC
{
	...
	SyncWEClient::SyncWEClient(SyncClient * _syncMgr, int PrgmID, uint16_t pmId) :syncMgr(_syncMgr), fPrgmID(PrgmID), fpmId(pmId), isError(false)
	{
		closingConnection = 0;
		Setup();
	}

	SyncWEClient::~SyncWEClient()
	{
		close();
	}

	void SyncWEClient::Setup()
	{
		fBusy = (true);
		...
		try
		{
			if (cl->connect())
			{
				...
				//ostringstream oss;
				//oss << "SyncClient: connected to " << fServer + " and this = " << this << " and pmcount is now " << pmCount;
	...
	int SyncWEClient::close()
	{
		...
		thReader->join();
		//ostringstream oss;
		//oss << "SyncClient: closed connection to wes and this = " << this << " and pmcount is now " << pmCount;
		//writeToLog(fPrgmID,__FILE__, __LINE__, oss.str() , LOG_TYPE_DEBUG);
		return 0;
	}
	void SyncWEClient::operator()() {
		SBS sbs;
		try {
			while (fBusy)
			{
	...
	void SyncWEClient::write(const messageqcpp::ByteStream &msg)
	{
		client->write(msg);
	}
