...
inline void IOSocket::sa(const sockaddr* sa)
	...
	if (fSocket)
		fSocket->sa( sa ); }
inline void IOSocket::open() { erydbassert(fSocket); fSocket->open(); }
...
inline const SBS IOSocket::read(const struct timespec* timeout, bool* isTimeOut, Stats *stats) const
	{ erydbassert(fSocket); return fSocket->read(timeout, isTimeOut, stats); }
inline void IOSocket::write(const ByteStream& msg, Stats *stats) const { erydbassert(fSocket); fSocket->write(msg, stats); }
inline void IOSocket::write_raw(const ByteStream& msg, Stats *stats) const { erydbassert(fSocket); fSocket->write_raw(msg, stats); }
inline void IOSocket::write(SBS msg, Stats *stats) const { erydbassert(fSocket); fSocket->write(msg, stats); }
inline const SocketParms IOSocket::socketParms() const { erydbassert(fSocket); return fSocket->socketParms(); }
inline void IOSocket::socketParms(const SocketParms& socketParms) { erydbassert(fSocket); fSocket->socketParms(socketParms); }
