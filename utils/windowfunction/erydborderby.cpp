...
//  $Id: erydborderby.cpp 3932 2013-06-25 16:08:10Z xlou $
...
void IdbOrderBy::initialize(const RowGroup& rg)
{
	...
	{
		cerr << ERYDBErrorInfo::instance()->errorMsg(fErrorCode)
			 << " @" << __FILE__ << ":" << __LINE__;
		throw ERYDBExcept(fErrorCode);
	}
...
bool EqualCompData::operator()(Row::Pointer a, Row::Pointer b)
{
	...
	{
		erydbSystemCatalog::ColDataType type = fRow1.getColType(*i);
		switch (type)
		{
			case erydbSystemCatalog::TINYINT:
			case erydbSystemCatalog::SMALLINT:
			...
			default:
			{
				...
				cerr << ERYDBErrorInfo::instance()->errorMsg(ec, type)
					 << " @" << __FILE__ << ":" << __LINE__;
				throw ERYDBExcept(ERYDBErrorInfo::instance()->errorMsg(ec, type), ec);
				break;
			}
		}
	}
