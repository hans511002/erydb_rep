...
#
   ...
   erydbDB=`cat $dirName/testInfo.txt | grep testDB |awk -F"=" '{print $2}'`
   ...
   swVersion=`cat $dirName/1/geterydbsoftwareinfo.log |grep Version |awk -F" " '{print $3}'`
   swRelease=`cat $dirName/1/geterydbsoftwareinfo.log |grep Release |awk -F" " '{print $3}'`
#
   fileName=`basename $scriptName`
   fileExt=${scriptName##*.}
   testRunDesc=""
   stackName=`cat 1/erydb.xml |grep "<SystemName>"|sed "s/</>/g"|awk -F">" '{print $3}'`
   stackConfig=""
#
   IOType=$grpTestType
#
...
#
      ...
      while read c1 c2 c3 idx qNum restofLine; do
         st=`cat tmp2.txt |grep "erydbStart ( ${idx} ${qNum} )" |awk -F" " '{print $6 " " $7}'`
         et=`cat tmp2.txt |grep "erydbEnd ( ${idx} ${qNum} )" |awk -F" " '{print $6 " " $7}'`
         qstats=`cat tmp2.txt |grep "erydbStats ( ${idx} ${qNum} )"`
...
#
   ...
   echo $testID\|$testRunID\|$testRunDesc\|$execServer\|$stackName\|$numDM\|$numUM\|$numPM\|$erydbDB\|$swVersion.$swRelease\|$grpTestNum\|$fileName\|$iterations\|$sessions\|$IOType\|$numStmts\|$numStmtsProcessed\|$runCompleted\| > testResultSummary.txt
   rm -f tmp*.txt
...
#
   ...
   echo $testID\|$testRunID\|$testRunDesc\|$execServer\|$stackName\|$numDM\|$numUM\|$numPM\|$erydbDB\|$fileName\|$numTables\|$tablesLoaded\|$runCompleted\|$rowCntMatched\|$st\|$et\| > testResultSummary.txt
   rm -f tmp*.txt
}
#=========================================================================================
