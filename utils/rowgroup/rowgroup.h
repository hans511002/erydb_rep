...
class Row
{
	public:
		...
		inline execplan::erydbSystemCatalog::ColDataType getColType(uint32_t colIndex) const;
		inline execplan::erydbSystemCatalog::ColDataType* getColTypes();
		inline const execplan::erydbSystemCatalog::ColDataType* getColTypes() const;
	...
	private:
		...
		uint32_t *offsets;
		uint32_t *colWidths;
		execplan::erydbSystemCatalog::ColDataType *types;
		uint8_t *data;
		uint32_t *scale;
		uint32_t *precision;
...
inline execplan::erydbSystemCatalog::ColDataType Row::getColType(uint32_t colIndex) const
{
	return types[colIndex];
}

inline execplan::erydbSystemCatalog::ColDataType* Row::getColTypes()
{
	return types;
}

inline const execplan::erydbSystemCatalog::ColDataType* Row::getColTypes() const
{
	return types;
}
...
{
	if (UNLIKELY(types[srcIndex] == execplan::erydbSystemCatalog::VARBINARY))
	...
	else if (UNLIKELY(types[srcIndex] == execplan::erydbSystemCatalog::LONGDOUBLE))
...
public:
	...
	*/
	...
	RowGroup(uint32_t colCount,
			...
			const std::vector<execplan::erydbSystemCatalog::ColDataType> &colTypes,
	...
	inline execplan::erydbSystemCatalog::ColDataType getColType(uint32_t colIndex) const;
	inline const std::vector<execplan::erydbSystemCatalog::ColDataType>& getColTypes() const;
	inline std::vector<execplan::erydbSystemCatalog::ColDataType>& getColTypes();
...
//	void convertToStringTableInPlace();
	...
	*/
	...
	void addToSysDataList(execplan::erydbSystemCatalog::NJLSysDataList& sysDataList);
...
private:
	...
	std::vector<execplan::erydbSystemCatalog::ColDataType> types;
...
void RowGroup::initRow(Row *r, bool forceInlineData) const
{
	...
	if (LIKELY(!types.empty())) {
		...
		r->types = (execplan::erydbSystemCatalog::ColDataType *) &(types[0]);
...
{
	return ((getColumnWidth(colIndex) <= 7 && types[colIndex] == execplan::erydbSystemCatalog::VARCHAR) ||
		(getColumnWidth(colIndex) <= 8 && types[colIndex] == execplan::erydbSystemCatalog::CHAR));
}
...
inline bool RowGroup::isLongString(uint32_t colIndex) const
{
	return ((getColumnWidth(colIndex) > 7 && types[colIndex] == execplan::erydbSystemCatalog::VARCHAR) ||
		(getColumnWidth(colIndex) > 8 && types[colIndex] == execplan::erydbSystemCatalog::CHAR) ||
		types[colIndex] == execplan::erydbSystemCatalog::VARBINARY);
}
...
inline execplan::erydbSystemCatalog::ColDataType RowGroup::getColType(uint32_t colIndex) const
{
	return types[colIndex];
}

inline const std::vector<execplan::erydbSystemCatalog::ColDataType>& RowGroup::getColTypes() const
{
	return types;
}

inline std::vector<execplan::erydbSystemCatalog::ColDataType>& RowGroup::getColTypes()
{
	return types;
}
...
inline void copyRow(const Row &in, Row *out, uint32_t colCount)
{
	...
	for (uint32_t i = 0; i < colCount; i++) {
		if (UNLIKELY(in.getColTypes()[i] == execplan::erydbSystemCatalog::VARBINARY))
		...
		else if (UNLIKELY(in.getColTypes()[i] == execplan::erydbSystemCatalog::LONGDOUBLE))
