...
RowGroup::RowGroup(uint32_t colCount,
	...
	const vector<erydbSystemCatalog::ColDataType> &colTypes,
...
void RowGroup::serialize(ByteStream &bs) const
{
	...
	serializeInlineVector<erydbSystemCatalog::ColDataType>(bs, types);
...
void RowGroup::deserialize(ByteStream &bs)
{
	...
	deserializeInlineVector<erydbSystemCatalog::ColDataType>(bs, types);
...
void applyMapping(const int *mapping, const Row &in, Row *out)
{
	...
	for (i = 0; i < in.getColumnCount(); i++)
		if (mapping[i] != -1)
		{
			...
			else if (UNLIKELY(in.getColTypes()[i] == execplan::erydbSystemCatalog::VARBINARY))
