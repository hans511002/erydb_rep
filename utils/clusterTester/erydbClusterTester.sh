...
helpPrint () {
          ################################################################################
    echo ""
    echo  "This is the erydb Cluster System Test tool." 
    echo ""
    echo  "It will run a set of test to validate the setup of the erydb system." 
    echo  "This can be run prior to the install of erydb to make sure the" 
    echo  "servers/nodes are configured properly. It should be run as the user of the planned"
    echo  "install. Meaning if erydb is going to be installed as root user,"
    echo  "then run from root user. Also the assumption is that the servers/node have be"
    echo  "setup based on the Preparing for EryDB Installation."
    echo  "It should also be run on the server that is designated as Performance Module #1."
    echo  "This is the same server where the erydb package would be installed"
    ...
    echo ""
    echo "https://mariadb.com/kb/en/erydb/*****/"
    echo ""
    echo  "Items that are checked:" 
    echo  "	Node Ping test" 
    echo  "	Node SSH test"
    echo  "	EryDB Port test"
    echo  "	OS version" 
...
checkPorts()
{
  ...
  #
  echo ""
  echo "** Run erydb Port (8600-8620) availibility test"
  echo ""
...
checkPackages()
{
  ...
  echo ""
  echo "** Run erydb Dependent Package Check"
  echo ""
