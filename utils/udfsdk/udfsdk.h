...
/** 
 * erydb interface for writing a user defined function (UDF).
 *
 * The basic steps are:
 *
 ...
 * 5. put the library in /usr/local/erydb/lib of all modules
 * 6. restart erydb
 * 7. Register the new functions with the commands like:
 *
 *    CREATE FUNCTION erydb_add returns REAL soname 'libudfsdk.so';
 *    CREATE FUNCTION erydb_isnull returns BOOL soname 'libudfsdk.so';
 *
 * The UDF functions run distributedly in the EryDB engine. The evaluation
...
/**
 * Example: ERYDB_add (args1, args2)
 *
 * ERYDB_add takes two arguments of any data type. It returns a double result.
 *
 ...
 * parent class Func, which will be called by ERYDB function and expression (F&E)
 ...
 * to evaluate the function ERYDB_add.
 *
 * select ERYDB_add(int1, int2) from t1; 
 * getDoubleVal() is called, because the result type of ERYDB_add is DOUBLE(real).
 *
 * select substr(string1, int1, ERYDB_add(int1+int2));
 * getIntVal() will be called, because ERYDB_add() is passed as the third argument 
 * to substr function, and an integer result is expected.
 *
 * If one API is not implemented but called for a function, ERYDB-5001 error will
 * be returned.
 */
class ERYDB_add : public funcexp::Func
{
public:
	/*
	 * Constructor. Pass the function name to the base constructor. 
	 */
	ERYDB_add() : Func("erydb_add") {}
	 ...
	 *       can be ignored. Struct ColType is defined in ~/dbcon/execplan/erydbsystemcatalog.h
	 ...
	 */
	execplan::erydbSystemCatalog::ColType operationType(funcexp::FunctionParm& fp, execplan::erydbSystemCatalog::ColType& resultType);
	 ...
	 *       in ERYDB_isnull() function example.
	 ...
	 *
	 * ERYDB_Decimal is defined in ~/execplan/treenode.h
	 */
	virtual execplan::ERYDB_Decimal getDecimalVal(rowgroup::Row& row,
								funcexp::FunctionParm& fp,
								bool& isNull,
								execplan::erydbSystemCatalog::ColType& op_ct);
...
/**
 * Example: ERYDB_isnull(arg1)
 *
 * The purpose of this example is to demostrate the NULL handling in the UDF interface
 */
class ERYDB_isnull : public funcexp::Func
{
public:
	/*
	 * Constructor. Pass the function name to the base constructor. 
	 */
	ERYDB_isnull() : Func("erydb_isnull") {}
	
	/*
	 * Destructor. ERYDB_add does not need to do anything here to clean up.
	 */
	virtual ~ERYDB_isnull() {}
