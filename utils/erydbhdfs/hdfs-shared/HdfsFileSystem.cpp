...
HdfsFileSystem::HdfsFileSystem() :
	ERYDBFileSystem( ERYDBFileSystem::HDFS )
{
	m_fs = HdfsFsCache::fs();
}
...
int HdfsFileSystem::mkdir(const char *pathname)
{
	...
	if( ERYDBLogger::isEnabled() )
		ERYDBLogger::logFSop( HDFS, "mkdir", pathname, this, ret);
...
int HdfsFileSystem::remove(const char *pathname)
{
	...
	if( ERYDBLogger::isEnabled() )
		ERYDBLogger::logFSop( HDFS, "remove", pathname, this, ret);
...
int HdfsFileSystem::rename(const char *oldpath, const char *newpath)
{
	...
	if( ERYDBLogger::isEnabled() )
		ERYDBLogger::logFSop2( HDFS, "rename", oldpath, newpath, this, ret);
...
off64_t HdfsFileSystem::size(const char* path) const
{
	...
	if( ERYDBLogger::isEnabled() )
		ERYDBLogger::logFSop( HDFS, "fs:size", path, this, retval);

	return retval;
}

size_t readFillBuffer(
	erydbdatafile::ERYDBDataFile* pFile,
	char*   buffer,
	size_t  bytesReq)
{
	char*   pBuf = buffer;
...
off64_t HdfsFileSystem::compressedSize(const char *path) const
{
    ...
    try
    {
        pFile = ERYDBDataFile::open(ERYDBDataFile::HDFS, path, "r", 0);
        ...
        compress::ERYDBCompressInterface decompressor;

        char hdr1[compress::ERYDBCompressInterface::HDR_BUF_LEN];
        nBytes = readFillBuffer( pFile,hdr1,compress::ERYDBCompressInterface::HDR_BUF_LEN);
        if ( nBytes != compress::ERYDBCompressInterface::HDR_BUF_LEN )
        {
            delete pFile;
            return -1;
        }
        ...
        int64_t ptrSecSize = decompressor.getHdrSize(hdr1) - compress::ERYDBCompressInterface::HDR_BUF_LEN;
...
int HdfsFileSystem::copyFile(const char* srcPath, const char* destPath) const
{
	...
	while (ret != 0 && n++ < 25)
	{
		...
		if( ERYDBLogger::isEnabled() )
			ERYDBLogger::logFSop2( HDFS, "copyFile-retry", srcPath, destPath, this, ret);

		ret = hdfsCopy(m_fs, srcPath, m_fs, destPath);
	}

	if( ERYDBLogger::isEnabled() )
		ERYDBLogger::logFSop2( HDFS, "copyFile", srcPath, destPath, this, ret);
