...
erydbSystemCatalog::ColType Func_greatest::operationType(FunctionParm& fp, erydbSystemCatalog::ColType& resultType)
{
						...
						execplan::erydbSystemCatalog::ColType& op_ct)
{
	...
	for (uint32_t i = 1; i < fp.size(); i++)
	{
		...
		int tmp = utf8::erydb_strcoll(greatestStr.c_str(), str1.c_str());
		if ( tmp < 0 )
...
ERYDB_Decimal Func_greatest::getDecimalVal(Row& row,
							FunctionParm& fp,
							bool& isNull,
