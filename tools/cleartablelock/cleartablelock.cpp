...
namespace
{
    ...
    void usage()
    {
        std::cout << "Usage: cleartablelock lockID" << std::endl
            ...
            << " -l clearLockOnly" << std::endl
            << " -h rollbackOnly" << std::endl;
    }
    ...
    //   tableLockID -
    //------------------------------------------------------------------------------
    void logInitStatus(const std::string& tableName,uint64_t tableLockID)
    {
    ...
    int constructPMList(std::vector<uint16_t>& pmList)
    {
        try
        {
            ...
            for (unsigned i = 0; i < pmCount; i++)
            {
                ...
                int moduleID = ::atoi( devName.substr(oam::MAX_MODULE_TYPE_SIZE,oam::MAX_MODULE_ID_SIZE).c_str());
    ...
    int constructPMList(const std::vector<uint16_t>& dbRootList, std::vector<uint16_t>& pmList)
    {
        ...
        } catch (std::exception& ex)
        {
            std::cout << "Error mapping DBRoot " << dbRoot << " to a PM.  " <<
                ex.what() << ".  Verify that this DBRoot is in erydb.xml." <<
                std::endl;
            return 5;
        } catch (...)
        {
            std::cout << "Error mapping DBRoot " << dbRoot << " to a PM.  " <<
                "Unknown exception.  Verify that this DBRoot is in erydb.xml" <<
                std::endl;
            return 5;
        }
    ...
    int createMsgQueClts(const std::vector<uint16_t>& pmList, std::vector<boost::shared_ptr<messageqcpp::MessageQueueClient> >& msgQueClts, std::string& errMsg)
    {
        ...
        try
        {
            ...
            for (unsigned k = 0; k < pmList.size(); k++)
            {
                ...
                messageqcpp::MessageQueueClient* cl = new messageqcpp::MessageQueueClient(srvName);
                msgQueClts.push_back(boost::shared_ptr<messageqcpp::MessageQueueClient>(cl));
            }
        } catch (std::exception& ex)
        {
    ...
    //                 code is returned.
    //------------------------------------------------------------------------------
    int execBulkRollbackReq(const std::vector<boost::shared_ptr<messageqcpp::MessageQueueClient> >& msgQueClts, const std::vector<uint16_t>& pmList, BRM::DBRM* brm, const BRM::TableLockInfo& tInfo,const std::string& tblName, bool rollbackOnly, std::string& errMsg)
    {
        errMsg.clear();
        try
        {
            if (!rollbackOnly)
            {
                ...
                int32_t   sessionID = -1;
                int32_t   transID = -1;
                bool      ownerChanged = brm->changeOwner(tInfo.id, processName, processID, sessionID, transID);
                if (!ownerChanged)
                {
...
int main(int argc, char** argv)
{
    ...
    if (!clearLockOnly)
    {
        std::vector<boost::shared_ptr<messageqcpp::MessageQueueClient> > msgQueClts;
        ...
        //----------------------------------------------------------------------
        rc = execFileCleanupReq(msgQueClts, pmList, &brm, tInfo, tblName.toString(), errMsg);
        if (rc != 0)
        {
