...
public:
	...
	const erydbSystemCatalog::ColType& ColumnType() { return fColType;}
...
// private:
	...
	OpType_t fOpType;
	erydbSystemCatalog::ColType fColType;
	uint32_t fSessionId;
	uint32_t fBOP;
	uint32_t fCOP1;
	uint32_t fCOP2;
...
{
 	boost::shared_ptr<erydbSystemCatalog> cat = execplan::erydbSystemCatalog::makeerydbSystemCatalog(getpid());
...
{
    if (DataType()==erydbSystemCatalog::BIT ||
    	DataType()==erydbSystemCatalog::TINYINT ||
    	DataType()==erydbSystemCatalog::SMALLINT ||
    	DataType()==erydbSystemCatalog::MEDINT ||
    	DataType()==erydbSystemCatalog::INT ||
    	DataType()==erydbSystemCatalog::DATE ||
    	DataType()==erydbSystemCatalog::BIGINT ||
    	DataType()==erydbSystemCatalog::DATETIME ||
        DataType()==erydbSystemCatalog::UTINYINT ||
        DataType()==erydbSystemCatalog::USMALLINT ||
        DataType()==erydbSystemCatalog::UMEDINT ||
        DataType()==erydbSystemCatalog::UINT ||
        DataType()==erydbSystemCatalog::UBIGINT)
	return true;

	if (DataType()==erydbSystemCatalog::CHAR && 1 == fColType.colWidth)
		return true;
				...
				uint32_t uniqueId)
{
	...
	erydbassert(fMsgHeader.Count > 0);
...
void doBatchOp_scan(OidOperation &OidOp)
{
	...
	DistributedEngineComm* dec = DistributedEngineComm::instance(rm);
	ThdFcn f1;
	boost::shared_ptr<erydbSystemCatalog> sysCat = execplan::erydbSystemCatalog::makeerydbSystemCatalog(getpid());
...
void doBatchOp_filt(OidOperation &OidOp)
{
	...
	DistributedEngineComm* dec = DistributedEngineComm::instance(rm);
	ThdFcn f1;
	boost::shared_ptr<erydbSystemCatalog> sysCat = erydbSystemCatalog::makeerydbSystemCatalog(getpid());
...
//first column is made into the first scan filter step including filters
	...
	uint32_t pid = getpid();
	pColScanStep scan(injs, outjs, dec, execplan::erydbSystemCatalog::makeerydbSystemCatalog(pid), scanOid, scanOid, sessionId, 0, sessionId, sessionId, sessionId, rm);
...
//The last filter step is added as a passthru step into the project list.
	...
	{
	  pColStep step(injs, outjs, dec, execplan::erydbSystemCatalog::makeerydbSystemCatalog(pid), (*op)->fOid, (*op)->fOid, sessionId, 0, sessionId, sessionId, sessionId, rm);
