...
int main(int argc, char* argv[]) {
    ...
    if (buildOption == SYSCATALOG_ONLY) {
        ...
        try {
            ...
            //@bug5554, make sure ERYDBPolicy matches the erydb.xml config
            try {
                string erydbConfigFile(startup::StartUp::installDir() + "/etc/erydb.xml");
                config::Config* sysConfig = config::Config::makeConfig(erydbConfigFile.c_str());
                string tmp = sysConfig->getConfig("Installation", "DBRootStorageType");
                if (boost::iequals(tmp, "hdfs")) {
                    // HDFS is configured
                    if (!ERYDBPolicy::useHdfs())  // error install plugin
                        throw runtime_error("HDFS is not enabled, installPlugin may have failed.");

                    else if (!ERYDBFileSystem::getFs(ERYDBDataFile::HDFS).filesystemIsUp())
