...
int main(int argc, char *argv[])
{
	...
	try {
		sysConfigOld = Config::makeConfig("./erydb.xml");		// system version
		sysConfigNew = Config::makeConfig("./erydb.xml.new");	// released version
	}
	catch(...)
	{
		cout << "ERROR: Problem reading erydb.xml files";
		exit(-1);
	}
	...
	try {
		...
		if ( !PMwithUM.empty() )
		{
			...
			{
				cout << "ERROR: Problem setting PMwithUM in the erydb System Configuration file" << endl;
				exit(-1);
			}
		}
	}
	...
	try {
		...
		if ( !MySQLRep.empty() )
		{
			...
			{
				cout << "ERROR: Problem setting MySQLRep in the erydb System Configuration file" << endl;
				exit(-1);
			}
		}
	}
	...
	try {
		...
		if ( oldFilesPerColumnPartition != newFilesPerColumnPartition )
		{
			...
			{
				cout << "ERROR: Problem setting FilesPerColumnPartition in the erydb System Configuration file" << endl;
				exit(-1);
			}
		}
	...
	try {
		...
		if ( !key.empty() )
		{
			...
			{
				cout << "ERROR: Problem setting Flags in the erydb System Configuration file" << endl;
				exit(-1);
			}
		}
	}
...
//		systemParentOAMModuleName = "pm2";
	...
	if ( !HA_IPadd.empty() ) {
		...
		{
			cout << "ERROR: Problem setting ProcMgr_HA from the erydb System Configuration file" << endl;
			exit(-1);
		}
	}
	...
	for ( int id = 1 ;; id ++ )
	{
		...
		if ( !CMP_IPadd.empty() ) {
			...
			{
				cout << "ERROR: Problem setting CMP from the erydb System Configuration file" << endl;
				exit(-1);
			}
		}
		else
	...
	switch ( IserverTypeInstall ) {
		case (oam::INSTALL_COMBINE_DM_UM_PM):	// combined #1 - dm/um/pm on a single server
		{
			...
			{
				cout << "ERROR: Problem setting RotatingDestination in the erydb System Configuration file" << endl;
				exit(-1);
			}
	...
	for ( unsigned int i = 0 ; i < sysModuleTypeConfig.moduletypeconfig.size(); i++)
	{
		...
		for( ; listPT != sysModuleTypeConfig.moduletypeconfig[i].ModuleNetworkList.end() ; listPT++)
		{
			...
			{
				cout << "ERROR: Problem setting ModuleDisableState in the erydb System Configuration file for " + moduleName << endl;
				exit(-1);
			}
			...
			for( unsigned int nicID=1 ; nicID < MAX_NIC+1 ; nicID++ )
			{
				...
				//exit out if parentOAMModuleIPAddr is NOT set, this means the System erydb.xml isn't configured
					if ( parentOAMModuleIPAddr == "0.0.0.0" ) {
						cout << "ERROR: System erydb.xml not configured" << endl;
						exit(-1);
					}
	...
	if (build3) {
		...
		if (build401)
		{
			...
			{
		//		cout << "ERROR: Problem setting Cloud Parameters from the erydb System Configuration file" << endl;
		//		exit(-1);
			}
		}
	...
	//		exit(-1);
		...
		try
		{
			...
			for( ; pt != dbrootConfigList.end() ; pt++)
			{
				...
				string pathID =  "/usr/local/erydb/data" + oam.itoa(id);
	...
	{ // pre 3.0 only
		...
		for( int i=1 ; i < DBRootCount+1 ; i++)
		{
			if ( DBRootStorageType != "local") {
				...
				{
					cout << "ERROR: Problem getting '" + DBRootStorageLocID + "' from the erydb System Configuration file" << endl;
					exit(-1);
				}
				...
				{
					cout << "ERROR: Problem setting '" + DBRootStorageLocID + "' in the erydb System Configuration file" << endl;
					exit(-1);
				}
			}
	...
	try {
		...
		if ( !ConcurrentTransactions.empty() )
		{
			...
			{
				cout << "ERROR: Problem setting ConcurrentTransactions in the erydb System Configuration file" << endl;
				exit(-1);
			}
		}
	}
	...
	try {
		...
		if ( !NetworkCompression.empty() )
		{
			...
			{
				cout << "ERROR: Problem setting NetworkCompression in the erydb System Configuration file" << endl;
				exit(-1);
			}
		}
	}
	...
	try {
		...
		if ( !CoreFileFlag.empty() )
		{
			...
			{
				cout << "ERROR: Problem setting CoreFileFlag in the erydb System Configuration file" << endl;
				exit(-1);
			}
		}
	}
	...
	try {
		...
		if ( !AllowDiskBasedJoin.empty() )
		{
			...
			{
				cout << "ERROR: Problem setting AllowDiskBasedJoin in the erydb System Configuration file" << endl;
				exit(-1);
			}
		}
	}
	...
	try {
		...
		if ( !Host.empty() )
		{
			...
			{
				cout << "ERROR: Problem setting QueryTele in the erydb System Configuration file" << endl;
				exit(-1);
			}
		}
	}
	...
	try {
		...
		if ( !AmazonAccessKey.empty() )
		{
			...
			{
				cout << "ERROR: Problem setting AmazonAccessKey in the erydb System Configuration file" << endl;
				exit(-1);
			}
		}
	}
	...
	try {
		...
		if ( !AmazonSecretKey.empty() )
		{
			...
			{
				cout << "ERROR: Problem setting AmazonSecretKey in the erydb System Configuration file" << endl;
				exit(-1);
			}
		}
	}
