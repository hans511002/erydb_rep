...
int main(int argc, char *argv[])
{
	...
	cout << endl;
	cout << "This is the erydb System Configuration tool." << endl;
	cout << "It will generate a erydb System specific erydb.xml file." << endl;
	...
	//get system name
	string systemName;
	Config* netConfig = Config::makeConfig("./systems/erydbSystems.xml");


	while(true)
	{
		prompt = "Enter erydb system name > ";
		...
		if (pcommand) {
			...
			try {
				...
				if ( systemCount == 0 ) {
					cout << "ERROR: SystemCount in ./systems/erydbSystems.xml equal to 0" << endl;
					exit(-1);
				}
			}
			catch(...)
			{
				cout << "ERROR: Problem getting SystemCount from ./systems/erydbSystems.xml" << endl;
				exit(-1);
			}
			...
			for ( int i = 1 ; i < systemCount+1 ; i++)
			{
				...
				{
					cout << "ERROR: Problem getting SystemName from ./systems/erydbSystems.xml" << endl;
					exit(-1);
				}
			...
			if ( !FOUND ) {
				cout << "ERROR: System Name '" + systemName + "' not in ./systems/erydbSystems.xml" << endl;
				continue;
			}
	...
	//determine which erydb.xml to use as a base
	while(true)
	{
		cout << endl;
		cout << "Enter the erydb.xml file do you want to use as a based version" << endl;
		cout << "Enter: 1 for System version (meaning copy from the system)" << endl; 
		cout << "       2 for Release version" << endl; 
		cout << "       3 for erydb.xml version already located in the systems directory" << endl; 
		...
		else
		{
			...
			switch(option) {
				case 1:	//get system erydb.xml
				{
					...
					cmd = "./remote_scp_get.sh " + parentOAMModuleIPAddr + " " + password + " /usr/local/erydb/etc/erydb.xml 0 ";
					rtnCode = system(cmd.c_str());
					if (rtnCode == 0) {
						cmd = "mv erydb.xml systems/" + systemName + "/.";
						rtnCode = system(cmd.c_str());
						if ( rtnCode != 0 ) {
							cout << "ERROR: No system erydb.xml found" << endl;
							continue;
						}
						else
							break;
				...
				case 2:	//get release erydb.xml
				{
					string release;
					while (true)
					{
						prompt = "Enter erydb Release number > ";
						pcommand = readline(prompt.c_str());
						if (!pcommand)
							continue;
						else 
						{
							...
							string cmd = "cd systems/" + systemName + ";rm -f erydb.xml;smbclient //cal6500/shared -Werydb -U" + oam::USERNAME + "%" + oam::PASSWORD + " -c 'cd Iterations/" + release + "/;prompt OFF;mget erydb.xml' > /dev/null 2>&1";
							int rtnCode = system(cmd.c_str());
							if (rtnCode != 0)
								cout << "FAILED: no Release erydb.xml found for " + release << endl;
							else
							{
								cmd = "cd systems/" + systemName + ";ls erydb.xml > /dev/null 2>&1";
								rtnCode = system(cmd.c_str());
								if (rtnCode != 0) {
									cout << "FAILED: no Release erydb.xml found for " + release  << endl;
									continue;
				...
				case 3:	//use erydb.xml alyread in system directory
					break;
	...
	setenv("ERYDB_HOME", env.c_str() , 1);
    Oam oam;

	Config* sysConfig = Config::makeConfig(env + "/erydb.xml");
	...
	{
		cout << "ERROR: Problem getting SingleServerInstall from the erydb System Configuration file" << endl;
		exit(-1);
	}
	...
	while(true)
	{
		...
		if ( singleServerInstall == "y" ) {
			...
			cout << "All erydb Processes will run on this single server." << endl;
		}
		else
		{
	...
	{
		cout << "ERROR: Problem setting DBRootStorageLoc in the erydb System Configuration file" << endl;
		exit(-1);
	}
	...
	{
		cout << "ERROR: Problem updating the erydb System Configuration file" << endl;
		exit(-1);
	}
	...
	while(true)
	{
		prompt = "Enter erydb Data Storage Mount Type [storage,local,nfs] (" + DBRootStorageType + ") > ";
		pcommand = readline(prompt.c_str());
		if (pcommand)
		{
	...
	while(true)
	{
		prompt = "Enter the Number of erydb Data Storage (DBRoots) areas (" + oam.itoa(DBRootCount) + ") > ";
		pcommand = readline(prompt.c_str());
		if (pcommand) {
	...
	for( int i=1 ; i < DBRootCount+1 ; i++)
	{
		...
		string pathID =  "/usr/local/erydb/data" + oam.itoa(i);
	...
	{
		cout << "ERROR: Problem setting OAMStorageType in the erydb System Configuration file" << endl;
		exit(-1);
	}
	...
	if ( OAMStorageType == "storage") {
		...
		{
			cout << "ERROR: Problem setting OAMStorageLoc in the erydb System Configuration file" << endl;
			exit(-1);
		}
	}
	...
	for ( unsigned int i = 0 ; i < sysModuleTypeConfig.moduletypeconfig.size(); i++)
	{
		...
		if ( moduleType == "um" || singleServerInstall == "y" ) {
			...
			{
				cout << "ERROR: Problem setting UserStorageType in the erydb System Configuration file" << endl;
				exit(-1);
			}
		}
		...
		for( int k=0 ; k < moduleCount ; k++, moduleID++ )
		{
			...
			if ( moduleType == "um" || singleServerInstall == "y" ) {
				...
				if ( UserStorageType == "storage") {
					...
					{
						cout << "ERROR: Problem setting UserStorageLoc in the erydb System Configuration file" << endl;
						exit(-1);
					}
				}
				else
	...
	for ( unsigned int i = 0 ; i < sysSwitchTypeConfig.switchtypeconfig.size(); i++)
	{
		...
		while(true)
		{
			...
			if (pcommand) {
				...
				{
					cout << "ERROR: Problem setting Switch Count in the erydb System Configuration file" << endl;
					exit(-1);
				}
			}
			break;
		...
		//add new entried to erydb System Config table for additional IP Addresses and Hostnames
	...
	for ( unsigned int i = 0 ; i < sysStorageTypeConfig.storagetypeconfig.size(); i++)
	{
		...
		while(true)
		{
			...
			if (pcommand) {
				...
				{
					cout << "ERROR: Problem setting Storage Count in the erydb System Configuration file" << endl;
					exit(-1);
				}
			}
			break;
	...
	{
		cout << "ERROR: Problem getting NMSIPAddress from erydb System Configuration file" << endl;
		exit(-1);
	}
	...
	{
		cout << "ERROR: Problem setting NMSIPAddress in the erydb System Configuration file" << endl;
		exit(-1);
	}
	...
	{
		cout << "ERROR: Problem getting transactionArchivePeriod from erydb System Configuration file" << endl;
		exit(-1);
	}
	...
	//
	//Update oerydbitmap in erydb.xml
	//

	try {
		sysConfig->setConfig("OIDManager", "OERYDBitmapFile", "/mnt/OAM/dbrm/oerydbitmap");
	}
	catch(...)
	{
		cout << "ERROR: Problem setting OERYDBitmapFile erydb System Configuration file" << endl;
		exit(-1);
	}
	...
	cout << endl << "Configure is successfuly completed, erydb.xml is located in systems/" + systemName << endl << endl;
}
