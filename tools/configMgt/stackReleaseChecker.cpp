...
int main(int argc, char *argv[])
{
   ...
   for( int i = 1; i < argc; i++ )
   {
      if( string("-h") == argv[i] ) {
			...
			cout << endl;
			cout << "The list of systems are located in './systems/erydbSystems.xml" << endl;
			cout << endl;
	...
	for ( int i = 1 ; i < systemCount+1 ; i++)
	{
		...
		if ( tempSystem == systemName || systemName == "all") {
			...
			if (!VERBOSE)
				...
				cmd = "./remote_command.sh " + installParentModuleHostName + " " + systemUser + " " + password + " '" + installDir + "/bin/eryadmin geterydbsoftware' 'tools' Error 10 " + debug_flag;

			cout << "***** erydb EryDB Package Release Information for stack '" << tempSystem << "' *****" << endl << endl;
			...
			cmd = "./remote_command.sh " + installParentModuleHostName + " " + systemUser + " " + password + " '" + installDir + "/bin/eryadmin getsystems' dummy Error 10 " + debug_flag;
