...
BatchInsertProc::BatchInsertProc(bool isAutocommitOn, uint32_t tableOid, execplan::erydbSystemCatalog::SCN txnId, BRM::DBRM* aDbrm) :
...
uint64_t BatchInsertProc::grabTableLock(int32_t sessionId)
{
	...
	{
		setError(dmlpackageprocessor::DMLPackageProcessor::INSERT_ERROR , ERYDBErrorInfo::instance()->errorMsg(ERR_HARD_FAILURE));
		return 0;
	}
	...
	if ( fTableLockid  == 0 )
	{
		...
		if (i >= numTries) //error out
		{
			...
			setError(dmlpackageprocessor::DMLPackageProcessor::TABLE_LOCK_ERROR , ERYDBErrorInfo::instance()->errorMsg(ERR_TABLE_LOCKED,args));
			return 0;										
		}
	}
	return fTableLockid;
...
void BatchInsertProc::buildLastPkg(messageqcpp::ByteStream& bs)
{
	...
	if (fErrorCode == dmlpackageprocessor::DMLPackageProcessor::ERYDBRANGE_WARNING)
		rt = 0;
	else
		rt = fErrorCode;
	bs.reset();
