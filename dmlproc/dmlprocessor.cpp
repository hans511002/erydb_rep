...
void PackageHandler::run()
{
	...
	try
	{
			...
			case dmlpackage::DML_INSERT:
				{
					...
					if (fConcurrentSupport) 
					{
						...
						if (fTableOid == 0)
						{
							...
							erydbSystemCatalog::ROPair roPair = fcsc->tableRID(tableName);
					...
					if (insertPkg.get_isBatchInsert())
					{
						...
						if (insertPkg.get_Logending() && insertPkg.get_Logging()) //only one batch need to be processed.
						{
							...
							if (rc == DMLPackageProcessor::ERYDBRANGE_WARNING)
							{
								result.result = dmlpackageprocessor::DMLPackageProcessor::ERYDBRANGE_WARNING;
								LoggingID logid( DMLLoggingId, insertPkg.get_SessionID(), fTxnid);
								logging::Message::Args args1;
								logging::Message msg(1);
						...
						else if (insertPkg.get_Logending()) //Last batch
						{
							...
							if (( rc == 0) || (rc == DMLPackageProcessor::ERYDBRANGE_WARNING))
							{
								//cout << " rc = " << rc << endl;
								batchProcessor->addPkg(bsSave);
			...
			case dmlpackage::DML_UPDATE:
				{
					...
					fQtc.postQueryTele(qts);
					// process it
					//@Bug 1341. Don't remove erydbsystemcatalog from this 
			...
			case dmlpackage::DML_DELETE:
				{
					...
					fQtc.postQueryTele(qts);
					// process it
					//@Bug 1341. Don't remove erydbsystemcatalog from this session to take advantage of cache.
			...
			case dmlpackage::DML_COMMAND:
				{
					...
					if (stmt == "CLEANUP")
					{
						execplan::erydbSystemCatalog::removeerydbSystemCatalog(commandPkg.get_SessionID());
						execplan::erydbSystemCatalog::removeerydbSystemCatalog(commandPkg.get_SessionID() | 0x80000000);
					}				
					else
					{
						// process it
		...
		else if (result.result == dmlpackageprocessor::DMLPackageProcessor::ERYDBRANGE_WARNING)
		{
			logging::LoggingID lid(21);
			logging::MessageLog ml(lid);
...
{
	csc = erydbSystemCatalog::makeerydbSystemCatalog();
...
void DMLProcessor::operator()()
{
	...
	try
    {
        ...
        for (;;)
        {
			//cout << "DMLProc is waiting for a erydb DML Package on " << fIos.getSockID() << endl;
			try
			{
...
//cout << "DMLProc received pkg. sessionid:type = " << sessionID <<":"<<(int)packageType << endl;
			...
			if (!fConcurrentSupport) 
			{
				...
				if (anyOtherActiveTransaction && (i >= numTries))
				{
					...
					results << ERYDBErrorInfo::instance()->errorMsg(ERR_ACTIVE_TRANSACTION, args);
					//@Bug 3854 Log to debug.log
					LoggingID logid(20, 0, 0);
					logging::Message::Args args1;
					logging::Message msg(1);
					args1.add(ERYDBErrorInfo::instance()->errorMsg(ERR_ACTIVE_TRANSACTION, args));
					msg.format( args1 );
					logging::Logger logger(logid.fSubsysID);
					logger.logMessage(LOG_TYPE_DEBUG, msg, logid);
