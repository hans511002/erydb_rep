...
public:
    PackageHandler(const messageqcpp::IOSocket& ios, boost::shared_ptr<messageqcpp::ByteStream> bs,
		...
		uint32_t sessionID, execplan::erydbSystemCatalog::SCN txnId, BRM::DBRM * aDbrm,
		const querytele::QueryTeleClient& qtc, boost::shared_ptr<execplan::erydbSystemCatalog> csc);
	~PackageHandler();

    void run();
	void rollbackPending();
	
	execplan::erydbSystemCatalog::SCN getTxnid() {return fTxnid;}
...
private:
    ...
    boost::shared_ptr<execplan::erydbSystemCatalog> fcsc;
	...
	typedef iterable_queue<execplan::erydbSystemCatalog::SCN> tableAccessQueue_t;
...
public:
     ...
     */
	 inline DMLResult processPackage(dmlpackage::erydbDMLPackage& cpackage)
	 {
		DMLResult result;
