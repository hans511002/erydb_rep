...
:: Checkout the server
bash %basedir%\genii\tools\reserveStacks\stack reserve srvbldwin1 nightly f:/erydb
...
EryDB64-ent.exe /S /D=f:\erydb
IF %ERRORLEVEL% NEQ 0 GOTO ERROR_HANDLER
cd \

echo Waiting for EryDB service to boot
call svcwait.bat
IF %ERRORLEVEL% NEQ 0 GOTO ERROR_HANDLER
echo EryDB service is ready
...
bash %basedir%\genii\tools\reserveStacks\stack release srvbldwin1 nightly f:/erydb
exit 1
