...
/******************************************************************************************
 * $Id: eryadmin.cpp 3110 2013-06-20 18:09:12Z dhill $
 *
 ******************************************************************************************/
...
{
    std::cout << "Received SIGTERM to terminate erydb Console..." << std::endl;
...
//	setupSignalHandlers();
	...
	}
  {
    ...
    {
        cout << endl << "erydb Admin Console" << endl;
        cout << "   enter 'help' for list of commands" << endl;
        cout << "   enter 'exit' to exit the erydb Command Console" << endl;
...
int processCommand(string* arguments)
{
    ...
    switch( CmdID )
    {
        ...
        case 3: // quit
        {
            ...
            logFile.close();
            cout << "Exiting the erydb Admin Console" << endl;
			...
			if (oid == 0)  // A table or column name was entered
			{
				...
				if (arguments[3] == "")
				{
					...
					try
					{
						...
						if (tableOid)
						{
							erydbSystemCatalog::RIDList rdlist = systemCatalogPtr->columnRIDs(tableName);
						...
						{
							cout << arguments[1] << "." << arguments[2] << " is not a erydb table" << endl;
							break;
						}
					}
				...
				else // A column name was given
				{
					...
					if (oid < 1)
					{
						cout << arguments[1] << "." << arguments[2] << "." << arguments[3] << " is not a erydb column" << endl;
						break;
					}
				...
				if (columnName.schema.size() == 0 || columnName.table.size() == 0 || columnName.column.size() == 0)
				{
					...
					if (columnName.schema.size() == 0 || columnName.table.size() == 0 || columnName.column.size() == 0)
					{
						...
						if (tableName.schema.size() == 0 || tableName.table.size() == 0)
						{
							...
							cout << "OID " << oid << " does not represent a table or column in erydb" << endl;
							break;
						}
						tableOid = oid;
						try
        ...
        case 16: // stopSystem - parameters: graceful flag, Ack flag
        {
			...
			cout << endl << "This command stops the processing of applications on all Modules within the erydb System" << endl;
    ...
    //					}
		...
		try
		{
			...
			{
				cmd = "pdsh -a '/" + startup::StartUp::installDir() + "/bin/erydb stop' > /tmp/cc-stop.pdsh 2>&1";
				system(cmd.c_str());
				if (oam.checkLogStatus("/tmp/cc-stop.pdsh", "exit") ) {
					cout << endl << "ERROR: Stopping erydb Service failure, check /tmp/cc-stop.pdsh. exit..." << endl;
				}
			}
			else
			{
				cmd = startup::StartUp::installDir() + "/bin/erydb stop > /tmp/status.log";
				system(cmd.c_str());
			}
		}
            	catch (exception& e)
			...
			if ( gracefulTemp == FORCEFUL )
			{
				cmd = startup::StartUp::installDir() + "/bin/erydb stop > /tmp/status.log";
				system(cmd.c_str());
				cout << endl << "   Successful shutdown of System (stopped local erydb service) " << endl << endl;
			}
	
			if (Failed.find("Connection refused") != string::npos)
			{
				cout << endl << "**** shutdownSystem Error : ProcessManager not Active, stopping erydb service" << endl;
				cmd = startup::StartUp::installDir() + "/bin/erydb stop > /tmp/status.log";
				system(cmd.c_str());
				cout << endl << "   Successful stop of local erydb service " << endl << endl;
			}
			else
			{
        ...
        case 18: // startSystem - parameters: Ack flag
        {
			...
			// if erydb service is down, then start system by starting all of the erydb services
			// this would be used after a shutdownSystem command
			// if erydb service is up, send message to ProcMgr to start system (which starts all processes)

			if (!oam.checkSystemRunning())
			{
				cout << "startSystem command, 'erydb' service is down, sending command to" << endl;
				cout << "start the 'erydb' service on all modules" << endl << endl;
				...
				if ( systemModuleNumber > 1 ) 
				{
					...
					//
					// perform start of erydb of other servers in the system
					//
					...
					if ( DBRootStorageType == "hdfs")
					{
						cmd = "pdsh -a '" + startup::StartUp::installDir() + "/bin/erydb restart' > /tmp/cc-restart.pdsh 2>&1";
						system(cmd.c_str());
						if (oam.checkLogStatus("/tmp/cc-restart.pdsh", "exit") ) {
							cout << endl << "ERROR: Restart erydb Service failure, check /tmp/cc-restart.pdsh. exit..." << endl;
							break;
						}
					}
					else
					{
						for( unsigned int i = 0 ; i < systemmoduletypeconfig.moduletypeconfig.size(); i++)
						{
								...
								++pt)
							{
								...
								{
									cmd = startup::StartUp::installDir() + "/bin/erydb restart > /tmp/start.log 2>&1";
									int rtnCode = system(cmd.c_str());
									if (geteuid() == 0 && WEXITSTATUS(rtnCode) != 0) 
									{
										cout << endl << "error with running 'erydb restart' on local module " << endl;
								...
								for( ; pt1 != (*pt).hostConfigList.end() ; pt1++)
								{
									...
									cmd = startup::StartUp::installDir() + "/bin/remote_command.sh " + (*pt1).IPAddr + " " + password + " '" + startup::StartUp::installDir() + "/bin/erydb restart' 0";
									int rtnCode = system(cmd.c_str());
									if (WEXITSTATUS(rtnCode) < 0) {
										cout << endl << "error with running 'erydb start' on module " + modulename << endl;
										cout << endl << "**** startSystem Failed" << endl;

										// stop local erydb service
										cmd = startup::StartUp::installDir() + "/bin/erydb stop > /tmp/stop.log 2>&1";
										system(cmd.c_str());
									...
									{
										if (rtnCode > 0) {
											cout << endl << "Invalid Password when running 'erydb start' on module " + modulename << ", can retry by providing password as the second argument" << endl;
											cout << endl << "**** startSystem Failed" << endl;
        ...
        case 19: // restartSystem - parameters: graceful flag, Ack flag
        {
			...
			if (!oam.checkSystemRunning())
			{
				...
				cout << "restartSystem command, 'erydb' service is down, sending command to" << endl;
				cout << "start the 'erydb' service on all modules" << endl << endl;
				...
				if ( systemModuleNumber > 1 ) 
				{
					...
					if ( DBRootStorageType == "hdfs")
					{
						...
						system(cmd.c_str());
						if (oam.checkLogStatus("/tmp/cc-restart.pdsh", "exit") ) {
							cout << endl << "ERROR: Restart erydb Service failue, check /tmp/cc-restart.pdsh. exit..." << endl;
							break;
						}
					}
					else
				...
				else
				{
					...
					int rtnCode = system(cmd.c_str());
					if (WEXITSTATUS(rtnCode) != 0) {
						cout << endl << "error with running 'erydb start' on local module " << endl;
						cout << endl << "**** restartSystem Failed" << endl;
						break;
					}
				}
...
//				break;
		...
		if ( MySQLRep == "y" && MySQLPasswordConfig == oam::UnassignedName ) {
			cout << endl;
			string prompt = "erydb Replication is enabled, is there a 'erydb' Password configured in " + HOME + "/.my.cnf  (y,n): ";
    ...
    //					}
        ...
        case 30: // getLogConfig
        {
            try
            {
                ...
                cout << endl << "erydb System Log Configuration Data" << endl << endl;
        ...
        case 32: // suspendDatabaseWrites
		{
			...
			cout << endl << "This command suspends the DDL/DML writes to the erydb Database" << endl;
            try
            {
    ...
    //					}
        ...
        {
			if ( arguments[1] != "y" ) {
				if (confirmPrompt("This command resumes the DDL/DML writes to the erydb Database"))
					break;
			}

			// resume writes to erydb Database
			...
			try{
				...
				cout << endl << "Resume erydb Database Writes Request successfully completed" << endl;
			}
			catch (exception& e)
			{
       ...
       case 42: 
        ...
        case 43: // assignElasticIPAddress
		{
			...
			{
				cout << "ERROR: Problem setting AmazonElasticModule in the erydb System Configuration file" << endl;
				break;
			}
		...
		if ( MySQLRep == "y" ) {
			string warning = "erydb Replication Feature is already enabled";
			// confirm request
			if (confirmPrompt(warning))
				break;
		}
		...
		if ( MySQLPasswordConfig == oam::UnassignedName ) {
			cout << endl;
			string prompt = "Is there a 'erydb' Password configured on the erydb Front-end Modules in " + HOME + "/.my.cnf (y,n): ";
			MySQLPasswordConfig = dataPrompt(prompt);
		}
		...
		try {
			...
			cout << endl << "   Successful Enabling of erydb Replication " << endl << endl;
			...
			cout << "   erydb Replication Master Node is " << PrimaryUMModuleName << endl << endl;
                }
                catch (exception& e)
                {
...
//do we needed this check????
			...
			if (moduletypeconfig.ModuleCount + moduleCount > licSize) {
				cout << endl << "**** addModule Failed :The license is only allowed for " << licSize << " machines. your have " << moduletypeconfig.ModuleCount<<" "<<
					moduletypeconfig.ModuleType<<"'s"<< endl;
				break;
			}
        ...
        case 49: // removeModule - parameters: Module name/type, number-of-modules
        {
			...
			if ( arguments[1].size() == 2 ) {
				...
				if ( arguments[3] != "y") {
					...
					string warning = "This command does a remove a module from the erydb System";
					// confirm request
					if (confirmPrompt(warning))
						break;
				}
			...
			else
			{
				...
				if ( arguments[2] != "y") {
					cout << endl << "!!!!! DESTRUCTIVE COMMAND !!!!!" << endl;
					string warning = "This command does a stop and remove a module from the erydb System";
					// confirm request
					if (confirmPrompt(warning))
						break;
				}
		...
		if ( MySQLRep == "n" ) {
			string warning = "erydb Replication Feature is already disable";
			// confirm request
			if (confirmPrompt(warning))
				break;
		}
		...
		try {
			...
			sleep(2);
                ...
                {
                    oam.disableMySQLRep();
			cout << endl << "   Successful Disable of erydb Replication " << endl;
                }
                catch (exception& e)
                {
		...
		else
		{
			// confirm request
			if ( arguments[2] != "y" ) {
				if (confirmPrompt("This command stops the processing of applications on a Module within the erydb System"))
					break;
			}
		}
	    ...
	    if ( SingleServerInstall == "y" ) {
            ...
            // confirm request
			if ( arguments[2] != "y" ) {
				if (confirmPrompt("This command starts the processing of applications on a Module within the erydb System"))
					break;
			}
...
int ProcessSupportCommand(int CommandID, std::string arguments[])
{
	...
	switch( CommandID )
    {
        ...
        case 1: // stopprocess - parameters: Process-name, Module-name, Graceful flag, Ack flag
        {
			...
			else
			{
				...
				if ( arguments[1] == "ProcessManager" ) {
            		if (confirmPrompt("ProcessManager is the Interface for the Console and should only be removed as part of a erydb Package installation"))
                		break;
				}
				else
				{
					if ( arguments[3] != "y" ) {
						getFlags(arguments, gracefulTemp, ackTemp, suspendAnswer, bNeedsConfirm);
						// confirm request
						if (confirmPrompt("This command stops the processing of an application on a Module within the erydb System"))
							break;
					}
				}
			}
        ...
        case 3: // restartprocess - parameters: Process-name, Module-name, Graceful flag, Ack flag
        {
            ...
            {
	            // confirm request
        	    if (confirmPrompt("This command restarts the processing of an application on a Module within the erydb System"))
                	break;
            }
        ...
        case 5: // rebootsystem - parameters: password
        {
			...
			if ( arguments[2] != "y") {
				cout << endl << "!!!!! DESTRUCTIVE COMMAND !!!!!" << endl;
				string warning = "This command stops the Processing of applications and reboots all modules within the erydb System";
				// confirm request
				if (confirmPrompt(warning))
					break;
			}
			...
			try
			{
				...
				else
				{
					...
					writeLog("End of a command session!!!");
					logFile.close();
					cout << endl << "Exiting the erydb Command Console" << endl;
					exit (0);
				}
			}
			catch(...)
        ...
        case 6: // rebootnode - parameters: module-name password
        {
			...
			if ( arguments[3] != "y") {
				cout << endl << "!!!!! DESTRUCTIVE COMMAND !!!!!" << endl;
				string warning = "This command reboots a node within the erydb System";
				// confirm request
				if (confirmPrompt(warning))
					break;
			}
        ...
        case 7: // stopdbrmprocess
        {
			if ( arguments[1] != "y" ) {
				// confirm request
				if (confirmPrompt("This command stops the dbrm processes within the erydb System"))
					break;
			}
        ...
        case 9: // restartdbrmprocess
        {
			if ( arguments[1] != "y" ) {
				// confirm request
				if (confirmPrompt("This command restarts the dbrm processes within the erydb System"))
					break;
			}
        ...
        case 11: // stopPrimProcs
        {
			if ( arguments[1] != "y" ) {
				// confirm request
				if (confirmPrompt("This command stops the PrimProc processes within the erydb System"))
					break;
			}
        ...
        case 13: // restartPrimProcs
        {
			if ( arguments[1] != "y" ) {
				// confirm request
				if (confirmPrompt("This command restarts the PrimProc processes within the erydb System"))
					break;
			}
        ...
        case 14: // stopExeMgrs
        {
			if ( arguments[1] != "y" ) {
				// confirm request
				if (confirmPrompt("This command stops the ExeMgr processes within the erydb System"))
					break;
			}
        ...
        case 16: // restartExeMgrs
        {
			if ( arguments[1] != "y" ) {
				// confirm request
				if (confirmPrompt("This command restarts the ExeMgr processes within the erydb System"))
					break;
			}
        ...
        case 18: // distributeconfigfile - parameters: option, moduleName
        {
            ...
            try
            {
                ...
                cout << endl << "   Successful Distribution of erydb Config File" << endl << endl;
            }
            catch (exception& e)
            {
                cout << endl << "**** Distribution of erydb Config File Failed :  " << e.what() << endl;
            }
        }
        break;
...
void printSystemStatus()
{
	...
	try
	{
			...
			if ( serverInstallType != oam::INSTALL_COMBINE_DM_UM_PM )
			{
				...
				if ( moduletypeconfig.ModuleCount > 1 )
				{
					if ( PrimaryUMModuleName != oam::UnassignedName )
						cout << "Primary Front-End erydb Module is '" << PrimaryUMModuleName << "'" << endl;
				}
			}
			else
			{
		...
		if ( MySQLRep == "y" )
			cout << "erydb Replication Feature is enabled" << endl << endl;
	}
	catch (exception& e)
	{
...
void printProcessStatus(std::string port)
{
	...
	cout << endl << "erydb Process statuses" << endl << endl;
