...
const std::string FE_MOUNT_DIR = "/var/log/erydb/";	// FE mount dir
...
//		serverMonitor.logCPUactive(averageCpuUsage);
		...
		while(p != pcl.end())
		{
			...
			// check if a erydb Process, if so alarm is over thresholds
...
void ServerMonitor::getCPUdata()
{
	...
	system("top -b -n1 | head -12 | awk '{print $9,$12}' | tail -5 > /tmp/erydb_tmp_files/processCpu");

	ifstream oldFile1 ("/tmp/erydb_tmp_files/processCpu");
	...
	//
	system("top -b -n 6 -d 1 | awk '{print $5}' | grep %id > /tmp/erydb_tmp_files/systemCpu");

	ifstream oldFile ("/tmp/erydb_tmp_files/systemCpu");
