...
int ServerMonitor::healthCheck(bool action)
{
	...
	//run Health script
	string cmd = startup::StartUp::installDir() + "/bin/dbhealth.sh > /var/log/erydb/dbfunctional.log1 2>&1";
	system(cmd.c_str());

	if (!oam.checkLogStatus("/var/log/erydb/dbfunctional.log1", "OK")) {
		if (oam.checkLogStatus("/var/log/erydb/dbfunctional.log1", "ERROR 1045") ) {
			LoggingID lid(SERVER_MONITOR_LOG_ID);
			MessageLog ml(lid);
			Message msg;
			Message::Args args;
