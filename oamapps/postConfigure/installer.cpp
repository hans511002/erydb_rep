...
int main(int argc, char *argv[])
{
	...
	string erydb_rpm1 = argv[1];
	string erydb_rpm2 = argv[2];
	string erydb_rpm3 = argv[3];
	...
	cout <<  erydb_rpm1 << endl;
	cout << erydb_rpm2 << endl;
	cout << erydb_rpm3 << endl;
	cout << mysql_rpm << endl;
   ...
   // restore cout stream buffer
	...
	//copy erydb.xml.rpmsave if upgrade option is selected
	if ( installType == "upgrade" ) {
		cmd = "/bin/cp -f " + installDir + "/etc/erydb.xml " + installDir + "/etc/erydb.xml.new 2>&1";
		system(cmd.c_str());
		cmd = "/bin/cp -f " + installDir + "/etc/erydb.xml.rpmsave " + installDir + "/etc/erydb.xml 2>&1";
		system(cmd.c_str());
	}
	...
	//store syslog config file in erydb Config file
		...
		for (; list1 != childmodulelist.end() ; list1++)
		{
				...
				(remoteModuleType == "pm" && IserverTypeInstall == oam::INSTALL_COMBINE_DM_UM_PM) )
			{
				...
				if ( packageType != "binary" ) {
					...
					cmd = installDir + "/bin/user_installer.sh " + remoteModuleName + " " + remoteModuleIP + " " + password + " " + erydb_rpm1 + " " + erydb_rpm2 + " " + erydb_rpm3 + " " + mysql_rpm + " " + mysqld_rpm + " " + installType + " " + packageType + " " + nodeps + " " + temppwprompt + " " + installer_debug;
				...
				{	// do a binary package install
					cmd = installDir + "/bin/binary_installer.sh " + remoteModuleName + " " + remoteModuleIP + " " +
						password + " " + erydb_rpm1 + " " + remoteModuleType + " " + installType + " " +
			...
			else
			{
				if (remoteModuleType == "pm")
				{
					if ( packageType != "binary" ) {
						//run remote installer script
						cout << endl << "----- Performing Uninstall on Module '" + remoteModuleName + "' -----" << endl << endl;
						cmd = installDir + "/bin/performance_installer.sh " + remoteModuleName + " " + remoteModuleIP + " " + password + " " + erydb_rpm1 + " " + erydb_rpm2 + " " + erydb_rpm3 + " " + mysql_rpm + " " + mysqld_rpm + " " + installType + " " + packageType + " " + nodeps + " " + installer_debug;
						int rtnCode = system(cmd.c_str());
						if (WEXITSTATUS(rtnCode) != 0) {
	...
	string erydbstartcmd = installDir + "/bin/erydb start";
	if ( IserverTypeInstall != oam::INSTALL_COMBINE_DM_UM_PM || 
		...
		cout << "System Install successfully completed, starting erydb" << endl;
		...
		for (; list1 != childmodulelist.end() ; list1++)
		{
			...
			cmd = installDir + "/bin/remote_command.sh " + remoteModuleIP + " " + password + " '" + erydbstartcmd + "' " +  installer_debug;
			int rtnCode = system(cmd.c_str());
			if (WEXITSTATUS(rtnCode) != 0)
		...
		int rtnCode = system(erydbstartcmd.c_str());
		if (rtnCode != 0)
			cout << "Error starting erydb local module" << endl;
		else
			cout << "Start erydb request successful" << endl;
	}
	else
	{
		// 
		...
		else
		{
			...
			//startup mysqld and erydb processes
			cout << endl;
			cmd = installDir + "/bin/clearShm > /dev/null 2>&1";
			system(cmd.c_str());
			system(erydbstartcmd.c_str());
		}
	}
	...
	{
		cout << " FAILED" << endl;
		cout << endl << "ERROR: erydb Process failed to start, check log files in /var/log/erydb" << endl;
	}
}
...
bool setOSFiles(string parentOAMModuleName, int serverTypeInstall)
{
	...
	for ( int i=0;;++i)
	{
		...
		if ( files[i] == "rc.local") {
			cmd = "touch " + installDir + "/local/etc/" + parentOAMModuleName + "/rc.local.erydb"; 
			system(cmd.c_str());
		}
		...
		else
		{
		    ...
		    cmd = "sudo cat " + installDir + "/local/etc/" + parentOAMModuleName + "/" + files[i] + ".erydb >> " + fileName;
		    if (geteuid() == 0) system(cmd.c_str());

		    cmd = "sudo rm -f " + installDir + "/local/ " + files[i] + "*.erydb > /dev/null 2>&1";
		    system(cmd.c_str());

		    cmd = "sudo cp " + installDir + "/local/etc/" + parentOAMModuleName + "/" + files[i] + ".erydb " + installDir + "/local/. > /dev/null 2>&1"; 
		    system(cmd.c_str());
		}
	}
...
bool updateProcessConfig(int serverTypeInstall)
{
	...
	cmd = "/bin/cp -f " + fileName + " " + fileName + ".erydbSave > /dev/null 2>&1";
	system(cmd.c_str());
