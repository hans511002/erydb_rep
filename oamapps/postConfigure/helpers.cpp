...
namespace installer
{
    ...
    bool waitForActive() {
        ...
        const string cmd = installDir + "/bin/eryadmin getsystemstatus > /tmp/wait.log";
        system(cmd.c_str());
    ...
    void dbrmDirCheck() {
        const string fname = installDir + "/etc/erydb.xml.rpmsave";
        ...
        if (!File2) {
            ...
            cout << "Please reference the EryDB Installation Guide on Upgrade Installs for" << endl;
        ...
        cmd = "chmod 755 -R " + installDir + "/metadata/dbrm > /dev/null 2>&1";
        system(cmd.c_str());
        return;
    }
                    ...
                    try {
                        ...
                        if (opState == oam::ACTIVE ||
                            ...
                            if (returnStatus != API_SUCCESS) {
                                cout << "ERROR: Error return in running the EryDB Upgrade, check /tmp/upgrade*.logs on " << (*pt).DeviceName << endl;
                                return returnStatus;
                            }
                        }
                    } catch (const std::exception &exc) {
                    ...
                    try {
                        ...
                        if (opState == oam::ACTIVE ||
                            ...
                            if ((*pt).DeviceName == masterModule) {
                                ...
                                if (returnStatus != API_SUCCESS) {
                                    cout << endl << "ERROR: Error return in running the erydb Master DB Distribute, check /tmp/master-dist*.logs on " << masterModule << endl;
                                    return returnStatus;
                                }
                                ...
                                if (returnStatus != API_SUCCESS) {
                                    cout << endl << "ERROR: Error return in running the erydb Master replication, check /tmp/master-rep*.logs on " << masterModule << endl;
                                    return returnStatus;
                                }

                                masterDone = true;
                                pt = systemmoduletypeconfig.moduletypeconfig[i].ModuleNetworkList.begin();
                            } else { // set for slave repl request
                                ...
                                if (returnStatus != API_SUCCESS) {
                                    cout << endl << "ERROR: Error return in running the erydb Slave replication, check /tmp/slave-rep*.logs on " << (*pt).DeviceName << endl;
                                    return returnStatus;
                                }
    ...
    ******************************************************************************************/
    ...
    void checkMysqlPort(std::string& mysqlPort, Config* sysConfig) {
        ...
        while (true) {
            ...
            if (oldFile) {
                ...
                if (size != 0) {
                    if (noPrompting) {
                        cout << endl << "The erydb port of '" + mysqlPort + "' is already in-use" << endl;
                        cout << "For No-prompt install, use the command line argument of 'port' to enter a different number" << endl;
                        exit(1);
                    }

                    cout << "The erydb port of '" + mysqlPort + "' is already in-use on local server" << endl;
    ...
    void checkSystemMySQLPort(std::string& mysqlPort, Config* sysConfig, std::string USER, std::string password, ChildModuleList childmodulelist, int IserverTypeInstall, bool pmwithum) {
        ...
        while (true) {
            ...
            if (!inUse) {
                ...
                for (; list1 != childmodulelist.end(); list1++) {
                    ...
                    if (remoteModuleType == "um" ||
                        ...
                        if (WEXITSTATUS(rtnCode) == 0) {
                            if (noPrompting) {
                                cout << endl << "The erydb port of '" + mysqlPort + "' is already in-use on " << remoteModuleName << endl;
                                cout << "For No-prompt install, use the command line argument of 'port' to enter a different number" << endl;
                                cout << "exiting..." << endl;
                                exit(1);
                            } else {
            ...
            if (inUse) {
                cout << endl << "The erydb port of '" + mysqlPort + "' is already in-use on " << inUseServer << endl;
                while (true) {
                    prompt = "Enter a different port number > ";
                    pcommand = callReadline(prompt.c_str());
                    if (pcommand) {
