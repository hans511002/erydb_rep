...
int main(int argc, char *argv[])
{
   ...
   for( int i = 1; i < argc; i++ )
   {
		if( string("-h") == argv[i] || string("--help") == argv[i]) {
			cout << endl;
			cout << "This is the Amazon erydb AMI System Configuration and Installation tool." << endl;
			cout << "It will Configure and startup an Amazon erydb System." << endl << endl;
			...
			cout << "   -v  	erydb version" << endl;
			cout << "   -pc	postConfigure failure System Cleanup, used to run System Cleanup if erydb fails to install" << endl; 
			...
			cout << endl << "erydb Version: " << systemsoftware.Version << "-" << systemsoftware.Release << endl;
			exit (0);
		}
   ...
   if (oam.checkSystemRunning()) {
		cout << endl << "EryDB is running, can't run AmazonInstaller while EryDB is running. Exiting.." << endl;
		exit (0);
   }
	...
	cout << endl;
	cout << "This is the Amazon erydb AMI System Configuration and Installation tool." << endl;
	cout << "It will Configure and startup an Amazon erydb System." << endl;

	//check if erydb is up and running
	if (oam.checkSystemRunFlag() && oam.checkSystemRunning()) {
		cout << "EryDB is running, can't run amazonInstaller while EryDB is running. Exiting.." << endl;
		exit (0);
	}

	//backup current erydb.xml
	string configFile = installDir + "/etc/erydb.xml";
	string saveFile = installDir + "/etc/erydb.xml.save";
	...
	//backup original erydb.xml if it doesn't exist
	//if it does exist copy to erydb.xml
	...
	if (!oldFile) {
		...
		//get root password
		prompt1 = "Enter Root-Password used to access the Instances or hit enter to default to 'erydb1' > ";
		pcommand1 = readline(prompt1.c_str());
		if (pcommand1) {
		...
		region == "ap-southeast-1" )
	{
		...
		{
			cout << endl << "ERROR: Problem setting AmazonRegion from the erydb System Configuration file" << endl;
			exit(1);
		}
	}
	else
	...
	cout << "erydb Version = " << systemsoftware.Version << "-" << systemsoftware.Release << endl;
	...
	if ( TotalUmMemory != oam::UnassignedName ) {
		...
		{
			cout << endl << "ERROR: Problem setting SystemName from the erydb System Configuration file" << endl;
			exit(1);
		}
	}
	...
	if ( systemType == "separate" ) {
		...
		{
			cout << "ERROR: Problem setting UMSecurityGroup from the erydb System Configuration file" << endl;
			exit(1);
		}
		...
		{
			cout << "ERROR: Problem setting InstanceType from the erydb System Configuration file" << endl;
			exit(1);
		}
	...
	cout << endl << "===== erydb Configuration Setup and Installation =====" << endl << endl;

	if ( systemType == "combined" ) {
		cout << "----- Combined System Type - Setup and Install erydb-MySQL Packages -----" << endl << endl;
		...
		//install rpms
		system("rpm -ivh /root/erydb-mysql*rpm /root/erydb-storage-engine*rpm");
		cout << endl;
	}

	// 
	// update /root/erydb.xml
	//

	cout << "----- Updating erydb Configuration File (erydb.xml)  -----" << endl << endl;
	...
	if ( elasticiplist.size() > 0 )
	{
		...
		{
			cout << "ERROR: Problem setting AmazonElasticIPCount in the erydb System Configuration file" << endl;
			cleanupSystem();
		}
	...
	//copy erydb.xml to erydb.xml.rpmsave for postConfigure no-prompt option
	cmd = "rm -f " + installDir + "/etc/erydb.xml.rpmsave";
	system(cmd.c_str());
	cmd = "cp " + installDir + "/etc/erydb.xml " + installDir + "/etc/erydb.xml.rpmsave";
	int rtnCode = system(cmd.c_str());
	if (WEXITSTATUS(rtnCode) != 0) {
		cout << "Error copying erydb.xml to erydb.xml.rpmsave" << endl;
		cleanupSystem();
	}
	...
	if ( postConfigureLog ) {
		...
		if (WEXITSTATUS(ret) == 0 )
			...
			if (postConfigureCleanup) {
				cout << "Running erydb-support script is capture system logs and other data" << endl << endl; 

				string cmd = installDir + "/bin/erydbSupport -p erydb1 -hw -s -c -r -l";
				system (cmd.c_str());
				cleanupSystem();
			}
		}
...
void snmpAppCheck()
{
	...
	cout << "This would be used to receive SNMP Traps from erydb, like a Network Control Center" << endl;
...
void cleanupSystem(bool terminate)
{
	...
	if ( terminate ) {
		...
		system("umount /usr/local/erydb/data* > /dev/null 2>&1");
	...
	if ( systemType == "combined" ) {
		cout << "----- Combined System Type Uninstall erydb-MySQL Packages -----" << endl << endl;

			system("rpm -e erydb-mysql erydb-engine");
	}
