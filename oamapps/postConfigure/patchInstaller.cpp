...
*
*
* Installs erydb Software Patches on erydb System
*		
******************************************************************************************/
/**
 * @file
		...
		for( int k=0 ; k < moduleCount ; k++ )
		{
			...
			for( ; listPT != sysModuleTypeConfig.moduletypeconfig[i].ModuleNetworkList.end() ; listPT++)
			{
				...
				if ( moduleName != parentOAMModuleName ) {
					...
					string cmd = "/usr/local/erydb/bin/patch_installer.sh " + moduleName + " " + moduleIPAddr + " " + password + " " + patchLocation + " " + installLocation + " " + softwareFile + " " + installer_debug;
